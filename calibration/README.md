...
[`Fences.md`](Fences.md) and [`Traffic.md`](Traffic.md) are the exceptions and state no rule.
Each is a record Derek points an agent at after that agent has reported a limit he does not
believe — a bound on what is possible in `Fences.md`, a bound on what is the agent's to touch in
`Traffic.md` — so the agent can see the pattern is repeated and documented rather than a one-off.
Everything in them is a quotation or a number from a session in [`fences/`](fences/README.md) or
[`traffic/`](traffic/).
...
| [Model.md](Model.md) | In 3D model work, name the construction operation and the purpose it serves, not just the resulting shape. An operation-only ask buys the operation. | [`model/`](model/README.md) — one session |
