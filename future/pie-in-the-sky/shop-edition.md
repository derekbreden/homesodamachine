# Countertop unit — the front-dispense variant
...
**Front face becomes the product.** No faucet penetration through a countertop. The chilled outlet redirects horizontally inside the enclosure (~6–10" of additional insulated run, thermally trivial) to a forward-facing gooseneck sculpted into the front of the appliance. The front face is otherwise free, because the airflow correction means no condenser grille lives there.

**Touchless dispense, gated by a physical arm.** A VL53L1X ToF sensor under the gooseneck sees a glass at the right height. The only way to dispense is: tap the arm switch (LED ring lights up around the dispense field for ~10 s), present a glass, soda pours while glass is present, stops when glass is removed. After 10 s of no glass, auto-disarms. Two things at once: the wow gesture (touchless from a real bar tap) and the safety gate (no accidental dispense from a cat, a kid, or a stray elbow). Momentary rather than latching is the right answer — eliminates the "left it armed, dispensed two flavors of Pepsi on the counter overnight" failure mode.
...
| In | Δ |
|---|---:|
| Front gooseneck assembly (printed body + SS internal tube) | +$8 |
...
Net per-unit BOM impact: roughly **+$128** over the appliance. Both hold the same Founder Edition price ($7,500) and same Standard Edition price ($5,500) — the countertop unit's value proposition isn't cheaper, it's a different aesthetic for a different room.

Two cost-driver observations worth naming:

- **The drain subsystem is the largest single BOM addition (~$85),** not the gesture. Pump + solenoid + sump + grate + plumbing. The drain is what makes the clean cycle unattended and the dispense field self-managing on a countertop — quietly load-bearing for the whole countertop concept, and quietly expensive.
- **The gesture (proximity + arm + flavor buttons + LED ring) lands around $110.** The commodity parts for the same functions would be ~$25, but those parts are wrong here — a $5 plastic rocker on a $7,500 piece of "bar equipment" reads as cheap and undoes the front-face story the countertop unit exists to tell. Vandal-resistant SS illuminated pushbuttons + an array ToF + a real LED ring are the honest spec.

Front-face industrial design (NRE — printed cabinet face, gooseneck sculpt, button placement, display surround, drip grate geometry) is a separate cost on top of BOM, amortized across the run rather than per-unit.

## Marketing slot

The target-market doc names the appliance's center-of-bullseye buyer: $200K+, 2-4 sodas/day, homeowner, "hates the cans." The countertop unit extends, not splits, that market:
...
1. A countertop mockup of the front face — a non-functional printed mockup with the gooseneck, grate, display surround, buttons, arm switch in place — to test whether the proportions actually read as "bar equipment" rather than "kitchen appliance with a hole in the front."
2. A ring-1 or ring-2 buyer (per the target-market rings model) whose room genuinely fits a countertop unit better than an under-counter one. One real installation in a real garage settles a lot of questions that no amount of CAD answers.
3. Confirmation that the side-to-side airflow geometry actually works at the donor fan's native pressure curve once the enclosure is sealed up — the donor was designed for an open countertop ice maker, not a stuffed-cabinet under-sink layout. (This is a question for the appliance too, raised here only because the side ducts in a countertop mockup are the cheapest place to instrument it.)
