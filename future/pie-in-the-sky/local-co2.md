...
Underneath the tool: a markdown directory in the repo, one file per metro (or one per state, as it scales), maintained by us via agent-assisted research. Each entry carries the same fields:
...
- The CGA-320 adapter kit customer ([`cga320-kit.md`](/future/pie-in-the-sky/cga320-kit.md)) who refills their own tank and would rather not pay for delivery.
- The appliance customer's first cylinder refill.
...
- **Upgrade path to [`co2-service.md`](/future/pie-in-the-sky/co2-service.md).** The guide ends with: "or, if `drive somewhere during business hours` is the part you hate, we deliver for $250 per refill. Click here." A free tool that funnels into a paid service is a real conversion mechanism.

## How it relates to [`co2-service.md`](/future/pie-in-the-sky/co2-service.md)
...
A full national directory is a substantial project. The minimum viable launch is much smaller:

1. **One metro to start.** Lincoln NE — Derek's home market — plus Omaha as the obvious nearby second. Three to five fill points per metro. Verified in person by Derek.
...
1. **Write the Lincoln + Omaha directory entries.** This is the smallest version of the product and it costs nothing to ship.
...
3. **Plan the agent-assisted expansion path.** Agent resources make metro-level research cheap enough to do at a fine pass — Airgas branches, paintball stores, homebrew shops, food-grade availability, hours, prices. The per-metro template: pick the next metro, run a research pass (Google Maps, supplier websites), then publish. Out of scope before the first metro is live.
