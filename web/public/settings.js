...
//   1. Notifications — PWA-only. Lazy-imports Firebase Messaging,
//      registers the SW, requests permission, and POSTs files=["*"] for
//      a global subscription. Off DELETEs the subscription.
//   2. Live-reload debug — flips boot.js's on-screen panel via
//      window.__hsmLiveDebug (localStorage hsmLiveDebug).

(function () {
  // --- Live-reload debug ---
  // Toggles boot.js's on-screen panel (socket health, build commit,
  // deploy events). window.__hsmLiveDebug.set flips the panel without a
  // reload; the localStorage fallback covers the panel appearing on next
  // load if boot.js hasn't run yet.
