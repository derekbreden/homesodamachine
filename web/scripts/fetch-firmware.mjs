// The images named in firmware/firmware-images.json, put on this disk.
//
//     node scripts/fetch-firmware.mjs            # fetch what is missing or wrong
//     node scripts/fetch-firmware.mjs --check    // 0 = every image here matches the pointer file
//
// Render runs this after `npm ci` (render.yaml), from web/. `/api/firmware` serves the manifest
// off the pointer file and `/firmware/<target>.bin` serves the bytes off `web/public/firmware/`, so a
// deploy clone — which carries the pointer file and none of the images — fills them in here.
//
...
// pointed-at image, and the other reads them and finds nothing to do.
//
...
// crc32 in the same pointer file by the board receiving it, so the bytes that reach flash are checked
// twice against one committed file.
//
// A tree that already holds every image at its pointed-at hash downloads nothing, so the machine that
...
if (!pointers) {
  console.log("[firmware] no pointer file — nothing to fetch");
  process.exit(0);
}

const images = pointers.images ?? {};
...
if (drifted.length) {
  console.log(`[firmware] ${drifted.length} image(s) hold bytes the pointer file does not name, left as they are:`);
  for (const target of drifted) console.log(`    ${target}`);
  console.log("    ~/.platformio/penv/bin/python tools/publish_firmware.py --write");
}

if (missing.length === 0) {
  console.log(`[firmware] ${Object.keys(images).length - drifted.length} image(s) at the pointed-at hash`);
...
console.log(`[firmware] ${missing.length} image(s) to fetch — ${asset} (${(pointers.bundle.bytes / 1e6).toFixed(1)} MB)`);
...
try {
  ...
  // `sha256` below, which is what decides whether the bytes about to be served are the pointed-at
  ...
  if (got !== pointers.bundle.sha256) {
    console.warn(`[firmware] ${asset} is not the pointed-at bundle`);
  ...
  for (const target of missing) {
    ...
    else if ((await sha256(file)) !== entry.sha256) bad.push(`${target} — not the pointed-at bytes`);
  }
  if (bad.length) {
    console.warn(`[firmware] ${bad.length} image(s) the pointer file does not vouch for:`);
