...
def git_rev():
    """Short commit SHA, with a `-dirty` suffix when the working tree has
    uncommitted tracked changes or untracked files. Returns "unknown" when
    git is unavailable or this is not a repo.

    This is the provenance stamp: a clean rev means the binary is exactly
    reproducible from that commit; `-dirty` means it was built from that
    commit plus uncommitted edits and so corresponds to no commit. The
    generated fw_version.h files are gitignored, so a rebuild never makes
    the tree look dirty on its own.
    """
