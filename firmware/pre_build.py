...
def fw_build_epoch():
    """HEAD's commit time as a Unix timestamp, which is what orders two builds.

    FW_VERSION is what a person reads, and a date is as fine as a string a
    screen has room for can be. Two builds made on one day are not ordered by
    it, and the phone deciding whether a published image is an update to a
    running one has to be sure — offering a machine its own past is the one
    error that reaches a board, because nothing downstream re-checks. So the
    ordering travels beside the display string rather than inside it, exact to
    the second, and neither constrains the other.

    It is HEAD's commit time, not the build's, for the same reason the version
    string carries the commit's date: it is a property of the source, so two
    machines building the same commit agree, and a rebuild does not invent a
    newer image. A build with uncommitted edits carries its commit's time, so
    it never sorts ahead of the commit it was made from.

    Zero when git cannot say, which every reader treats as "this board did not
    tell me" rather than as the epoch.
    """
    project_dir = env.subst("$PROJECT_DIR")
    try:
        r = subprocess.run(
...
write_if_moved(
    ...
    f"#define FW_BUILD_EPOCH {fw_build_epoch()}u\n",
)
...
# The loading animation is not in that board's image; it is in the `art`
