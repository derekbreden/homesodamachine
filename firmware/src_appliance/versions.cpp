...
static uint32_t faucetEpoch = 0;
static uint32_t enclosureEpoch = 0;
...
void versionsOnReport(uint8_t board, const char *version, uint32_t artCrc32,
                      uint32_t buildEpoch) {
    if (board == OTA_TGT_ENCLOSURE) { enclosureArtCrc = artCrc32; enclosureEpoch = buildEpoch; }
    if (board == OTA_TGT_FAUCET) faucetEpoch = buildEpoch;
...
static void put(VersionPayload &e, uint8_t board, const char *v,
                uint32_t buildEpoch, uint32_t artCrc32 = 0) {
    e.board = board;
    memset(e.version, 0, sizeof(e.version));
    strncpy(e.version, v, FW_VERSION_MAX);
    e.artCrc32 = artCrc32;
    e.buildEpoch = buildEpoch;
}

void versionsFill(VersionsPayload &out) {
    out.count = 3;
    put(out.entries[0], OTA_TGT_SELF, FW_VERSION, FW_BUILD_EPOCH);
    put(out.entries[1], OTA_TGT_FAUCET, faucetV, faucetEpoch);
