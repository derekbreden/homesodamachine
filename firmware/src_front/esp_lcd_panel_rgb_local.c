...
// This appliance keeps the upstream underflow/manual-recovery path, but does
...
// The normal scan ISR is latency-sensitive. Keep the local RGB driver and the
// two application callbacks it validates in IRAM so flash/PSRAM activity from
// a wake cannot delay its underflow recovery into active pixels.
#undef CONFIG_LCD_RGB_ISR_IRAM_SAFE
...
#include "esp_psram.h"
#include "esp_lcd_common_local.h"
#include "esp_cache.h"
...
// Reset GDMA only when the application explicitly requests it or a bounce-buffer
// EOF was missed. A continuous, healthy stream must not reset at every VSYNC:
// restarting after the LCD controller has started a frame can re-send its first
// bytes and visibly offset that one frame.
...
// Linked by firmware/src_front/main.cpp. This is intentionally a read-only
// diagnostic boundary; panel recovery stays wholly inside the driver.
uint32_t home_soda_rgb_restart_count(void)
{
    return rgb_panel_restart_count;
}
