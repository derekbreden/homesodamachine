...
const uint16_t *frontArtMap(uint32_t wantCount, uint16_t wantW, uint16_t wantH) {
  ...
  if (h.magic != FRONT_ART_MAGIC) return nullptr;
  if (h.format != FRONT_ART_FORMAT) return nullptr;
  if (h.count != wantCount || h.w != wantW || h.h != wantH) return nullptr;

  const uint64_t need = (uint64_t)FRONT_ART_HEADER_SIZE +
                        (uint64_t)wantCount * wantW * wantH * 2u;
  if (need > artPart->size) return nullptr;

  return (const uint16_t *)((const uint8_t *)artBase + FRONT_ART_HEADER_SIZE);
}

bool frontArtVerify() {
  FrontArtHeader h;
  if (!frontArtHeader(h)) return false;
  if (h.magic != FRONT_ART_MAGIC) return false;

  const uint32_t bytes = h.count * (uint32_t)h.w * h.h * 2u;
  if ((uint64_t)FRONT_ART_HEADER_SIZE + bytes > artPart->size) return false;

  const uint8_t *p = (const uint8_t *)artBase + FRONT_ART_HEADER_SIZE;
  uint32_t crc = 0;
