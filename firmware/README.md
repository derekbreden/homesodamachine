...
- **Relay #1 (`IO19`) and the condenser fan follow the cold loop.** `lib/machine_policy/cold_policy.h` is the whole decision — the setpoint band on the carbonator wall, the freeze cutout on the suction line, the compressor's minimum off-time and the parking of both on a reading that is missing or stale. The condenser fan shares its write with every valve, so `machine.cpp` routes them all through one `applyOutputs()` and a valve change during a run does not stop the fan.
- **Relay #1 (`IO19`) and the condenser fan follow the cold loop.** [`lib/machine_policy/cold_policy.h`](lib/machine_policy/cold_policy.h) is the whole decision — the band on the carbonator wall, the freeze cutout on the suction line, the compressor's minimum off-time, and both parked on a reading that is missing or stale. The fan shares its write with every valve, so `machine.cpp` routes all of them through one `applyOutputs()`: a valve change during a compressor run does not stop the condenser.
- **Relay #2 (`IO2`) off while a dispense is open.** The main board peaks at 3.33 A and the SeaFlo diaphragm pump at 5 A on the same 12 V rail — 8.32 A together, against a 6.7 A supply. The carbonator's low reed asserts mid-pour, so the refill it queues waits for the dispense window to close ([`/hardware/assembly/acceptance-and-burn-in.md`](/hardware/assembly/acceptance-and-burn-in.md) step 5). Nothing in hardware enforces this. `machine_policy::kRefillDuringDispense` is the policy that refuses such a plan and `machineDispenseWindowOpen()` is the accessor that asks. [`lib/machine_policy/refill_policy.h`](lib/machine_policy/refill_policy.h) is the draw itself: CLO debounced, the ask held in `Queued` while the window is open, the latch to CHI, a ceiling on pumping time and a fault on two reeds that cannot both be closed. A pour beginning mid-draw stops the draw before it opens its own path.
...
`pio test -e native` holds these policies off-board: it checks the cold loop's band, its freeze
cutout and recovery, its minimum on- and off-times and every path out of a reading it cannot
believe; the refill's debounce, dispense interlock, latch, ceiling and fault; the canonical operation table,
