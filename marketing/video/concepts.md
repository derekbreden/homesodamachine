...
- **Where it lives in the repo:** `tools/dev-server/server.js` (dev wrapper, file watcher, SSE), `lib/push.js` (FCM push on STEP/Mermaid changes), `lib/viewer-pages.js` and `lib/viewer-routes.js` (shared production+dev viewer).
