...
12. **Render.** [`cut.py`](cut.py) extracts each cut, generates a title card (still color background + centered text, resolution + fps matched to source), prepends it to the chain, applies cross-dissolves between every cut, draws section / tool overlays per cut, then burns the captions in. Use `--preset fast --crf 23` for review drafts, `--preset medium --crf 18` for final lock. See cut.py's header for the chained-xfade gotchas (TL;DR: don't use `setpts`; do re-pin `format=yuv420p,fps={rate}` after every xfade).
    - **Motion-graphics overlays (optional).** Pass `--overlays <sidecar.json>` to layer text callouts, persistent HUD readouts, lower thirds, big "stamp" reactions, outlined boxes, picture-in-picture, or magnifier insets onto the rendered video. Schema lives in [`overlays.py`](overlays.py); times are authored against the OUTPUT timeline so the value you see on the draft is the value you write in the sidecar. When `--overlays` is set, captions are burned in the same final pass as the overlays (one extra encode vs. captions-only).
...
16. **Upload.** YouTube Studio. Click upload, select the rendered MP4, paste title + description, upload custom thumbnail (the same 1280×720 PNG), set audience to "not made for kids," skip subtitles (we burn them in), set Public, publish. Once published, the video *file* is immutable — you can edit metadata (title, description, thumbnail) and add audio from Studio → Editor → Audio's curated subset, but you cannot replace the file or add music from outside that subset without taking the video down and re-uploading (which creates a new URL, requires a follow-up post-URL edit, and forces a permanent delete of the old version). The Editor's audio subset is much smaller than the full Audio Library at `studio.youtube.com/channel/<id>/music`; for any track that's only in the full library, mux it into the local MP4 *before* this step. One-shot mux against the no-audio render: `ffmpeg -i out.mp4 -i music.mp3 -filter_complex "[1:a]afade=in:st=0:d=1,afade=out:st=<end-2>:d=2,volume=0.85[a]" -map 0:v -map "[a]" -c:v copy -c:a aac -b:a 192k -shortest out-with-music.mp4`.
...
- [`sync.py`](sync.py), [`segment.py`](segment.py), [`make-subtitles.py`](make-subtitles.py), [`cut.py`](cut.py), [`overlays.py`](overlays.py), [`export-from-photos.sh`](export-from-photos.sh) — the pipeline scripts. Each script's header has the gotchas + design rationale.
