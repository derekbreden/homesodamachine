...
14. **Thumbnail.** [`../thumbnail/make.sh`](../thumbnail/make.sh) takes a source image + headline text and produces a 1280×720 PNG with white-on-black-outline text lifted above YouTube's duration pill. Pre-crop the source frame in ImageMagick if the action isn't already centered — make.sh's center-crop assumes it is.
...
17. **(Optional) Caption fix.** YouTube auto-captions still run in parallel to the burned-in captions; if YouTube's version has an error that crosses the brand-cringe threshold, fix it in Studio → Subtitles → click into the auto-caption editor. Burned-in captions are unaffected.
18. **Updates-feed post on homesodamachine.com.** Write a video-launch post at `posts/YYYY-MM-DD-HHMM.md` per the post format documented in [`../../posts/README.md`](../../posts/README.md) — specifically the *Video-launch posts* section. Reuse the YouTube thumbnail PNG: copy the 1280×720 PNG to `public/post-images/YYYY-MM-DD-<slug>-thumbnail.png` and link to the YouTube watch URL. Example: `posts/2026-05-08-1145.md` accompanying *I've Never Tapped*. Committing the post triggers an FCM push to subscribers — treat as a publish action.
