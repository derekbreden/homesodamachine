...
[`internal-plumbing.md`](/hardware/assembly/internal-plumbing.md) §3 begins before enclosure closure because `enclosure-front-top` is the assembly fixture. Its order is bare tees loaded into the filled body; aft valves raised from the open underside and pressed into their sockets; right carrier half fed inward at release, then parked; left half fed inward at connected and moved to release; right half brought fore and upper lap closed with two M3 screws through the empty cartridge bay; two ties per tee; compressed springs fed through the inner tee wells to their seats below the upper guide; fore valves and four bowed flex stubs; then cartridge connection after the box closes by squeeze, bottom and release. The rest of internal plumbing follows [`enclosure-mechanical.md`](/hardware/assembly/enclosure-mechanical.md), and [`wiring.md`](/hardware/assembly/wiring.md) takes a chassis that already carries its plumbing. The card deck in [`/hardware/assembly/cards/`](/hardware/assembly/cards/) prints in this order.
