...
| [`scripts/`](/hardware/scripts/) | Project Python tooling: [`probe.py`](/hardware/scripts/probe.py) (ask the placed machine a geometry question instead of reasoning about it — where a body is, how close two come, what a volume runs into, how far a line runs, and where a piece of a routed line can stand), `_cadq_export.py` (the shared atomic STEP/DXF/PDF export helper imported tree-wide) and the doc-sync / totals generators that maintain the `ledger/` docs. |
