...
The soda machine's physical design — the integrated under-counter unit that consolidates carbonator, refrigeration, flavor reservoirs, pumps, valves, and electronics into one enclosure.
...
The carbonator is a custom 316L stainless pressure vessel standing vertically, with four NPT ports in two identical plates. The top water-inlet elbow carries a small jet that sends filtered tap water through the CO2 headspace into the water below. CO2 enters through a plain bottom port, carbonated water leaves through the other bottom port, and a dedicated pressure-relief valve guards the top. Temperature, CO2 pressure, jet mixing and refill timing together determine the carbonation delivered to the glass. The water level inside is read without piercing it — a magnet rides on an internal rod and external reed switches see it through the wall.

The selected diaphragm pump is the received [G Ganen sample](/hardware/reference/g-ganen-pump/README.md). The enclosure uses its measured rigid housing and ports, with four identical nominal 7 mm rubber feet at the selected rail-end stations. Installed discharge points toward enclosure −X. [Print readiness](/hardware/printed-parts/enclosure/print-readiness.md) records the current build and print queue. The Mean Well IRM-90-12ST supply is the current reference. The [water-inlet jet](/hardware/assembly/water-inlet-jet.md) is a drilled 316 cap welded onto the existing elbow's male tip; its fit and full root fusion require physical qualification. [Acceptance and burn-in](/hardware/assembly/acceptance-and-burn-in.md) establish actual refill, restart and first- and successive-glass performance. [Carbonation Plan B](/future/carbonation-plan-b.md) maps a measured shortfall to a conditional change.
...
The canonical valve-state truth table, with the manifold and tray diagrams beside it, is in [`/hardware/topology/fluid-topology.md`](/hardware/topology/fluid-topology.md). How a valve is held on a printed face is in [`/hardware/printed-parts/valve-seat/`](/hardware/printed-parts/valve-seat/); the carrier, its integral service tabs and center lap joint in [`/hardware/printed-parts/enclosure/tee-carrier/`](/hardware/printed-parts/enclosure/tee-carrier/); the pumps in [`/hardware/printed-parts/enclosure/pump-tray/`](/hardware/printed-parts/enclosure/pump-tray/) and the pump cartridge they ride out on in [`enclosure/`](/hardware/printed-parts/enclosure/enclosure/); the funnel in [`/hardware/printed-parts/zone-c/`](/hardware/printed-parts/zone-c/). The four 12 mm bowed dimensions are exposed developed paths across 10 mm sleeve-face chords, not tube-blank cut lengths. The valve-side insertion depths are bench-fit as each tube blank is made. Derek’s [physical collet observations](/hardware/reference/tee-connector/README.md#observed-push-connect-action) establish the release and relocking action; the assembled unit’s dry cycle checks guided movement, retention and return. The reservoir level-sensing pattern is in [`/hardware/printed-parts/cold-core/reservoir/level-sensing.md`](/hardware/printed-parts/cold-core/reservoir/level-sensing.md). One pump serving N flavors on a single line, switching by reverse-purge and re-prime, is a desire captured in [`/future/pie-in-the-sky/single-pump-flavor-switching.md`](/future/pie-in-the-sky/single-pump-flavor-switching.md).

### Enclosure (back to front)

The enclosure stands in the cabinet beneath the sink, in the slot beside the disposal that [`/marketing/install-envelope.md`](/marketing/install-envelope.md) describes. Read it back to front. The cold core is yawed a quarter turn and seated in the back-bottom corner, flat on the floor and flush at the side walls; the diaphragm pump runs front to back over its lid and is what the rear wall stands off. The band above the core is the service bay: the diaphragm pump down the middle, the electronics bay against the +X wall, the water deck in the −X lane. The compressor stands upright on the floor at the front, bolted to the slab, with the condenser beside it and the **machine corridor** behind — the band the refrigerant loop turns in on its way to the core, and the one the manifold's cross-machine lines run along. The condenser's airflow crosses the cabinet: in through the grille on one side face, out the other. A flat 45° facet chamfers the top-front arris, wall to wall, carrying the machine display; the funnel takes the top wall's full width behind it. The cabinet prints as four telescoping quadrants that cross-pin with screws from the side faces. The pump cartridge and its top clamp ride in the front bay; behind them a fixed release plate fronts four tee journals and a spring-loaded carrier. The carrier has two physical stops separated by 2.15 mm. The fore stop serves release and squeeze, holding the collets fully depressed against the fixed plate. The aft stop serves both connected and empty rest, with the collets fully extended and 0.5 mm from the plate. Pulling the cartridge brings the tied tees to the fore stop; when the four tubes leave, the springs return the empty carrier aft. Insertion is squeeze, bottom all four tubes, release. No face is a panel of its own. The **+Y wall of back-top** lands every connection the machine makes to the world — the water inlet, the CO2 inlet, the AC cord, the signal jack and the umbilical — one row above the deck. The front wall's one opening is the bay, and the pump cartridge's own face closes it.
...
Above the counter, the faucet lever pours, and the faucet display at the end of the gooseneck shows the selected flavor and switches it by touch. On the machine's 45° top-front facet, angled up toward the standing user, the machine display is the configuration surface, over the flat face that is the pump cartridge's own. Flavor is refilled from the top: lift the silicone funnel out and pour. The pumps are reached from the front instead — pull the pump cartridge and both are in your hand. Once the cartridge is clear, its cord unplugs from the pump jack, an RJ11 keystone centred behind the display; the plug's clip faces down into the empty bay, and the fixed lead returns to J13 through the cable clip near that ridge wall's +X edge. Returning it is a deliberate two-hand fluid connection: click the plug into the pump jack, squeeze the two recessed side tabs, bottom the four pump tubes, then release the tabs and confirm they settle evenly at connected.

The faucet's printed parts — the faucet shell, display cover, above-counter plate, above-counter gasket and o-ring — are in [`/hardware/printed-parts/faucet/`](/hardware/printed-parts/faucet/), over the cut [`under-counter plate`](/hardware/cut-parts/faucet/). The displays' reference geometry is in [`/hardware/reference/waveshare-43b-display/`](/hardware/reference/waveshare-43b-display/) and [`/hardware/reference/touch-flo-faucet/`](/hardware/reference/touch-flo-faucet/); the facet the machine display is let into, and the pump cartridge under it, in [`/hardware/printed-parts/enclosure/enclosure/README.md`](/hardware/printed-parts/enclosure/enclosure/README.md); the funnel in [`/hardware/printed-parts/zone-c/README.md`](/hardware/printed-parts/zone-c/README.md). What the customer does on install day is drawn in [`/hardware/quickstart/`](/hardware/quickstart/). The behavior these surfaces drive is firmware — [`/firmware/`](/firmware/).

### Build order

The sequence the whole machine is built in is the dependency chain of the procedure docs in [`/hardware/assembly/`](/hardware/assembly/), with the skilled-hand tasks summarized in [`handwork.md`](/hardware/assembly/handwork.md): carbonator, cold core, refrigerant loop, then three benches that run in parallel with that chain and feed the chassis — cable assemblies, electronics bay, faucet and umbilical — then the loose-front-top manifold mechanism, enclosure closure, the remaining internal plumbing, wiring, commissioning, burn-in, pack and ship.

[`internal-plumbing.md`](/hardware/assembly/internal-plumbing.md) §3 begins before enclosure closure because `enclosure-front-top` is the assembly fixture. Its order is bare tees loaded into the filled body; left carrier half entered through the rear, lowered behind the fixed body and seated outward on the aft stop; right half entered the same way onto the left half's tongue, the full-height lap closed with two M3 screws from the open rear; carrier moved to release; aft valves raised from the underside and pressed into their sockets; two ties per tee; compressed springs fed down the outer tee wells and through the windows into the service tabs' channels; fore valves and four bowed flex stubs. After the box closes, squeeze to bottom the tubes with the cartridge 2.15 mm short of seating, then relax the grips and push the cartridge through that final stroke. The rest of internal plumbing follows [`enclosure-mechanical.md`](/hardware/assembly/enclosure-mechanical.md), and [`wiring.md`](/hardware/assembly/wiring.md) takes a chassis that already carries its plumbing. The card deck in [`/hardware/assembly/cards/`](/hardware/assembly/cards/) prints in this order.

## Layout

| Path | What's there |
|---|---|
| [`design-pressures.md`](/hardware/design-pressures.md) | What the machine is optimised for and what it is not. Placement decisions answer to this: volume and assemblability are optimised, field service and disassembly are not. |
| [`ledger/`](/hardware/ledger/) | Bookkeeping. [`purchases.md`](/hardware/ledger/purchases.md) is the source-of-truth capex ledger; [`bom.md`](/hardware/ledger/bom.md) (per-unit parts), [`tools.md`](/hardware/ledger/tools.md) (active tooling), and [`inventory.md`](/hardware/ledger/inventory.md) (spares / abandoned / donor / diagnostic stock) are views over it. Three more price time rather than parts: [`labor.md`](/hardware/ledger/labor.md) (attended minutes per unit), [`machine-time.md`](/hardware/ledger/machine-time.md) (hours a machine is occupied per unit), and [`build-time.md`](/hardware/ledger/build-time.md) (seconds a generator run takes, which is what a change to this repo costs rather than what a soda machine costs). |
...
| [`service/`](/hardware/service/) | Procedures run on a finished machine rather than a bench: [`pump-replacement.md`](/hardware/service/pump-replacement.md) and the dry cycle it runs first. |
...
| [`manifold-layout/`](/hardware/manifold-layout/), [`cold-core-layout/`](/hardware/cold-core-layout/), [`faucet-layout/`](/hardware/faucet-layout/) | The assemblies the parts above stand in, each written as one multi-solid STEP: the packed machine, the core one frame further in, and the above-counter column. `/3d` browses the tree these head — the machine and the faucet as its two cards, and the machine stands the core's own bodies, so opening it is already being inside the core. |
...
| [`quickstart/`](/hardware/quickstart/) | The wordless one-sheet quick start that lies on top of the packing. |
| [`install-guide/`](/hardware/install-guide/) | The bound install guide that ships in the install kit beside it. |
