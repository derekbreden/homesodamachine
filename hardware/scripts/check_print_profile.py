...
A print log says what a plate was printed at, and names the file it read that off:
    ...
    ## The PET-GF15 exterior (settings per [`enclosure-front-top-petgf.3mf`](...))
    - `nozzle_temperature` **290 °C** (initial 290)

That heading is a claim about a `.3mf` sitting beside it, and until now nothing held the two
together. These numbers are not decoration — a nozzle temperature is what the next person or
agent slices at, and PET-GF at the wrong one is a plate of delamination.
...
file and states nothing is no claim and is not checked; a section stating a figure for a file
that is not there is a claim about nothing, and fails.
...
for md in sorted(_HW.rglob("*print-log.md")):
    ...
    for i, (_start, name) in enumerate(heads):
        section, slice_path = text[edges[i]:edges[i + 1]], md.parent / name
        ...
        if not slice_path.is_file():
            ...
            continue
        try:
            s = settings(slice_path)
        except (OSError, KeyError, zipfile.BadZipFile, json.JSONDecodeError) as exc:
            failures.append(f"{rel} does not read as a slice: {exc}")
            continue
        for m, keys in stated:
            for claimed, key in zip(m.groups(), keys):
                ...
                if not same:
                    drifted.append(
                        f"{md.relative_to(_HW.parent)} states {key} {claimed} for {rel}, "
