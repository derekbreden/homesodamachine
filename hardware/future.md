...
- **Plan B (still under exploration):** racetrack body formed from two 7.754" × 6.000" half-blanks in 0.048" 304 SS, press-formed into D-halves and butt-welded along both flat sides; press-domed 0.060" 304 SS racetrack end caps. Retained as fallback if the 1/4"-plate-to-0.065"-tube weld can't be made reliably. Plan B inventory remains 304 SS (existing SendCutSend stock) — accepted inconsistency for a fallback that may never be invoked. Blank DXFs, press dies, racetrack end caps, and dishing dies are preserved at the `archive-plan-b` git tag (recoverable with `git checkout archive-plan-b -- hardware/printed-parts/plan-b/ hardware/cut-parts/plan-b/`); they were removed from the active tree to keep the working set focused on Plan A.
...
Flavor-reservoir Plan B is a pair of custom printed hard reservoirs, one per flavor, conforming directly to the cold-core envelope instead of forcing off-the-shelf bottle geometry into the under-sink package. The reservoirs are vented, not service-pressure vessels: 1L usable volume target, low outlet sump, high filtered vent, fill/dispense/clean paths through the same valve manifold. For Plan B the reservoirs remain separate printed parts that fit inside the foam-bag shell; an integrated reservoir/shell print is a later Plan C idea. Today's foam-bag-shell pockets are just under 1L internal volume, so the expected package change is a small X-axis pocket enlargement without disturbing the Y/Z pressure-vessel and plumbing envelope. The test plan and candidate food-contact PET-family filaments live in `printed-parts/cold-core/reservoir/README.md`. Plan B only replaces the bladder/cradle physical package; the user experience and manifold behavior stay the same.
