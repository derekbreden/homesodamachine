...
A non-combustible sheet-metal shroud encloses the compressor's terminal block area to make the R-600a refrigerant system genuinely safe in the customer's kitchen. UL 60335-2-89 (hydrocarbon appliance safety) is the engineering standard that codifies what that requires for this refrigerant class; the shroud follows the standard's substance for that reason (see [`regulatory.md`](regulatory.md)). 0.059" G90 hot-dipped galvanized steel, laser-cut + bent at SendCutSend. Open-bottom 5-sided shroud (or two-piece U-channel + back wall, TBD on the box-bend max-flange-height constraint), dropping over the compressor from above.
