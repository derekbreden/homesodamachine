...
export default () => (
  <board layers={4} outline={[{ x: -66.9, y: -51 }, { x: 60.9, y: -51 }, { x: 60.9, y: 47.7 }, { x: -66.9, y: 47.7 }]} minTraceWidth="0.2mm" minViaHoleDiameter="0.3mm" minViaPadDiameter="0.5mm" pcbStyle={{ silkscreenFontSize: "0.8mm" }} autorouter={{ traceClearance: 0.45 }}>
    ...
    {/* I2C bus — routed top bus, not poured (two co-located nets). pretty="maze:i2c"
        routes each segment at build time (pretty-routes.ts). */}
    <trace from=".U1B > .IO21" to=".U6I > .SDA" pretty="maze:i2c" />
    <trace from=".U1B > .IO22" to=".U6I > .SCL" pretty="maze:i2c" />
    <trace from=".U2I > .SDA" to=".U3I > .SDA" pretty="maze:i2c" />
    <trace from=".U2I > .SCL" to=".U3I > .SCL" pretty="maze:i2c" />
    <trace from=".U1B > .IO21" to=".U3I > .SDA" pretty="maze:i2c" />
    <trace from=".U1B > .IO22" to=".U3I > .SCL" pretty="maze:i2c" />
    ...
    <trace from=".U7T > .TXD" to=".U1A > .IO34" pretty="maze:faucet485" />
    <trace from=".U7T > .RXD" to=".U1A > .IO32" pretty="maze:faucet485" />
    ...
    {/* FAUCET UART (IO33 TX / IO35 RX). pretty="maze:faucet485" (with the RS485 pair). */}
    ...
    {/* REEDS A (reservoir A) -> 0x20 GPB inputs. pretty="maze:j6" threads J6 past U2's
        I2C header at build time (pretty-routes.ts). */}
    <trace from=".J6 > .RA1" to=".U2B > .GPB0" pretty="maze:j6" />
    <trace from=".J6 > .RA2" to=".U2B > .GPB1" pretty="maze:j6" />
    <trace from=".J6 > .RA3" to=".U2B > .GPB2" pretty="maze:j6" />
    <trace from=".J6 > .RA4" to=".U2B > .GPB3" pretty="maze:j6" />
    <trace from=".J6 > .GND" to="net.GND" />

    {/* REEDS B (reservoir B + carbonator low/high) -> 0x21 GPB inputs. RB2..CHI fan as
        pretty="clean:fanRowToColumn"; RB1->GPB0 is the autorouter's own matching path. */}
    <trace from=".J7 > .RB1" to=".U3B > .GPB0" />
    <trace from=".J7 > .RB2" to=".U3B > .GPB1" pretty="clean:fanRowToColumn" />
    <trace from=".J7 > .RB3" to=".U3B > .GPB2" pretty="clean:fanRowToColumn" />
    <trace from=".J7 > .RB4" to=".U3B > .GPB3" pretty="clean:fanRowToColumn" />
    <trace from=".J7 > .CLO" to=".U3B > .GPB4" pretty="clean:fanRowToColumn" />
    <trace from=".J7 > .CHI" to=".U3B > .GPB5" pretty="clean:fanRowToColumn" />
    ...
    <trace from=".U8 > .IO" to=".U1B > .IO4" pretty="maze:io4u8" />
    ...
    <trace from=".J11 > .AOUT" to=".R1 > .pin1" pretty="maze:divider" />
    <trace from=".R1 > .pin2" to=".R2 > .pin1" pretty="maze:divider" />
    <trace from=".R1 > .pin2" to=".U1A > .IO39" pretty="maze:divider" />
    <trace from=".R2 > .pin2" to="net.GND" />
    <trace from=".J11 > .DOUT" to=".R3 > .pin1" pretty="maze:divider" />
    <trace from=".R3 > .pin2" to=".R4 > .pin1" pretty="maze:divider" />
    <trace from=".R3 > .pin2" to=".U1A > .IO36" pretty="maze:divider" />
