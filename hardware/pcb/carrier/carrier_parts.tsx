...
// row, the function label, the pin labels and the ref-des. Laid out once for a
// horizontal row — function label along one long side, pin labels + ref-des along
// the other — then the whole connector is rotated by `rot`, so a vertical one is
// the horizontal turned a quarter-turn and every label reads the same way (the
// pin labels are drawn here, not by the footprint, which can't orient them to the
// bottom-to-top convention; the footprint string is pads-only). Margins to the
// fence are even on all four sides.
export const Jst = ({ name, x, y, count, labels, rot = 0, label, labelDir }: { name: string; x: number; y: number; count: number; labels: string[]; rot?: number; label: string; labelDir?: number }) => {
  ...
  // Map perpendicular u (+ = function-label side) and along-row v to a board offset,
  // so the connector reads one way (bottom-to-top vertical, left-to-right horizontal).
  ...
  return (
    <>
      <pinheader name={name} pinCount={count} pitch="2.54mm" gender="male" footprint={`pinrow${count}_nopinlabels_norefdes`} pcbRotation={rot} pinLabels={labels} {...at(x, y)} />
