...
Board `esp32dev` (ESP32-WROOM-32E), firmware `firmware/src_appliance/main.cpp`. A wired GPIO +
UART hub:

- No WiFi, no BLE — the main board links no radio. BLE is on the faucet display
  (`firmware/src_faucet`), which hosts the GATT server the iOS app talks to
  (`firmware/README.md`, "A shipped unit is updated from the iOS app"). The main board
  reaches both displays over wired UART (RS485 to the enclosure display, TTL UART to the
  faucet display).
- Flashed over serial (`pio run -e appliance -t upload`): UART0 + EN/IO0.
...
- UART → RS485 enclosure display: IO32 (TX), IO34 (RX, input-only) — south edge
...
- Sensors: IO26 (1-wire, 4.7k external pull-up to 3V3 — R9; two probes on the one bus, told apart by family code — DS18B20 `0x28` = carbonator wall / compressor setpoint, DS18S20 `0x10` = evaporator-coil / freeze-protect cutout), IO25 (flow), IO27 (backflow signal) — three adjacent south-edge pins — plus IO23 (north edge): the moisture module's switched VCC, driven only while sampling so the ASSE drip pan's electrodes sit unpowered between samples
