...
- **`pcbPath={[{ x, y }, { x, y, via: true, toLayer: "inner2" }, …]}`** — a full manual path:
  ...
  tool that can force a via and a landing layer. Numeric points anchor to `pcbPathRelativeTo`
  (default: the `from` component's frame); this board's hand traces declare
  `pcbPathRelativeTo="board"` so every point is a board coordinate — the `frame` helper and
  how to move a component tighter are in [`hand-routing.md`](hand-routing.md).
