 ...
 *
 * Every point is a BOARD coordinate. Each trace carrying numeric points declares
 * pcbPathRelativeTo="board", so a {x, y} literal lands exactly where the viewer reads it.
 * Manual vias are full-stack top<->bottom ONLY, so hand paths live on the top or bottom layer.
 *
 * frame(el) captures a placed component and turns its pin geometry into board points:
 *   .ref(pin)             the pad string anchor, e.g. "U14.pin1" — a pcbPath's endpoints
 *   .pin(pin)             the pad's board position {x, y}
 *   .fromPin(pin, dx, dy) a point (dx, dy) mm from this frame's pad — rides the pad when
 *                         the part moves
 */
...
export type Frame = {
  pin: (p: string) => Pt
  ref: (p: string) => string
  fromPin: (p: string, dx?: number, dy?: number) => Pt
}
...
// coord points to hand-route against. Call it with the placed ELEMENT — `frame(<Usblc6 name x y
...
// pad names aren't footprint pins (a Jst's labels are board-assigned, not <smtpad> ids).
...
export function frame(a: any, cx = 0, cy = 0, rot = 0, pins: Record<string, [number, number]> = {}): Frame {
  ...
  return {
    pin,
    ref: (p: string) => `${name}.${p}`,
    fromPin: (p: string, dx = 0, dy = 0): Pt => { const b = pin(p); return { x: b.x + dx, y: b.y + dy } },
  }
}

// A no-via "U" that ties two pads of the same connector `f`: out from `a` by the stub, across
...
// pcbPath per dest pin. No vias.
export const pcbFan = (srcF: Frame, srcPin: string, exit: [number, number], destF: Frame, destPins: string[], laneX: number) =>
  destPins.map((d) => [srcF.ref(srcPin), srcF.fromPin(srcPin, ...exit), { x: laneX, y: destF.pin(d).y }, destF.ref(d)])
...
export const orthoDrop = (fromF: Frame, pin: string, toF: Frame, toPin: string, dropX?: number): (Pt | string)[] => {
  ...
  return [fromF.ref(pin), { x: dropX ?? p.x, y: q.y }, toF.ref(toPin)]
}
