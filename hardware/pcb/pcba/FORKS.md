...
| `@tscircuit/props` | [props](https://github.com/derekbreden/props) | `v0.0.565` | override | `viaMode` + `viaInPad` on the autorouter config schema; `pcbComb` trace prop | yes ([props#715](https://github.com/tscircuit/props/pull/715)) |
| `@tscircuit/core` | [core](https://github.com/derekbreden/core) | `v0.0.1380` | override | `viaMode`/`viaInPad`/`traceClearance` lowering, honest `EVERY_LAYER`, 6/8-layer names, pour stitch/keepout/carve, hole clearance ring, silk ref-des rotation; `pcbComb` straight→45°→straight comb in the manual-trace phase | mixed — the `viaMode` lowering + `pcbComb` yes ([core#2567](https://github.com/tscircuit/core/pull/2567)); the pour/stitch logic is project-specific |
