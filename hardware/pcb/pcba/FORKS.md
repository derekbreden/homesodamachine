...
|---|---|---|---|---|---|
| `@tscircuit/capacity-autorouter` | [tscircuit-autorouter](https://github.com/derekbreden/tscircuit-autorouter) | `v0.0.620` | override | via emission spans top↔bottom in through-hole mode; `viaInPad` pulls terminal transition vias onto their pads | yes (with props/core/rectdiff) |
| `@tscircuit/rectdiff` | [rectdiff](https://github.com/derekbreden/rectdiff) | `v0.0.47` | CAR devDep SHA | mesh: a node is via-capable iff its `availableZ` spans the full stack; `viaRingKeepout: false` skips the extra via-ring carve | yes |
| `@tscircuit/props` | [props](https://github.com/derekbreden/props) | `v0.0.565` | override | `viaMode` + `viaInPad` + `viaRingKeepout` on the autorouter config schema; `pcbComb` trace prop | yes ([props#715](https://github.com/tscircuit/props/pull/715)) |
| `@tscircuit/core` | [core](https://github.com/derekbreden/core) | `v0.0.1380` | override | `viaMode`/`viaInPad`/`viaRingKeepout`/`traceClearance` lowering, honest `EVERY_LAYER`, 6/8-layer names, pour stitch/keepout/carve, hole clearance ring, silk ref-des rotation; `pcbComb` straight→45°→straight comb in the manual-trace phase; manual-trace vias record the barrel as a full-stack through-hole (`layers` spans every copper layer even when the copper transition ends on an inner layer — pours antipad by that list, so an understated span floods a plane over the physical drill) | mixed — the `viaMode` lowering + `pcbComb` yes ([core#2567](https://github.com/tscircuit/core/pull/2567)); the pour/stitch logic is project-specific |
...
| `circuit-json-to-gerber` | [circuit-json-to-gerber](https://github.com/derekbreden/circuit-json-to-gerber) | `v0.0.78` | override | (1) inlined Hershey single-stroke font for gerber text; (2) **F_Paste derived from `pcb_smtpad` copper** — every SMD lead (incl. pill/rotated_pill gull-wings) gets a paste aperture and exposed pads are windowpaned, replacing the old `pcb_solder_paste` path that emitted paste only for `rect` pads (which left all 9 gull-wing ICs — both MCP23017, both ULN2803, both DRV8870, RTC, CH340, RS485 — unpasted). A `paste-coverage` scorecard gate guards against regression. **Drill hazard for any consumer:** the drill emitter classifies each hole by `from_layer`/`to_layer` in preference to `layers` and writes only holes whose span matches the requested file — a via recording an inner copper transition lands in **no** drill file while its pads stay on every layer. Any board rendering through this package must restate via spans at the drill call (`render-board.ts` `throughDrilled`) and assert coverage (`assertFullyDrilled`); rendering without those reproduces the undrilled-via defect silently. | partial — paste fix PR-able; font maybe |
...
**Fork status vs. the hand-routed board.** The branch name (`through-hole-vias`) records the original motive: teaching the capacity-autorouter to emit through-hole / via-in-pad copper. The board is now **100% hand-routed** and renders with **no autoroute pass** (`render-board.ts` logs `no 2nd autoroute`; scorecard `0 auto`), so `@tscircuit/capacity-autorouter` + its `rectdiff` pin are **dormant** — still configured on the board tag (governing the mesh only if a connection is ever handed back), but owning no copper. Pruning that pair to upstream is a possible cleanup that would shrink the fork surface. The rest stay **load-bearing even hand-routed**: `core` + `copper-pour-solver` antipad the inner 3V3/5V planes around every through-hole barrel and via-in-pad (without them a plane floods over the pin and shorts to it — and the pour-blind DRC cannot see it); `circuit-json-to-gerber` generates the gerbers on every render **and carries the paste fix**, so reverting it to upstream silently reintroduces the 9-unpasted-IC bug; `props` + `footprinter` are schema/label plumbing. None of the seven are merged upstream yet — the weekly sync still rebases our changes on top of upstream, so all are still local.
...
The gerber changes — the Hershey silkscreen font **and the F_Paste-from-copper fix** (see the table) — have a single home in the `circuit-json-to-gerber` fork.
