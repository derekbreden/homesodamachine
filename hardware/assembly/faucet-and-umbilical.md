...
| SendCutSend 0.060" 304 SS under-counter keyhole plate | `touch_flo_under_counter_plate.dxf` | Single-piece Ø 54.35 mm disc with two open-edge channels — one for the shank (Ø 12.6 pocket + 12.6 mm wide channel to the rim) and one for the flavor-tube pill (13.2 × 6.85 mm pocket, long-axis-along-channel + 6.85 mm wide channel to the rim). Channels exit the rim at different points (different Y heights in plate coords), so a single lateral slide motion seats both cylinders. Cylinders-in-channels hold the plate in alignment passively, letting the customer thread the nut one-handed. Order qty 1 per appliance. |
...
- Loose install-kit parts bagged together: **one SS under-counter keyhole plate**, factory shank nut + washer, Mudder tube cutter, installer instruction sheet
