...
The production procedure for the above-counter fixture stack and the 3-tube umbilical that connects it to the rear panel — the visible half of the appliance from the user's perspective. Output of this bench is a single bagged sub-assembly that ships loose inside the appliance carton: the customer (or their installer) drills the 1-3/8" countertop hole, drops the faucet body through it from above, lands the under-counter plate + nut from below, and pushes the three tube tails into the rear-panel PP1208E bulkheads. No factory pre-installation, no field cuts to the umbilical bundle.

This bench runs in parallel with the main appliance chain. Its inputs are upstream of [`pressure-vessel.md`](pressure-vessel.md) (no dependency on the vessel build) and its output ships in the carton alongside the finished enclosure produced by [`finish-pack-ship.md`](finish-pack-ship.md) (TBD). Design intent for the user-facing surface lives in [`../future.md`](../future.md) "User-facing elements, by location" and the dispense-head geometry rationale lives in [`../bom.md`](../bom.md) §9 (Dispensing).

## Scope

In: one shell + plate + Touch-Flo body sub-assembly (output of [`../printed-parts/faucet/touch-flo-shell/ASSEMBLY.md`](../printed-parts/faucet/touch-flo-shell/ASSEMBLY.md)); the 3-tube dispense head (1/4" SS center tube + 2× 1/8" SS flanking tubes + decorative ferrules + PE liner); the TPU mounting gasket + SendCutSend 0.060" SS under-counter plate (loose, for the installer's countertop step); 3× 1/4" OD LLDPE tubes cut to length (1× blue carbonated-water + 2× black flavor); CARGEN nitrile foam pipe-insulation segments; cable sleeve; the RP2040 round display; the KRAUS air switch; one Cat6 cable carrying both signals through a single countertop pass.

Out: a complete above-counter fixture stack ready to drop into a countertop hole — faucet body with dispense head installed, RP2040 round display nested into the printed shell, KRAUS air switch tail emerging from the body, three sleeved tubes terminated bare and push-to-connect-ready at the rear-panel end, foam insulation only on the cold (carbonated-water) tube, Cat6 + tube bundle co-sleeved through the countertop. Bagged with the installer-facing instructions sheet (TBD), drop-shipped inside the appliance carton.

Not in scope: countertop drilling itself, and the under-counter mating to the rear-panel PP1208E bulkheads — that's the customer/installer step at install time, covered in the install guide (placeholder: `install-above-counter.md`, see Open items). Cat6 termination at the electronics shelf — that's `wiring.md` (TBD).
...
| TPU mounting gasket (printed) | [`../printed-parts/faucet/touch-flo-mounting-gasket/`](../printed-parts/faucet/touch-flo-mounting-gasket/) | Sits between the printed mounting plate and the countertop top surface at install; **ships loose in the bag, not pre-installed**, because the installer slides it over the shank from below at countertop-install time |
| SendCutSend 0.060" 316 SS under-counter plate | `touch_flo_under_counter_plate.dxf` | Distributes the under-counter nut's clamping load; ships loose with the factory shank nut, installed by the installer below the countertop |
...
Into the bag with the umbilical, add the loose installer parts:

- TPU mounting gasket (loose, not slid onto the shank)
- SendCutSend 0.060" SS under-counter plate (loose)
...
- One installer-facing instruction sheet covering the above-counter install — countertop drill template, gasket-and-plate-on-shank order, shank-nut tighten sequence, rear-panel push-to-connect rule "blue tube into the blue-ringed bulkhead, black-into-either-black", Cat6 termination at the rear-panel access (TBD which side terminates Cat6). Doc placeholder: `install-above-counter.md` (see Open items).
...
- One 3-tube umbilical bundle (blue carbonated-water + 2× black flavor) inside a single sleeve, foam insulation on the cold tube only, Cat6 alongside the three tubes inside the same sleeve
- Three tubes terminated bare and square-cut at the rear-panel end, ready for push-into-PP1208E at install
- Cat6 unterminated at both ends — the above-counter end gets cut to length at the customer's countertop position during install, the rear-panel end gets terminated at the electronics shelf per `wiring.md` (TBD) at appliance final assembly
- Loose TPU gasket + SS under-counter plate + factory shank nut + Mudder tube cutter + installer instruction sheet bagged together
...
5. **Installer instruction sheet — `install-above-counter.md`.** Customer-facing install guide covering countertop drilling, gasket + under-counter plate seating order, shank-nut sequence, the blue-tube-into-blue-bulkhead rule, and the umbilical trim step at the rear-panel end. Scope + doc name placeholder pending dedicated installer-docs branch.
