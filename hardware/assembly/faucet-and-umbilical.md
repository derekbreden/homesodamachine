...
In: one shell + plate + Touch-Flo body sub-assembly (output of [`../printed-parts/faucet/touch-flo-shell/ASSEMBLY.md`](../printed-parts/faucet/touch-flo-shell/ASSEMBLY.md)); the 3-tube dispense head (1/4" SS center tube + 2× 1/8" SS flanking tubes + decorative ferrules + PE liner); the TPU mounting gasket (ships loose with the assembly, slips up the shank from below at install — TPU 90A flexes around the dangling umbilical); two halves of the split SendCutSend 0.060" SS under-counter plate (ships loose with the assembly, halves clip around the umbilical at install — pending DXF update, see Open items); 3× 1/4" OD LLDPE tubes cut to length (1× blue carbonated-water + 2× black flavor); CARGEN nitrile foam pipe-insulation segments; cable sleeve; the RP2040 round display; the KRAUS air switch; one Cat6 cable carrying both signals through a single countertop pass.
...
| TPU mounting gasket (printed) | [`../printed-parts/faucet/touch-flo-mounting-gasket/`](../printed-parts/faucet/touch-flo-mounting-gasket/) | Sits between the printed mounting plate and the countertop top surface at install. **Ships loose in the bag.** The installer stretches it up the shank from below — TPU 90A is flexible enough to flex past the already-attached umbilical. (Open item: confirm TPU flex margin at first install, or commit to a split gasket like the under-counter plate.) |
| SendCutSend 0.060" 304 SS under-counter plate — **two-half split design** | `touch_flo_under_counter_plate.dxf` (currently solid disc — DXF update pending, see Open items) | Distributes the under-counter nut's clamping load. **Ships as two semicircular halves** that the installer clips around the umbilical from below at install time, removing the "thread tubes through a pill slot" step that a solid disc would require with an already-attached umbilical |
...
Into the bag with the umbilical, add the install-kit parts the customer needs at countertop install:

- TPU mounting gasket (loose, not slid onto the shank — the installer flexes it up the shank from below past the dangling umbilical)
- **Two halves of the split SendCutSend 0.060" 304 SS under-counter plate** — the two semicircular halves that clip around the umbilical from below at install. (DXF update to actually split the disc is pending — see Open items.)
- Factory shank nut (loose — already separated from the Touch-Flo body at this assembly step's upstream)
- One Mudder PEX/PE tube cutter (the per-appliance install-kit tool from [`../bom.md`](../bom.md) §14, same SKU as the bench-side cutter)
- One installer-facing instruction sheet covering the above-counter install — countertop drill template, drop-through-from-above sequence, **clip-the-two-plate-halves-around-the-umbilical-from-below** step, gasket-stretches-up-shank step, shank-nut tighten sequence, rear-panel push-to-connect rule "blue tube into the blue-ringed bulkhead, black-into-either-black", Cat6 termination at the rear-panel access (TBD which side terminates Cat6). Doc placeholder: `install-above-counter.md` (see Open items).
...
- Loose install-kit parts bagged together: TPU gasket, **two halves of the split SS under-counter plate** (pending DXF update), factory shank nut, Mudder tube cutter, installer instruction sheet
...
1. **Under-counter plate split-DXF update.** [`../cut-parts/faucet/touch-flo-under-counter-plate/generate_dxf.py`](../cut-parts/faucet/touch-flo-under-counter-plate/generate_dxf.py) currently produces a solid Ø 54.35 mm disc with a single shank hole and a single pill slot — the part it generates **cannot be installed around an already-attached umbilical** without threading the tubes through the pill slot from below, which is exactly the install step the split design exists to eliminate. The DXF generator needs to be reworked to produce two semicircular halves whose mating edges meet at the plate's vertical centerline (or some equivalent split). Open questions: how do the two halves stay registered to each other under the under-counter nut's clamping load (just clamped flat by the nut's washer, or via interlocking edges / a tab-and-slot feature), and does the split add a serviceability win (replace one damaged half rather than the whole plate). Until the DXF is updated, the procedure here describes the intended part, but the actually-fabricated part is a solid disc that the installer would have to thread tubes through.
2. **TPU mounting gasket — solid or split.** Same install-time geometry concern as the under-counter plate: with an already-attached umbilical, can the TPU 90A gasket flex enough to slide up the shank past the dangling tubes, or does it need a split design like the under-counter plate? TPU 90A is fairly flexible; first install will confirm. If flex margin is inadequate, the gasket's `generate_step_cadquery.py` would need the same split treatment as the under-counter plate.
3. **Cat6 conductor pinout for SIG-5 + SIG-6.** [`../wiring/ac-wiring-schedule.md`](../wiring/ac-wiring-schedule.md) defines the runs at the schedule level (SIG-5 = air-switch contacts, SIG-6 = RP2040 UART + 5 V) but does not pin specific Cat6 pair-to-conductor assignments. Needed for the RP2040-end pin-header build and for the rear-panel termination in `wiring.md`.
4. **Umbilical design length.** Cabinet-routing length depends on countertop thickness, faucet-to-back-of-cabinet horizontal offset, and rear-panel position inside the enclosure. Three numbers are unresolved and the umbilical length sums them all. Once spec'd, this also fixes the CARGEN foam segment count for step 4.
5. **Cable sleeve selection.** Braided polyester vs spiral wrap, per [`../printed-parts/enclosure/back-panel/README.md`](../printed-parts/enclosure/back-panel/README.md). The cleaner one for the customer-visible sleeve segment between countertop and rear-panel cluster is whichever installs with the bundle and Cat6 already inside; spiral wrap allows post-bundle install (wrapping around an assembled bundle), braided requires pre-thread.
6. **Blue LLDPE small-spool sourcing.** A small spool of 1/4" OD blue LLDPE is in flight per the back-panel README. SKU + supplier not yet in `bom.md` or `purchases.md`.
7. **Installer instruction sheet — `install-above-counter.md`.** Customer-facing install guide covering countertop drilling, drop-through-from-above sequence, clip-the-split-plate-halves-around-the-umbilical step, gasket-stretches-up-shank step, shank-nut tighten sequence, blue-tube-into-blue-bulkhead rule at the rear panel, and the umbilical trim step at the rear-panel end. Scope + doc name placeholder pending dedicated installer-docs branch.
8. **RP2040 display termination at the above-counter end.** Whether the Cat6's 4 display conductors land at the display via soldered leads, Dupont female header, or a JST-XH 4-pin connector. Affects both the rework-friendliness of the display and the BOM (JST headers already in [`../bom.md`](../bom.md) §11; soldered leads are zero-BOM but make field-swap of a failed RP2040 harder).
9. **Cat6 strain relief through the countertop.** The 1-3/8" countertop hole that takes the faucet shank is sized for the shank + gasket + plate stack; the Cat6 piggybacks through the same hole alongside the tubes. Whether the Cat6 needs a separate grommet, a printed strain-relief collar, or just rides loose through the gasket's pill-slot region is undefined.
