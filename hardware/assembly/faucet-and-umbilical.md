...
1. **Umbilical signal cable family.** [`/hardware/wiring/ac-wiring-schedule.md`](/hardware/wiring/ac-wiring-schedule.md) defines SIG-4 (DIGITEN flow meter, 3 conductors) and SIG-6 (1.47" faucet display, direct TTL UART: TX / RX / 5 V / GND) at the schedule level but does not pin a cable family. The umbilical carries both — ~7 conductors — so Cat6 (4 twisted pairs) is appropriately sized: the faucet-display UART rides one twisted pair (conservative baud for margin over the cabinet-noise run), the flow meter another, the remaining pairs for 5 V / GND.
