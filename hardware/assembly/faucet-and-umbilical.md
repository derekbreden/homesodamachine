...
Not in scope: countertop drilling itself; the customer-side install steps — drop-through from above, slide the keyhole under-counter plate laterally onto the dangling umbilical from below, washer + nut tighten, trim the field ends, push-into-PP1208E at the rear wall — covered in the visual quick start that ships with the appliance ([`/hardware/quickstart/`](/hardware/quickstart/README.md); packing and content contract: [`/marketing/unboxing-and-quickstart.md`](/marketing/unboxing-and-quickstart.md)). Signal-cable termination at the electronics shelf — that's `wiring.md`.
...
2. ~~**Cable sleeve size.**~~ **CLOSED — Alex Tech 1" PET expandable braid, black/blue, B075VRDS53.** Five 1-ft segments per build, one to each foam segment, per §3. 1" nominal expanding 50% covers the Ø[31.66 mm](SLEEVE_BORE) — [1.25"](SLEEVE_BORE_IN) — the pack opens it to. 100 ft roll, [`purchases.md`](/hardware/ledger/purchases.md) 2026-08-17.
