...
In: one shell + plate + Touch-Flo body sub-assembly (output of [`../printed-parts/faucet/touch-flo-shell/ASSEMBLY.md`](../printed-parts/faucet/touch-flo-shell/ASSEMBLY.md)); the TPU mounting gasket — slid onto the shank during this bench, sits permanently between the printed mounting plate's underside and where the countertop top surface will be at install; the 3-tube dispense head (1/4" SS center tube + 2× 1/8" SS flanking tubes + decorative ferrules + PE liner); two SendCutSend 0.060" SS under-counter half-plates (two units of the same D-shaped part, ships loose in the install bag — halves clip around the umbilical at install); 3× 1/4" OD LLDPE tubes cut to length (1× blue carbonated-water + 2× black flavor); CARGEN nitrile foam pipe-insulation segments; cable sleeve; the RP2040 round display; the KRAUS air switch; one Cat6 cable carrying both signals through a single countertop pass.
...
| SendCutSend 0.060" 304 SS under-counter half-plate | `touch_flo_under_counter_plate.dxf` | One D-shaped half is in the DXF; order quantity 2 per appliance. The two identical pieces clip around the umbilical from below at install (one flipped 180° relative to the other), forming a complete plate around the shank + tube bundle. Removes the "thread tubes through a pill slot" step that a one-piece disc would require with an already-attached umbilical. |
