...
4. **Umbilical cable strain relief through the countertop.** The Ø [34.93 mm](COUNTERTOP_HOLE_D) countertop hole carries the shank, the two flavor tubes and the umbilical signal cable through one opening, with [3.428 mm](HOLE_MARGIN) of slab left to the nearest of them at the centre item 5 gives. Whether the cable needs a separate grommet, a printed strain-relief collar, or just rides loose through the gasket's pill-slot region is undefined — and that margin is what any added collar has to fit inside.
5. **Locating the offset countertop hole.** The hole does not centre on the faucet body axis. The flavor pair stands behind the axis and reaches further from it than the shank does, so a hole centred on the axis clears the shank by [11.96 mm](HOLE_ON_AXIS_SHANK) and misses the flavor pair by [-4.899 mm](HOLE_ON_AXIS_FLAVOR) — the tubes land inside the stone. The centre that balances the two is [8.537 mm](HOLE_CENTER_Y) behind the axis, clearing shank and flavor pair alike by [3.428 mm](HOLE_MARGIN); `balanced_hole_center_y()` in [`/hardware/faucet-layout/faucet_assembly.py`](/hardware/faucet-layout/faucet_assembly.py) solves it and the assembly draws the slab cut there. Undefined is how the installer hits that offset: it is not eyeballed against a Ø [34.93 mm](COUNTERTOP_HOLE_D) hole in stone, drilling is out of scope above, and the install kit ships no locating aid — a template is the obvious answer and is not yet a part.
