...
| Printed tube collar × 3 | [`/hardware/printed-parts/enclosure/tube-collar/`](/hardware/printed-parts/enclosure/tube-collar/README.md) | `tube-collar-carb` on the blue tail, `tube-collar-flavor-a` and `-flavor-b` on the two black. The rear wall's own chip bored for the tube and run [30 mm](COLLAR_LENGTH) along it — same word, same spool, threaded on end-first before the sleeve goes over. The other two stations' collars are in the install kit, for the runs the customer cuts |
...
| Turn-in at the wall — lead + 90° at R12 + collet | 60 | CAD; the collet stands 9.5 mm proud of the port chip it bears on, and that chip lies flush in the wall (`printed-parts/enclosure/port-ring/`) |
...
- **Two printed tube collars — `tube-collar-water` and `tube-collar-co2`** ([`/hardware/printed-parts/enclosure/tube-collar/`](/hardware/printed-parts/enclosure/tube-collar/README.md)). The other two rear-wall stations take a tube the customer cuts in their own kitchen: the tap-water run to their angle stop and the tether to their cylinder's regulator. Each collar threads onto its run and carries the station's word — `TAP`, `CO2` — to the end that has no ring on it
...
- Loose install-kit parts bagged together: **one SS under-counter keyhole plate**, factory shank nut + washer, Mudder tube cutter, the two `TAP` and `CO2` tube collars
