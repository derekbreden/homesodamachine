...
4. ~~**Drip-pan landing.**~~ **CLOSED.** The basin is [100 × 30 × 22 mm](/hardware/printed-parts/enclosure/drip-pan/) — 30 mm deep because the aft strip behind the SeaFlo is only 55 mm — and it is packed centred on the vent column, so the drip falls 28.6 mm onto the middle of its floor where the Shutao plate lies. It seats on the foam-cap top through a floorless printed cradle VHB'd to the cap lid, and lifts 8 mm off that rail and draws west to be emptied. §7 row added. Still open: the SIG-9 cable length.
