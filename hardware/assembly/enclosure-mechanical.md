...
The arrangement is [`front_half.py`](/hardware/manifold-layout/front_half.py): it places every body and sizes the box around them, and `front-half.scorecard.json` beside it carries every port, every run and every clearance the placement holds. Design intent lives in [`/hardware/future.md`](/hardware/future.md) "Enclosure layout", "Compressor compartment shroud", "User-facing elements by location", "Rear-panel AC inlet", and "Rear-panel nameplate". Part-level READMEs are the source of truth for every component this procedure handles; this document is the build-cadence wrapper.
...
Per-unit BOM lives in [`/hardware/ledger/bom.md`](/hardware/ledger/bom.md) §7 (printed enclosure parts), §8 (the rear wall's JG bulkheads), §3 (the §3 PP1208E tap-water inlet), §4 (the DERPIPE CO2 PTC), §5 (the C14 inlet + the compressor shroud + its SS cable gland), and §13 (mechanical attach hardware). **The chassis fastener schedule at Open items 1 has no §13 rows at all** — every screw §13 buys is spent on the foam caps, the reservoir caps, the touch-flo plate or the electronics shelf. Status (ACQUIRED / ON-ORDER) lives in [`/hardware/ledger/purchases.md`](/hardware/ledger/purchases.md) §6 + §11. The table below is the procedure-level summary.
...
**Install the compressor shroud over the terminal block + PTC relay/overload module BEFORE the stratum goes in the box.** Bench access to the terminal block is much easier with the compressor still on the build cart; once the stratum is on the floor the cold core lands right behind it and seating the shroud is much harder. Sequence:

1. Set the compressor upright on a clean bench, the cold-core assembly steady on its cart with no tension on the refrigerant lines.
2. Fit the SS cable gland into the shroud's [7/8"](PANEL_HOLE) sidewall hole and secure its locknut, on the side that will face the power column on the +X flank — the shortest path for the AC pigtail run.
...
Then set the shrouded compressor and the condenser+fan into the front-bottom piece **as one mated pair**, both standing on the floor slab and packed forward — the front wall stands its own seam clearance off their front faces, and nothing else fences them there. They mate face to face with nothing between them: the shroud fills the west, the condenser stands east of it, and the plane between them is a YZ plane. That mating carries the condenser's airflow axis **across the machine** — the air takes the cabinet the short way and the finstack faces the two ±X walls. Nothing in the box fastens either body today; both stand on the floor and are held by it (see Open items 1, and Open items 2 for the grille openings the air still has no route through).
...
- Refrigeration stratum standing on the floor slab, shroud west and condenser east, the plane between them closed, the shroud's sheet-metal box fastened over the terminal block + PTC module with its gland-fitted [7/8"](PANEL_HOLE) AC pass-through facing the power column
...
- Chassis bonding lead ring-terminated at the compressor shroud's Ø6 mm earth-bond hole on its back face, routed toward the ground stack, not yet terminated
...
1. **Nothing in the box fastens the refrigeration stratum.** The compressor's four grommeted feet, the shroud's two Ø4.5 mounting holes and the condenser's harvested fan-shroud ears all want a printed feature to land on, and `enclosure.py` cuts none: the shroud, the condenser and the cold core alike are held by the floor slab they stand on and by each other. What this needs is the pad, boss and ear stations on `enclosure-front-bottom`, struck off each donor body's own hole pattern the way the C14's two bosses are struck off its — plus the §13 BOM rows for the screws, which do not exist. Two of the three patterns are still estimates until a donor is measured: the compressor's foot pitch, and the condenser's ear pattern. **The grommet is the isolation element and is never crushed** — whatever the schedule ends up being, the screw bears on the steel bushing, not the rubber.

2. **The condenser's air has no route through the box.** The mating settles the axis and the side: the condenser stands east of the shroud with its `AIRFLOW` axis across the machine, so the air takes the cabinet the short way and the finstack faces the two ±X walls. But the box has no opening on either side face — `pack.east_ports` and `pack.front_ports` are both empty, and the −X wall carries only the drip tray's slot. Needs an intake and an exhaust cut at the finstack's own footprint, clear of the ±X boss chains and the Z-seam pods, and the fan's flow direction confirmed against the donor's native orientation per [`/hardware/reference/ice-maker/README.md`](/hardware/reference/ice-maker/README.md) so the two are not swapped.
