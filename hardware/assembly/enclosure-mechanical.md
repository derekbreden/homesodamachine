...
The chassis is built from the floor up: enclosure shell on the bench, compressor on the floor under its shroud, condenser/fan against the chosen side wall, cold core dropped in at the rear, drip pan onto the foam-cap top behind it, hopper at the top-front, back panel (bulkheads pre-installed) mounted last, electronics stack seated on the pump-2 column beside the rear-panel C14 inlet. The cold core drops in as a single pre-assembled unit — it is not built up in place. See [`refrigerant-loop.md`](/hardware/assembly/refrigerant-loop.md) "Output condition" for the state the cold core arrives in.
...
- **Water-inlet bulkhead** (John Guest PP1208E, same SKU and panel hole as the umbilical trio): customer-facing 1/4" JG push-to-connect facing out — the install-kit 1/4" LLDPE pushes in, no tools — retained by its flange + nut sandwich like the umbilical cluster below. The FFL38BARB38 it feeds sits inside the cabinet on the ASSE 1022 chain, not on this panel ([back-panel §2](/hardware/printed-parts/enclosure/back-panel/README.md)).
...
Seat the printed drip pan on the cold core's foam-cap top, in the rear strip behind where the electronics shelf row will land and under where the drip off the Multiplex 19-0897's atmospheric vent tip falls — the tip hangs over the strip pointing down, its fall onto the cap verified by the assembly scorecard's `fall` rule (the pan + vent location are inside the cabinet, not routed through any panel; see [`/hardware/future.md`](/hardware/future.md) "Backflow vent monitoring"). Place the moisture sensor in the pan with its leads routed toward the shelf's landing point on the same deck. Leave the leads loose for now — termination at the shelf happens in [`wiring.md`](/hardware/assembly/wiring.md) (run SIG-9 in [`/hardware/wiring/ac-wiring-schedule.md`](/hardware/wiring/ac-wiring-schedule.md)).
...
Mechanically seat the bench-built electronics shelf ([`electronics-shelf.md`](/hardware/assembly/electronics-shelf.md)) flat on the cold core's foam-cap top, in the front half of the band above it, ahead of the panel-port bodies: the **power tray** — C14 inlet pigtail, AC distribution block, Mean Well IRM-90-12ST PSU, both Teyleten relays, and the ground bus — at the left on the C14 inlet's column, the **PCBA tray** at the right, and the DC distribution block behind the power row, per [`/hardware/printed-parts/enclosure/README.md`](/hardware/printed-parts/enclosure/README.md) Zone B ([`/hardware/wiring/ac-wiring-schedule.md`](/hardware/wiring/ac-wiring-schedule.md) header).
...
Confirm the shelf row sits ahead of the C14 and bulkhead bodies reaching into the band behind it. No wiring runs are made at this step.
...
4. **Drip-pan landing.** The pan is designed ([`/hardware/printed-parts/enclosure/drip-pan/`](/hardware/printed-parts/enclosure/drip-pan/) — printed PETG) and the moisture sensor is selected and purchased (Shutao LM393 module, [`/hardware/ledger/bom.md`](/hardware/ledger/bom.md); SIG-9 pins assigned — sampling supply on IO23, DO → IO27). Open: the pan is not yet in the enclosure pack — the scorecard verifies the vent-tip fall onto the foam-cap top, but the pan as drawn runs into the packed SeaFlo when set under the tip, so pan size/position resolve with the water-deck layout in [`enclosure-assembly/_contents.py`](/hardware/printed-parts/enclosure/enclosure-assembly/_contents.py) — plus its mounting (the held axis, no bosses or tabs), its missing [`/hardware/ledger/bom.md`](/hardware/ledger/bom.md) §7 row, and the SIG-9 cable length.
