...
6. **Cold-core bearing plane across the Y seam.** The core lands flat on the floor slab, and the box's Y seam falls at y = 223.3 — inside the core's own 200–381 footprint — so its datum is two printed pieces meeting at a shiplap. Open: a first-article check that the two floor slabs come off the plate flat and level enough that the core does not rock across the joint. The lid it stands on is a 283 × 181 plate over pour foam, stiff at its perimeter wall and unsupported inboard, so a step at the seam is taken at the perimeter where the cap wall bears.
