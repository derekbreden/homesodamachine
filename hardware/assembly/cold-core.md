...
The two caps pour identically but they are **not** the same part: only `foam-cap-top` (and `foam-cap-lid-top`) carries the CO2 bore + boss, and only the top cap carries the eight **deck-mount columns**. Those stand on its floor and are what the controller PCBA and the PSU bolt to in the finished machine, so the foam pours around their shanks and they are trimmed to nothing. The two sets end at different heights: the board's four rise through clearance holes in the lid and stand proud of it, holding the board's through-hole tails clear, while the PSU's four stop at the cap's mouth rim under the lid, and the PSU lies on the lid's own face with its screws crossing it. Set a ruthex M3 short into each column's top bore before the pour, the same iron and the same feel as the twelve shell-face inserts at step 2. Keep them labeled — the top cap also goes on rotated 180° at step 7. Geometry detail at [`/hardware/printed-parts/cold-core/foam-cap/foam_cap.py`](/hardware/printed-parts/cold-core/foam-cap/foam_cap.py).
