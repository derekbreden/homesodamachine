...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated against the refrigeration base — the condenser flush on it, the compressor's plate just short of that plane — so a bore struck there would open into that base rather than into the machine. What crosses that face is the **two reed cables** at the front port field and the **three lines in the wall's two lane slots**, and nothing else.
  ...
  **Every vessel is filled high and drawn low.** Each of the three — the carbonator and both reservoirs — is entered above its liquid and drawn at its lowest point, so nothing that enters can leave without crossing the liquid. That is what the air-purge and clean-flush service modes in [`fluid-topology.md`](/hardware/topology/fluid-topology.md) run on.
  ...
  - Refrigerant inlet, refrigerant outlet, PRV vent LLDPE → **two** Z-elongated slots in the −X outer wall, one per lane. The port lane's stands directly above the front port field and carries the evaporator **inlet** low with the **PRV vent** one bore pitch over it; the west lane's carries the evaporator **outlet** alone, at the height the inlet crosses at. Not one of the three crosses at the height its own fitting sits at — each line turns onto its lane and drops it — so the port lane's two continue the field's own column at the field's own pitch. The PRV vent LLDPE press-fits into the prv-shroud cap, comes down the port lane and through its slot at its own Z height, and terminates open inside the appliance interior.
- **PRV vent LLDPE** press-fits into the cap of the [`/hardware/printed-parts/cold-core/prv-shroud/`](/hardware/printed-parts/cold-core/prv-shroud/) subassembly on Port 4 (threaded into the vessel at [`pressure-vessel.md`](/hardware/assembly/pressure-vessel.md) step 9). The LLDPE routes from the cap, takes a slight bend, and enters the port lane's slot at its allocated Z height. Far end terminates open inside the appliance interior.
- **Three copper plugs** — lower, middle, top — slid down into the two slots from above: `lower` and `middle` tile the port lane over its two pass-throughs, and `top` is the whole of the west lane's own stack (binder-clip geometry in [`/hardware/printed-parts/cold-core/foam-shell/README.md`](/hardware/printed-parts/cold-core/foam-shell/README.md) "The two lane slots and their copper plug stacks")
