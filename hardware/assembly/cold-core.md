...
- **Penetrations routed through the outer shell walls:**
  - CO2 inlet → dedicated Ø[6.5 mm](TUBE_HOLE_D) hole, +Z outer wall, at x=[+19.05](COTWO_INLET_X) — beside the water outlet and at the same height, one bore run straight through the outer wall and the tank support ring on the bottom plate's own port line. **Nothing turns in the cavity.** The vessel's bottom-plate TAISHER NPT elbow and its PP010822E 1/4" PTC × 1/4" NPT M adapter are made up at the bench and hang inboard of the ring; the 1/4" OD LLDPE is pushed in from outside once the vessel is seated, and bottoms in that collet. Tug-test it before the pour — it is the one joint the foam puts out of reach
  - Water outlet → dedicated Ø[6.5 mm](TUBE_HOLE_D) hole, +Z outer wall
  - Reservoir lines (+X, −X) → dedicated Ø[6.5 mm](TUBE_HOLE_D) holes through the bag-pocket +Z wall at x=[±97](FLAVOR_HOLE_X) — the same wall the shared slot pierces, not the ±X far walls. Each line then turns along the pour band and leaves the +Z **outer** wall well inboard, at x=[±47](FLAVOR_SHELL_X), through a second hole on its own axis. Each reservoir's reed cable does the same through its own Ø[6.5 mm](TUBE_HOLE_D) pair, leaving the pocket wall at x=[±109](CABLE_HOLE_X) outboard of the flavor line ([`reservoir/level-sensing.md`](/hardware/printed-parts/cold-core/reservoir/level-sensing.md)) and the outer wall at x=[±60](CABLE_SHELL_X). The two-bore geometry is in [`foam-shell/README.md`](/hardware/printed-parts/cold-core/foam-shell/README.md) "Two-bore front pass-throughs"
