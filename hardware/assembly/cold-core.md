...
- The **DS18B20 tank-wall probe** (bare TO-92, family 0x28, leads heat-shrunk) taped flat against the carbonator OD, in the bare band below the wind so no wrap has to climb over it. This is the compressor-cycling setpoint sensor, so it reads the carbonator wall, not the coil.
...
Wind GOORY 1/4" OD × 0.031" wall ACR copper tubing as a single-layer helical coil around the carbonator OD — [13.41 ft](LAID_FT) of wrap per carbonator as it lies on the tank, plus one tie-in allowance per end: [335.5 mm](STUB_INLET) at the inlet, [429.6 mm](STUB_OUTLET) at the outlet. **The two are not the same length, and neither is spare copper.** Each is that tail's own run — off the helix, down its lane, out through the copper-plug wall — plus the length that protrudes past the plug for the brazier to work with; the outlet's in-shell run is the longer of the two, its protrusion the shorter. The tie-in stubs exit through the foam-shell's copper-plug holes; brazing happens in [`refrigerant-loop.md`](/hardware/assembly/refrigerant-loop.md).
...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated against the refrigeration base — the condenser flush on it, the compressor's plate just short of that plane — so a bore struck there would open into that base rather than into the machine. What crosses that face is the **three lines in the wall's two lane slots**, and nothing else — no round bore at all. The two reed cables leave by the lid with everything else.
  ...
  **Every carbonator is filled high and drawn low.** Each of the three — the carbonator and both reservoirs — is entered above its liquid and drawn at its lowest point, so nothing that enters can leave without crossing the liquid. That is what the air-purge and clean-flush service modes in [`fluid-topology.md`](/hardware/topology/fluid-topology.md) run on.
  ...
  - CO2 inlet → the top cap's `co2-in` conduit, and the only one of the seven that FEEDS rather than drains. **The lane end is fed from above:** the line falls the shell's whole height down the port lane, turns once in the open onto the bore's own axis, and runs out through the **one bore in the tank support ring** — a Ø[6.5 mm](TUBE_HOLE_D) hole leaning across the shell's floor from the bottom plate's lane-side port at y=[-19.05](COTWO_INLET_Y) to a mouth under the conduit's own column. The carbonator's bottom-plate TAISHER NPT elbow and its PP010822E 1/4" PTC × 1/4" NPT M adapter are made up at the bench, clocked to this line, and hang inboard of the ring; the tube bottoms in that collet, so **nothing is made up in the cavity**. Lay it down the lane before the top cap goes on — it is not pushed in from outside — and tug-test it before the pour, because it is the one joint the foam puts out of reach. Inside the carbonator that port feeds the barb, the silicone stub and the sparge stone hanging in the water column, so the gas enters **below** the liquid and dissolves on the way up
  - Carbonated-water outlet → the top cap's `carb-water-out` conduit, and the carbonator's own drain. **It has no bore in this shell at all.** The bottom-plate Port 3 elbow turns the line laterally under the tank, and it leaves the tank support ring through **one of the ring's own slots** — the 225° one, which already exists to let the pour reach the under-tank floor and which lies where a line heading for this conduit's column wants to go. The CO2's lean crosses at the 315° slot, one each. So neither crossing costs a bore and all four bearing segments stay whole. From there the 1/4" OD LLDPE climbs beside the coil, inboard of the port lane because the CO2 owns the lane's own strip at the bottom of the shell, and steps out onto the lane and into the conduit only once the tank's top plate is under it. Lay it before the pour: it is potted the whole way up
...
- Both temperature probes potted in the foam against their metal surfaces — DS18B20 (tank-wall, 0x28) on the carbonator OD, DS18S20 (coil, 0x10) at the suction end of the coil — leads routed out the +Z slot and sealed, joining SIG-1 at the cold-core exit
