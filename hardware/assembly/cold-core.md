...
Out: a fully foam-poured cold core, capped + gasketed top and bottom, with the wound evaporator coil bonded around the vessel and its inlet/outlet stubs protruding through the foam-shell's copper-plug exits — [200 mm](PROT_INLET) of inlet stub, [150 mm](PROT_OUTLET) of outlet stub, measured from the plug face.
...
| GOORY 1/4" OD × 0.031" wall ACR copper tubing | B0DKSW5VL9 | ~[15.84 ft](CUT_FT) per vessel for coil wrap + tie-in stubs ([1/3](ROLL_SHARE) of 50 ft roll per build) — ACQUIRED |
...
| ruthex M3 short heat-set inserts × [36](CC_INSERTS) — every one this procedure presses ([24](CAP_INSERTS) foam-caps: [12](CAP_CLAMP_INSERTS) clamp bosses plus [12](CAP_DECK_INSERTS) deck-mount columns, + [12](RES_INSERTS) reservoir). The touch-flo plate's and the power column's are pressed elsewhere; `bom.md` §13 carries the appliance total | ruthex B0D39W228K (100-pc bag) | ON-ORDER |
...
Wind GOORY 1/4" OD × 0.031" wall ACR copper tubing as a single-layer helical coil around the vessel OD — [13.41 ft](LAID_FT) of wrap per vessel as it lies on the tank, plus one tie-in allowance per end: [335.5 mm](STUB_INLET) at the inlet, [404.6 mm](STUB_OUTLET) at the outlet. **The two are not the same length, and neither is spare copper.** Each is that tail's own run — off the helix, down its lane, out through the copper-plug wall — plus the length that protrudes past the plug for the brazier to work with; the outlet's in-shell run is the longer of the two, its protrusion the shorter. The tie-in stubs exit through the foam-shell's copper-plug holes; brazing happens in [`refrigerant-loop.md`](/hardware/assembly/refrigerant-loop.md).
...
Wind around the printed [coil-mandrel](/hardware/printed-parts/cold-core/coil-mandrel/coil_mandrel.py) — hollow PETG cylinder with a shallow [1 mm](GROOVE_DEPTH) helical guide groove, mandrel OD [123 mm](MANDREL_OD), tank OD [127 mm](TANK_OD), net coil undersize [3 mm](NET_UNDERSIZE). Wind length [119.4 mm](WIND_LENGTH), [9.687](TOTAL_WRAPS) wraps, pitch [12.33 mm](PITCH) — [4.06 m](SPRUNG_LEN) of copper in the wrap once it has sprung off the mandrel onto the tank. The wrap runs between the two tails at Y=[47](TAIL_INLET_Y) and Y=[166.4](TAIL_OUTLET_Y) — the elbow bands clear of the vessel, not the slot stations. The tails leave mirrored about the mandrel's centre plane, one either side, because each takes its own lane: the inlet to the port lane, the outlet to the west, and each drops its own lane's slot to a station at the height the other's is at. Pull the wound coil off the mandrel and slip it onto the foil-taped vessel. Coil springback: 1–3 mm radial. The wraps land where the pitch puts them; the bridge lifts whichever ones cross it, so the coil is clocked only by its two tails, not to the vessel.
...
The two caps pour identically but they are **not** the same part: only `foam-cap-top` carries the **deck-mount columns**, and only `foam-cap-lid-top` carries their clearance holes and the [3](CAP_CRADLES) **valve cradles** on its outer face. The columns stand on the cap's floor, so the foam pours around their shanks and they are trimmed to nothing. Four stations, two columns each, all flush at the cap's mouth rim under the lid; nothing bolts to one today, and what they are is the cap's rotation key and eight bolt stations standing ready ([`bom.md`](/hardware/ledger/bom.md) §7). They are owned by [`_cold_core_interface.deck_mounts`](/hardware/printed-parts/cold-core/_cold_core_interface.py). Set a ruthex M3 short into each column's top bore before the pour, the same iron and the same feel as the twelve shell-face inserts at step 2.
...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated against the refrigeration base — the condenser flush on it, the compressor's plate just short of that plane — so a bore struck there would open into that base rather than into the machine. What crosses that face is the **two reed cables** at the front port field and the **three lines in the wall's two lane slots**, and nothing else.
  ...
  **Every vessel is filled high and drawn low.** Each of the three — the carbonator and both reservoirs — is entered above its liquid and drawn at its lowest point, so nothing that enters can leave without crossing the liquid. That is what the air-purge and clean-flush service modes in [`fluid-topology.md`](/hardware/topology/fluid-topology.md) run on.
  ...
  - Reed cables → **both** cross the −Y bag-pocket wall, whichever side they come from, at their own Ø[6.5 mm](TUBE_HOLE_D) crossing at x=[±109](CABLE_HOLE_X) ([`reservoir/level-sensing.md`](/hardware/printed-parts/cold-core/reservoir/level-sensing.md)) — because the front port field is on the port lane and there is one of it. Each runs through the open bag-pocket bottom to its pocket bore, then climbs the lane to its own station, the two stations one bore pitch apart at the bottom of the field. Reservoir A's draw crosses that same wall a bore pitch inboard of A's cable; B's cable is the only thing in its own pocket that comes out this side
  - Refrigerant inlet, refrigerant outlet, PRV vent LLDPE → **two** Z-elongated slots in the −X outer wall, one per lane. The port lane's stands directly above the front port field and carries the evaporator **inlet** alone; the west lane's carries the **PRV vent** low with the evaporator **outlet** one bore pitch over it. Not one of the three crosses at the height its own fitting sits at — each line turns onto its lane and drops it — so all three stand at the field's own pitch. The vent is UNDER the copper because that tail falls IN the west lane from its wrap to its own station, which closes that column at every storey above it. The PRV vent LLDPE press-fits into the bore in the prv-shroud's barrel, comes down the west lane and through its slot at its own Z height, and terminates open inside the appliance interior.
- **PRV vent LLDPE** press-fits into the barrel bore of the [`/hardware/printed-parts/cold-core/prv-shroud/`](/hardware/printed-parts/cold-core/prv-shroud/) subassembly on Port 4 (threaded into the vessel at [`pressure-vessel.md`](/hardware/assembly/pressure-vessel.md) step 9). The bore opens on the barrel's underside, already pointing down the lane, so the LLDPE falls straight from it and enters the west lane's slot at its allocated Z height with one corner. Far end terminates open inside the appliance interior.
...
- Drop the pre-soldered reed columns into the still-open reed channels, one per ±X side, from above. Feed each column's 5-conductor cable out through the channel's open back face, down through the air space in the bag pocket below the reservoir, out its Ø[6.5 mm](TUBE_HOLE_D) hole at x=[±109](CABLE_HOLE_X) in the pocket's −Y wall, then up the port lane to its own front-field station. Lay it in the band before the pour: it is potted where it crosses
...
- Vessel + bonded coil installed, seated in the cylinder cavity, surrounded by foam; coil stubs protruding through the foam-shell's copper-plug exits — [200 mm](PROT_INLET) at the inlet, [150 mm](PROT_OUTLET) at the outlet
...
- All seven cap conduits carrying their lines, and every vessel filled above its liquid and drawn at its lowest point
...
- Reed columns dropped into the reed channels, their cables out the −X wall at the front port field's two stations — the only thing on that face besides the two lane slots
