...
| GOORY 1/4" OD × 0.031" wall ACR copper tubing | B0DKSW5VL9 | ~[15.92 ft](CUT_FT) per carbonator for coil wrap + tie-in stubs ([1/3](ROLL_SHARE) of 50 ft roll per build) — ACQUIRED |
...
Wind GOORY 1/4" OD × 0.031" wall ACR copper tubing as a single-layer helical coil around the carbonator OD — [13.41 ft](LAID_FT) of wrap per carbonator as it lies on the carbonator, plus one tie-in allowance per end: [335.5 mm](STUB_INLET) at the inlet, [429.6 mm](STUB_OUTLET) at the outlet. **The two are not the same length, and neither is spare copper.** Each is that tail's own run — off the helix, down its lane, out through the copper-plug wall — plus the length that protrudes past the plug for the brazier to work with; the outlet's in-shell run is the longer of the two, its protrusion the shorter. The tie-in stubs exit through the foam-shell's copper-plug holes; brazing happens in [`refrigerant-loop.md`](/hardware/assembly/refrigerant-loop.md).
...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated against the refrigeration base — the condenser flush on it, the compressor's plate just short of that plane — so a bore struck there would open into that base rather than into the machine. What crosses that face is the **three lines in the wall's two lane slots**, and nothing else — no round bore at all. The two reed cables leave by the lid with everything else.
  ...
  **Every carbonator is filled high and drawn low.** Each of the three — the carbonator and both reservoirs — is entered above its liquid and drawn at its lowest point, so nothing that enters can leave without crossing the liquid. That is what the air-purge and clean-flush service modes in [`fluid-topology.md`](/hardware/topology/fluid-topology.md) run on.
  ...
  - Reservoir draws → each reservoir's line starts on its **floor bulkhead, at the bottom of the wet V and the lowest drainable point in the cavity**. The elbow under the raised floor turns it laterally and it crosses its pocket's own ±Y wall at x=[±97](FLAVOR_HOLE_X), on the Ø[6.65 mm](TUBE_HOLE_D)-around-1/4"-OD tight fit every penetration here takes: **A crosses its −Y wall onto the port lane, B its +Y wall onto the west lane**, because that is the wall each one's elbow points at. Both then come about and climb the forward band — the [6.8 mm](FORWARD_BAND) strip between a pocket's own wall and the shell's — potted to their own `reservoir-a` and `reservoir-b` conduits. **Neither crosses the −X wall**, and neither has a second bore in the shell: the band each lands in runs clear to the open top, and the feature at the far end of it is a hole in the cap
...
1. ~~**Carbonator reed mounting method.**~~ **CLOSED.** The printed [`reed-bridge`](/hardware/printed-parts/cold-core/reed-bridge/) is the mount, the retention and the height gauge at once: reed pockets cut through to bare 316L so the mount adds no standoff, a 3 mm plateau the crossing wrap rides over, ramps on all four sides so the coil can be dragged down over it, and its own setting gauge off the tube's bottom rim. Heights `CLO` [67.12 mm](LOW_LEVEL) / `CHI` [95.25 mm](HIGH_LEVEL), derived from the wetted column in the part's README. Azimuth locked at [`pressure-vessel.md`](/hardware/assembly/pressure-vessel.md) open item 6.
