...
The two caps pour identically but they are **not** the same part: only `foam-cap-top` carries the **deck-mount columns**, and only `foam-cap-lid-top` carries their clearance holes. Those stand on its floor and are what the valve trays bolt up into in the finished machine, so the foam pours around their shanks and they are trimmed to nothing. Four tray stations, two columns each, all flush at the cap's mouth rim under the lid — the tray lies on the lid's own face with its screws crossing it. The stations are owned by [`_cold_core_interface.deck_mounts`](/hardware/printed-parts/cold-core/_cold_core_interface.py). Set a ruthex M3 short into each column's top bore before the pour, the same iron and the same feel as the twelve shell-face inserts at step 2. Keep them labeled — the top cap also goes on rotated 180° at step 7. Geometry detail at [`/hardware/printed-parts/cold-core/foam-cap/foam_cap.py`](/hardware/printed-parts/cold-core/foam-cap/foam_cap.py).
...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated flush against the refrigeration base, which stands the height of the compressor's shroud against it, so a bore struck there would open into that base rather than into the machine. What crosses that face is the **two reed cables** at the front port field and the **three lines in the shared slot** above them, and nothing else.
  ...
  **Every vessel is filled high and drawn low.** Each of the three — the carbonator and both reservoirs — is entered above its liquid and drawn at its lowest point, so nothing that enters can leave without crossing the liquid. That is what the air-purge and clean-flush service modes in [`fluid-topology.md`](/hardware/topology/fluid-topology.md) run on.
  ...
  - Refrigerant inlet (low), refrigerant outlet (high), PRV vent LLDPE → shared Z-elongated slot on the port lane, in the −X outer wall, directly above the front port field. Not one of the three crosses at the height its own fitting sits at: the cold tail climbs the lane to reach the slot, the warm tail and the prv-shroud's line drop to it, so the three continue the field's own column at the field's pitch. The PRV vent LLDPE press-fits into the prv-shroud cap, routes through the slot at its own Z height, and terminates open inside the appliance interior.
...
- **Three copper plugs** — lower, middle, top — slid down into the shared slot from above, sealing between and above its three pass-throughs (binder-clip geometry in [`/hardware/printed-parts/cold-core/foam-shell/README.md`](/hardware/printed-parts/cold-core/foam-shell/README.md) "Shared slot and copper plug stack")
...
- Reed columns dropped into the reed channels, their cables out the −X wall at the front port field's two stations — the only thing on that face besides the shared slot
