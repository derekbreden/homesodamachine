...
In: one hydro-tested + passivated carbonator (output of [`pressure-vessel.md`](/hardware/assembly/pressure-vessel.md)); GOORY 1/4" OD × 0.031" wall ACR copper tubing for the evaporator coil; 3M 425 aluminum foil tape; the printed coil-winding mandrel; the printed shell stack (foam-shell, foam-cap × 2, foam-cap-lid × 2 — all PET-GF; copper-plug × 3 and reservoir × 2 — PETG); TPU 90A gaskets × 2; pour-in-place 2 lb closed-cell polyurethane foam (two-part 1:1); M3 × 25 SHCS × 12 and ruthex M3 inserts × 12 for the caps.
...
| Copper plug × 3 (printed PETG) | [`/hardware/printed-parts/cold-core/copper-plugs/`](/hardware/printed-parts/cold-core/copper-plugs/) | Print — lower, middle, top |
...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated against the refrigeration base — the condenser flush on it, the compressor's plate just short of that plane — so a bore struck there would open into that base rather than into the machine. What crosses that face is the **three lines in the wall's two lane slots**, and nothing else — no round bore at all. The two reed cables leave by the lid with everything else.
  ...
  **Every carbonator is filled high and drawn low.** Each of the three — the carbonator and both reservoirs — is entered above its liquid and drawn at its lowest point, so nothing that enters can leave without crossing the liquid. That is what the air-purge and clean-flush service modes in [`fluid-topology.md`](/hardware/topology/fluid-topology.md) run on.
  ...
  - Reservoir draws → each reservoir's line starts on its **floor bulkhead, at the bottom of the wet V and the lowest drainable point in the cavity**. The elbow under the raised floor turns it laterally and it crosses its pocket's own ±Y wall at x=[±97](FLAVOR_HOLE_X), on the Ø[6.5 mm](TUBE_HOLE_D)-around-1/4"-OD tight fit every penetration here takes: **A crosses its −Y wall onto the port lane, B its +Y wall onto the west lane**, because that is the wall each one's elbow points at. Both then come about and climb the forward band — the [6.8 mm](FORWARD_BAND) strip between a pocket's own wall and the shell's — potted to their own `reservoir-a` and `reservoir-b` conduits. **Neither crosses the −X wall**, and neither has a second bore in the shell: the band each lands in runs clear to the open top, and the feature at the far end of it is a hole in the cap
