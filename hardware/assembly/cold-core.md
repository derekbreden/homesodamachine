...
- **Penetrations routed through the outer shell walls:**
  - CO2 inlet → dedicated Ø6.5 hole, −Z outer wall
...
3. **Reservoir-internal assembly procedure (upstream of this doc).** The reservoir arrives in this procedure already assembled — cap installed, six M3×12 SHCS into ruthex inserts clamping a TPU gasket, PTFE vent membrane installed. That reservoir-internal sequence is not captured in any production-procedure doc; [`../printed-parts/cold-core/reservoir/README.md`](../printed-parts/cold-core/reservoir/README.md) is part-spec + qualification-test-plan, not the assembly SOP.
4. **Reservoir final-qualification status.** The Comfy Materials PETG-Carbon filament is ACQUIRED but the reservoir is still under qualification per [`../printed-parts/cold-core/reservoir/README.md`](../printed-parts/cold-core/reservoir/README.md) "Test Sequence". Production sign-off pending water + syrup-dwell pass.
