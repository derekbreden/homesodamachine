...
- **Penetrations.** **Every fluid line leaves this core by the TOP**, up its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated flush against the refrigeration base, which stands the height of the compressor's shroud against it, so a bore struck there would open into that base rather than into the machine. What crosses that face is the **two reed cables** at the front port field and the **three lines in the shared slot** above them, and nothing else.
