...
| GOORY 1/4" OD × 0.031" wall ACR copper tubing | B0DKSW5VL9 | ~[16.6 ft](CUT_FT) per vessel for coil wrap + tie-in stubs (1/3 of 50 ft roll per build) — ACQUIRED |
...
A cradle is the valve-manifold family's own cell printed into the lid's outer face rather than into a plate that bolts to it ([`_cold_core_interface.cap_cradles`](/hardware/printed-parts/cold-core/_cold_core_interface.py)): a pad with the Beduan's port saddle troughed along it and a blind socket at each of its four corner posts. The valve presses in — no insert, no screw, nothing bonded. The pour hole opens in that same face and its nearest pad corner stands [1.5 mm](POUR_CRADLE_GAP) off it, so trim the cured foam to the plate and not into a pad. Keep the caps labeled — the top one also goes on rotated 180° at step 7. Geometry detail at [`/hardware/printed-parts/cold-core/foam-cap/foam_cap.py`](/hardware/printed-parts/cold-core/foam-cap/foam_cap.py).
