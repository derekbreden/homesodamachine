...
- **Penetrations.** **Every fluid line crosses this core's boundary at the TOP**, up (or, for the CO2, down) its own band to one of the seven conduits in the top cap. No live station goes through a wall: the shell's −X face is mated flush against the refrigeration base, which stands the compressor's own height against it, so a bore struck there would open into that base rather than into the machine. What crosses that face is the **two reed cables** at the front port field and the **three lines in the wall's two lane slots**, and nothing else.
