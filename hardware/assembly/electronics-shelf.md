...
- **AC-1** — [~50 mm](PIGTAIL_SHORT) pigtails on H, N, G with ferrules crimped on the load-side end and the inlet-side end left long ([~150 mm](PIGTAIL_SLACK) slack) for the C14 inlet's solder-tab terminations during [`wiring.md`](wiring.md). Label each conductor "AC-1 H" / "AC-1 N" / "AC-1 G" with heat-shrink flags.
...
1. **Legrand 1597BKCCD12 GFCI module** — between the C14 inlet LINE and the AC distribution block LOAD.
