...
In: the controller PCBA, the Teyleten 3.3 V opto-isolated relays ×2, the Mean Well IRM-90-12ST PSU, Wago 221-413 lever blocks ×5 for the AC and 12 V distribution, the solid-copper ground bus (ring-terminal stack), 16 AWG appliance wire + 22 AWG hookup wire + crimp ferrules/forks/rings, and the RELAYS J5 loom from [`cable-assemblies.md`](/hardware/assembly/cable-assemblies.md).
...
| Wago 221-413 lever-nut connector ×5 | per [`/hardware/ledger/bom.md`](/hardware/ledger/bom.md) §11 | Every distribution splice: H / N / G for the mains and + / GND for the [12 V](DC_BUS_V) rails. Each presses butt-first into a well printed on the enclosure's own +X wall — there is no carrier part and nothing bolts them. Each carries one in-leg and two out-legs. |
