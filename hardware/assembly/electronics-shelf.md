...
| L298N Dual H-Bridge | B0C5JCF5RS (1 of 4-pack) | Drives both Kamoer peristaltic pumps from MCP23017-adjacent ESP32 GPIO; the pumps connect via crimped female faston receptacles on the pump-motor spade tabs. Its onboard 7805/78M05 makes the 5 V logic rail (MCUs + relay VCC). |
| Teyleten 3.3 V opto-isolated relay module ×2 | B07XGZSYJV (2 of 5-pack) | Relay #1 switches the compressor 120 VAC hot leg (ESP32 [GPIO 17](RELAY_COMPRESSOR_GPIO)); relay #2 gates 12 V to the SeaFlo diaphragm pump ([GPIO 16](RELAY_DIAPHRAGM_GPIO)). Both stay on the shelf, outside the compressor shroud per [`/hardware/wiring/power.mmd`](/hardware/wiring/power.mmd). |
...
- **UART hops** — two 4-pin XH headers on the ESP32, shelf-side ends seated, off-shelf ends left open for system integration. **SIG-7** carries the ESP32 UART (GPIO 32 TX / 34 RX) to the shelf-mounted TTL-to-RS485 transceiver (powered from 3.3 V), then RS485 + 12 V + GND to the front-face ESP32-S3-Touch-LCD-4.3B per [`/hardware/printed-parts/enclosure/front-panel/README.md`](/hardware/printed-parts/enclosure/front-panel/README.md). **SIG-6** carries a second ESP32 UART (GPIO 33 TX / 35 RX) + 5 V + GND directly up the umbilical to the 1.47" faucet display — no transceiver.
- **MCP23017 → ULN2803A** — the MCP port presents a **10-pin** header (VCC + GND + 8 GPIO), the ULN input side a **9-pin** header; the connecting ribbon carries the 9 used conductors (GND + the 8 GPIO — VCC not carried across), two per MCP (one per port). Port A of 0x20 trunks into ULN #1's 8-channel input row (the carrier reverses the bit order — GPA_k → input 8−k — because the ULN mounts silk-side-up); Port B[0:3] of 0x20 lands on ULN #2 inputs 1-4. Port A[3] of 0x21 lands on ULN #2 input 5 (the condenser-fan channel). The Reservoir A reed inputs on 0x20 PB[4:7] and Reservoir B reed inputs on 0x21 PA[0:3] terminate at JST headers at the edge of the shelf — those plug into the Keszoox pigtails that route to the reservoir-mounted reeds at [`wiring.md`](/hardware/assembly/wiring.md).
...
- **L298N control** — 6-pin XH on the L298N control row (ENA / IN1–4 / ENB; desolder the stock control pins first). The ESP32 ends (GPIO 27/25/26 for pump A, 19/18/5 for pump B per [`/hardware/wiring/esp32-pinout.mmd`](/hardware/wiring/esp32-pinout.mmd)) land on the DIN-breakout screw terminals. OUT-A and OUT-B land on the peristaltic-pump motor spade tabs via crimped female faston receptacles during [`wiring.md`](/hardware/assembly/wiring.md).
- **3.3 V relay control (LV-1, LV-2, LV-3)** — all screw terminals, no JST: the Teyleten modules' input side is a 3-position screw terminal (VCC / GND / IN), and the ESP32 ends ([GPIO 17](RELAY_COMPRESSOR_GPIO) → relay #1 IN, [GPIO 16](RELAY_DIAPHRAGM_GPIO) → relay #2 IN) land on the DIN-breakout screw terminals. LV-3 feeds 5 V + GND from the L298N's onboard 5 V rail to each relay module's VCC screw terminal.
