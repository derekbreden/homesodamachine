...
- **3.3 V relay control (LV-1, LV-2, LV-3)** — all screw terminals, no JST: the Teyleten modules' input side is a 3-position screw terminal (VCC / GND / IN), and the ESP32 ends ([GPIO 19](RELAY_COMPRESSOR_GPIO) → relay #1 IN, [GPIO 23](RELAY_DIAPHRAGM_GPIO) → relay #2 IN) land on the DIN-breakout screw terminals. LV-3 feeds 5 V + GND from the L298N's onboard 5 V rail to each relay module's VCC screw terminal.
