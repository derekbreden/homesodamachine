...
Not in scope: physical install of the shelf into the enclosure top-back, including chassis-ground-stud landing — that is [`enclosure-mechanical.md`](/hardware/assembly/enclosure-mechanical.md). Landing the AC pigtails into the C14 inlet's solder-tab pins and routing the AC-4/5/6 bundle through the compressor-shroud grommet — that is [`wiring.md`](/hardware/assembly/wiring.md). Flashing firmware to the MCUs and first power-up — that is [`firmware-and-commissioning.md`](/hardware/assembly/firmware-and-commissioning.md). The ESP32-S3 rotary display lives on the front face per [`/hardware/printed-parts/enclosure/front-panel/README.md`](/hardware/printed-parts/enclosure/front-panel/README.md); its UART trunk (SIG-7) lands on the shelf-side header at system integration.
...
Per [`/hardware/wiring/ac-wiring-schedule.md`](/hardware/wiring/ac-wiring-schedule.md) "Sensors and signal", crimp a Keszoox pigtail for each SIG-N run with the shelf-side end terminated in its JST XH housing (soldered onto the appropriate ESP32 GPIO during step 2), and the load-side end left bare with a labeled heat-shrink flag identifying the run by ID (SIG-1 DS18B20 bus, SIG-2/3 reeds, SIG-4 flow meter, SIG-7 S3 rotary display, SIG-9 backflow moisture). The load-side terminations happen at [`wiring.md`](/hardware/assembly/wiring.md) once each sensor is in its final location.
