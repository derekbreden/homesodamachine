...
The production procedure for the appliance's electronics shelf — the parts that ride the cold core's foam-cap top in the band above it, in the service bay's front third with the water deck behind them: the controller PCBA, the Mean Well PSU, both Teyleten relay modules, the AC + DC distribution, and the ground bus. Each bolts to a deck-mount column of the cap itself; the one printed carrier is the [ac-hub](/hardware/printed-parts/electronics/ac-hub/), which holds the three lever nuts that have nothing of their own to bolt with. Feeds [`enclosure-mechanical.md`](/hardware/assembly/enclosure-mechanical.md) alongside [`faucet-and-umbilical.md`](/hardware/assembly/faucet-and-umbilical.md).
...
1. **Relay #2 + DC distribution block station.** Neither has one; both stage loose. What is missing is a root, not room. The cap's deck plane is saturated — no relay-sized rectangle stands free anywhere on it, and neither does a column footprint through the band a second storey's legs would have to cross, because the board, the AC hub, relay #1 and the ground stack leave each other only their placement gaps. The storey above the deck is open, and every board connector sits on an edge row or an edge column, so the board's middle is clear from above. Rooting a second storey means one of: a shouldered column carrying two seats, a bracket sharing the board's own four columns, the clear channel west of the cap that runs the machine's full depth, or the deck area the AC hub holds. `fit.py slab` over the deck band and the one above it is what settles which. Commit a station once the DC-block hardware (per [`/hardware/ledger/bom.md`](/hardware/ledger/bom.md) §11 placeholder) is picked and its footprint is known.
