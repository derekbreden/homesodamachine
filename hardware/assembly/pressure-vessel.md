...
**Plate clocking.** Both plates are welded with their port pairs on the same axis, and the rod register — at right angles to each plate's own pair — is what holds them together: the rod is tack-welded into the bottom plate's register and must enter the top plate's at closure, so a plate turned relative to the other has no register to meet. In the cold core that axis is the foam shell's **±Y** (`_cold_core_interface.vessel_port_offset`): the top plate's water-inlet port leaves straight up through the cap into the service bay, and the SeaFlo lying in that bay is 98 mm wide inside a 181 mm interior, so a port on the shell's ±X axis stands on the machine's centreline the pump covers wherever it is put.
...
- **Port 2 (food-contact water inlet, top plate):** a **straight** JG PP010822E 1/4" PTC × 1/4" NPT M adapter, collet up — no elbow. The line leaves the plate vertically and runs up the top cap's conduit, which stands coaxial with this port ([`cold-core.md`](/hardware/assembly/cold-core.md)).
