...
**Joint — flush ID-fit plug, welded at the rim.** The end plate is an ID-fit plug: its OD is sized just under the tube ID for a ~0.005" radial slip ([`endcap_circular_dxf.py`](/hardware/cut-parts/carbonation/endcaps-circular/endcap_circular_dxf.py)). Seat it **flush** — outer face coplanar with the tube rim — so the full 1/4" plug body fills the last 1/4" of bore. Welded this way the plug body sits directly behind the 0.065" tube wall as an integral chill/backing bar: it sinks heat out of the thin lip and backs the root, so the thin wall can't blow through. Bias the beam onto the thick plate edge and wash it onto the tube-wall edge — both meet coplanar at the accessible rim, and drive enough penetration to fuse across the slip gap (not just skim the outer edges; the hydro test + dye-pen catch a cosmetic-only bead). **Do not recess the plug below the rim:** that strips this backing away, stands the thin wall up proud and unbacked, and buries the only pressure seam in a blind interior fillet. ("Inset 1/4"" refers to the plug's *inner* face sitting 1/4" inboard of the rim — a consequence of seating a 1/4" plug flush, not a recess of the outer face.)
