...
- **SIG-6, SIG-7** — UART trunks to the front-face displays. SIG-6 runs from the ESP32 (GPIO 32 TX / 35 RX + 5 V + GND) to the detachable RP2040 — the display-end cord pays out behind the panel as the customer pulls the display per [`../printed-parts/enclosure/front-panel/README.md`](../printed-parts/enclosure/front-panel/README.md) "RP2040 detach mechanism". SIG-7 runs from the ESP32 (GPIO 15 TX / 34 RX + 5 V + GND) to the fixed ESP32-S3 on the front face. Both runs are internal to the appliance (no longer through the umbilical); shelf end terminates via short JST XH pigtails into the ESP32, display end terminates at the display's pin header.
...
- **Shelf-to-front-face bundle** — SIG-6 (RP2040 detach cord) and SIG-7 (ESP32-S3 fixed) routed forward through the enclosure interior to the front face. The SIG-6 cable's display-end pays out behind the panel when the customer detaches the RP2040; cord-retention scheme per [`../printed-parts/enclosure/front-panel/README.md`](../printed-parts/enclosure/front-panel/README.md) "RP2040 detach mechanism".
- **Shelf-to-umbilical bundle** — SIG-4 (flow meter), SIG-5 (KRAUS air switch), and the under-counter SS plate chassis-ground bond. All ride the umbilical together up to the under-counter zone; spiral-wrap on the shelf side, separator at the umbilical's bulkhead.
...
- SIG-6 (RP2040 detach cord) and SIG-7 (ESP32-S3 fixed) seated at the shelf and landed at the front face displays
...
2. **Umbilical cable selection.** With the display runs (SIG-6, SIG-7) moved to short internal shelf-to-front-face runs, the umbilical now carries only SIG-4 (flow meter, 3 conductors) and SIG-5 (KRAUS air switch, 2 conductors) plus the under-counter SS plate ground bond. The Cat6 oversize that made sense when carrying two display trunks is no longer needed — pick a thinner cable family for the umbilical, or keep Cat6 and accept the spare capacity.
