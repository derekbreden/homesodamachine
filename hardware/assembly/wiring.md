...
- **SIG-6** — direct TTL UART to the gooseneck-mounted ESP32-S3-Touch-LCD-1.47 faucet display, on the BNTECHGO 28 AWG 4-conductor ribbon (TX / RX / [5 V](LOGIC_V) / GND). From the ESP32 (GPIO 32 TX / 35 RX), [~1 m](SIG_UMBILICAL_LEN) up the umbilical to the display, which breaks out TTL UART (no transceiver).
