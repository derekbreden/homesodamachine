...
- **Compressor shroud** — ring terminal at a Ø ~[6 mm](GND_STUD_HOLE) bond point on a side wall. The current shroud DXF has no bond feature — its method (PEM stud, thread-forming screw, or metal-to-metal contact) and whether this internal cover inside the all-plastic enclosure needs the bond at all are open; see [`/hardware/cut-parts/compressor-shroud/README.md`](/hardware/cut-parts/compressor-shroud/README.md) "Grounding & mounting". This is run AC-6 in [`/hardware/wiring/ac-wiring-schedule.md`](/hardware/wiring/ac-wiring-schedule.md).
