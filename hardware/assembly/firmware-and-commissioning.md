...
In: a fully wired chassis fresh out of [`wiring.md`](wiring.md) — AC and DC continuity checks passed, never powered. The flash tooling — three USB cables (one micro-USB for the ESP32-DevKitC, one USB-C for the ESP32-S3, one micro-USB for the RP2040), the `./tools/flash.sh` wrapper (see project root `MEMORY.md`), and the firmware source tree at [`/firmware/`](../../firmware/) configured per [`/platformio.ini`](../../platformio.ini). A multimeter for runtime DC-rail spot checks. A serial console (`pio device monitor`) for log capture.
...
- **MQ-6 hydrocarbon sensor** — needs ~60 s warm-up to reach operating temperature. After warm-up, expect a clean-air baseline reading on its analog input (open the shroud briefly to a known clean-air environment if the bench has solvents or LPG nearby). Architecture: the MQ-6 sits inside the compressor shroud as the hardware-only backstop to the firmware-controlled cutoffs ([`refrigerant-loop.md`](refrigerant-loop.md) "Safety").
