...
Trigger the firmware-override compressor-on command at the serial console. The firmware drops the [3-minute](MIN_OFF_TIME) minimum-off-time guard for this command only, asserts [GPIO 14](GPIO_RELAY1), energizes the Teyleten relay #1, and closes the AC leg into the compressor terminal block inside the shroud.
...
- Carbonator refill threshold: low-level reed ([GPIO 17](GPIO_REED_LOW))
...
Snapshot the serial-monitor output from steps 6–9 into a per-serial log file. At minimum capture: firmware build ID (`fw_version.h`), I²C ACK list, DS18B20 addresses + first-read temperatures, all [10](REEDS_TOTAL) reed baselines, flow-meter pulse count, air-switch press count, MQ-6 baseline, drip-pan baseline, valve self-test pass/fail per channel, suction-line probe temperatures before/during/after the relay #1 verification.
...
- Both MCP23017s ACK'd at [0x20](MCP_VALVES) + [0x21](MCP_RESERVOIRS), DS3231 ACK'd at [0x68](RTC_ADDR), both DS18B20 probes addressed on the 1-wire bus
