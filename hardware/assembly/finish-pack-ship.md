...
| The customer's plumbing | [`/hardware/ledger/bom.md`](/hardware/ledger/bom.md) §3, §4 | Made up at step 6: the John Guest PP0208E tee and the short jumper tube; the white tap-water run with the Waterdrop 15UC-UF filter inline and `tube-collar-water` on its `TAP` end; the ~12 in red tether with the MI4508F4SLF flare connector at its regulator end and `tube-collar-co2` on its `CO2` end. Every end the customer meets is a bare, square-cut push-fit tail. |
...
- The compressor sitting square on its four floor posts, every washer home on its post's crown with the grommet under it visibly taken up and not squashed flat; the green ring on the can's own earth screw, and the AC lead's jacket unbroken back to the power column.
...
First make up the customer's two runs and thread their collars, the way the umbilical bench threads its three ([`faucet-and-umbilical.md`](/hardware/assembly/faucet-and-umbilical.md) §4): `tube-collar-water` onto the white run's `TAP` end, `tube-collar-co2` onto the red tether's `CO2` end, each run up its bare tail by hand with its flag turned outward. The white run carries the Waterdrop 15UC-UF filter inline, pushed home at both of its collets; the tether carries the MI4508F4SLF flare connector at its regulator end. Every end the customer pushes is bare and square-cut.
...
- **The customer's plumbing** (§3, §4): the PP0208E tee and the short jumper; the white run with its filter and `TAP` collar, coiled; the red tether with its flare connector and `CO2` collar. These are what the quick start's steps 4 and 5 push.
...
- **Customer documentation packet on top** (so the customer hits it first when they open the kit): printed safety + regulatory inserts per [`/business/regulatory.md`](/business/regulatory.md) (flame symbol ISO 7010 W021, flammable-refrigerant text marking, R-600a charge mass note, 120 V 60 Hz only warning), and the Founder Edition welcome letter on letterhead, hand-signed. The quick start is not in the kit: it lies on top of everything at step 7.
...
- Carton sealed with appliance + bagged faucet-and-umbilical sub-assembly + install kit (the collet press and cold kit inside it) + customer documentation packet, the quick start lying on top of all of it
...
9. **Kit run lengths.** The white tap-water run and the short jumper ship at lengths the tree does not yet state; the red tether is ~12 in ([`/hardware/ledger/bom.md`](/hardware/ledger/bom.md) §4). Both install uncut, so their lengths are what every kitchen gets.
