...
First make up the customer's two runs and thread their collars, the way the umbilical bench threads its three ([`faucet-and-umbilical.md`](/hardware/assembly/faucet-and-umbilical.md) §4): `tube-collar-water` onto the white run's `TAP` end, `tube-collar-co2` onto the red tether's `CO2` end, each run up its bare tail by hand with its flag turned outward. The white run carries the Waterdrop 15UC-UF filter inline, pushed home at both of its collets; the tether carries the PM4508F4S flare connector and PI061008S stem reducer at its regulator end. Push the reducer's 5/16" stem fully into the connector's 5/16" socket, then push the red 1/4" tube fully into the reducer's 1/4" socket. Tug-check both connections; the pair stays assembled when the customer attaches the tether to the regulator.
...
- **The customer's plumbing** (§3, §4): the tee subassembly — PP0208E, jumper, and the coiled white run with its filter and `TAP` collar; the bagged John Guest ASVPP1LF beside it; the red tether with its flare connector, stem reducer and `CO2` collar. These are what the quick start's steps 5 and 6 push.
