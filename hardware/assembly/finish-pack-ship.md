...
Whether a starter SodaStream concentrate pair + a primed CO2 tank also ride in the install kit is an Open item — the BOM currently calls both user-supplied, but the Founder Edition framing might justify a starter pack to make the first pour happen the day the machine arrives, rather than asking the customer to source CO2 from a local welding-gas supplier (per [`/marketing/target-market.md`](/marketing/target-market.md) "The CO2 pain point needs honesty") before they get their first soda.

### 7. Pack the carton

Stand the empty carton up on its bottom face with the TOP arrow on the interior facing up. Drop one molded foam end-cap into the bottom of the carton, recess facing up. Lift the machine with the umbilical-port row facing the carton's interior-marked REAR wall (so the customer at unboxing pulls the machine out and sees the user-facing front face first, not the +Y wall). Lower the machine into the bottom end-cap; the end-cap's molded recess cradles the machine's lower corners and bottom face. Drop the second molded foam end-cap onto the machine from the top, mating with the machine's upper corners and the funnel surround.

Side voids — the carton is sized so the machine + foam end-caps leave two flanking voids on the long sides of the carton, sized for the two satellite items:

- **Faucet-and-umbilical bag** (output of [`faucet-and-umbilical.md`](/hardware/assembly/faucet-and-umbilical.md)) lays into one side void, coiled to its 8–12" loop diameter per that bench's bagging step. Orient the bag so the faucet sits at the carton's TOP end — keeps the heavier faucet away from the carton's bottom drop zone, which the foam end-caps already cover for the machine itself.
