...
Design intent for the Founder Edition shipping experience lives in [`/marketing/unboxing-and-quickstart.md`](/marketing/unboxing-and-quickstart.md), [`/hardware/future.md`](/hardware/future.md) "Rear-panel nameplate", and [`/marketing/target-market.md`](/marketing/target-market.md) "Founder Edition". The dev-phase task list for the very first unit ships lives in [`/hardware/handwork.md`](/hardware/handwork.md); this doc is the per-unit repeatable bench across the [50](FOUNDER_EDITION_COUNT)-unit Founder Edition run.

## Scope

In: one appliance that has passed acceptance + burn-in per [`acceptance-and-burn-in.md`](acceptance-and-burn-in.md) with its per-serial log archive intact under `logs/<serial>/`; the pre-generated per-unit nameplate plaque (printed per [`/hardware/printed-parts/enclosure/nameplate/README.md`](/hardware/printed-parts/enclosure/nameplate/README.md) with serial + QR baked in, signature still to be applied at this bench); the bagged faucet-and-umbilical sub-assembly (output of [`faucet-and-umbilical.md`](faucet-and-umbilical.md)); the install kit components per [`/hardware/bom.md`](/hardware/bom.md) §14 (Mudder PTFE tubing cutter, CARGEN nitrile foam segments for the cold dispense line, NEMA 5-15P → C13 line cord from §1, and the customer documentation packet); the appliance carton with molded foam end-caps; tracking + labeling supplies.
...
| Mudder PTFE plastic tubing cutter | B08VW15TK8, 1 of 3 pk per appliance | [`/hardware/bom.md`](/hardware/bom.md) §14. Installer trims the umbilical's rear-panel end to actual cabinet depth before pushing into the rear-panel PP1208E bulkheads. |
| CARGEN nitrile foam pipe-insulation segments | B0D2XFK337 ([`/hardware/bom.md`](/hardware/bom.md) §9) | Loose 1-ft segments in the install kit so the installer can extend the foamed cold-dispense run to actual cabinet length. Quantity per appliance TBD pending the umbilical's cabinet-routing-length spec (see [`faucet-and-umbilical.md`](faucet-and-umbilical.md) Open items). |
| Monoprice NEMA 5-15P → IEC C13 line cord, 18 AWG, 6 ft, UL-listed | B08VS8D4WC, 1 of 6 pk per appliance | [`/hardware/bom.md`](/hardware/bom.md) §1. Standard US wall outlet to C14 inlet. |
...
- **BiB adapter** — caps the BiB quick-connect. Customers who don't use a BiB syrup supply (the working default — the BiB path is present but not prominently marketed per [`/hardware/future.md`](/hardware/future.md) "Flavor subsystem") leave this cap in place permanently.
...
- **NEMA 5-15P → C13 line cord** ([`/hardware/bom.md`](/hardware/bom.md) §1): 6 ft, UL-listed Monoprice B08VS8D4WC. Laid flat at the bottom of the kit box.
- **CARGEN nitrile foam segments** ([`/hardware/bom.md`](/hardware/bom.md) §9): loose 1-ft segments for the installer to extend the foamed cold-dispense run to the customer's actual cabinet length, segment count per appliance TBD pending cabinet-length spec. Bundled with a single zip-tie so the segments stay packed together until the installer needs them.
- **Mudder PTFE tubing cutter** ([`/hardware/bom.md`](/hardware/bom.md) §14): used at install to trim the umbilical's rear-panel end before pushing into the PP1208E bulkheads. Sits above the foam and cord so the installer pulls it out first when they get to the umbilical-trim step.
...
6. **Carton + foam end-cap + transit-cap source.** No SKU committed yet for the outer carton, the molded foam end-caps, or the three rear-panel transit caps. Working assumption is a custom-cut carton + custom-poured foam from a local packaging house; the per-unit cost lands in [`/hardware/bom.md`](/hardware/bom.md) once the source is committed.
