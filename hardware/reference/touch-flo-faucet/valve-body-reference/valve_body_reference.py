...
part:
    +Z = up. Z = 0 is the body bottom / countertop deck plane; the
         threaded shank extends in -Z below the deck.
...
The entire front half of the top face (local -X / world -Y) plus the
plateau strip from the water port forward must remain open for the
lever. The shell can only wrap the cylindrical base, the two flanking
arches, and the back end behind the water port.
"""
...
# Zone 0 — threaded shank, clamped through the deck from below by a
# locknut. Thread profile not modeled.
...
# Zone 2 — rectangular column, z = cylinder_height → plateau_z. Long X
# dim carries over from the cylinder; short Y dim is the body's thin
# axis. The two Y-facing short faces meet the cylinder top in a concave
# cove.
rect_long = body_od
...
plateau_z = 39.0
# [6 mm](TRANSITION_FILLET_R) cylinder→rectangle cove radius.
...
# [14 mm](PLATEAU_WIDTH) plateau Y width.
...
port_edge_gap_x = 2.0
# [8.75 mm](PORT_X) port center X.
port_center_x = rect_long_half - port_edge_gap_x - port_radius
port_center_y = 0.0
port_bore_depth = 20.0

# External reference — the deck hole the [11 mm](SHANK_OD) shank passes through.
# [34.93 mm](COUNTERTOP_HOLE) = 1-3/8" standard countertop hole.
countertop_hole_diameter = 34.93

# Plunger geometry. ~[1 mm](PLUNGER_GAP) gap between the port wall (x =
# [3.75 mm](PORT_WALL_X)) and the plunger wall; plunger OD ≈ [5.5 mm](PLUNGER_OD_EST).
...
def build_shank():
    """Zone 0: shank_r cylinder on the body axis, extending shank_length
    below the deck (-Z from z = 0)."""
    return (
...
def build_transition_cove(center_y_sign):
    """Concave cove of radius transition_fillet_r at the cylinder→rectangle
    transition for one Y face, smoothing the corner between the cylinder-top
    ledge and the flat Y-facing face of the rectangular column.
...
def cut_water_port_bore(body):
    """Water port bore: from z = plateau_z down port_bore_depth into the
    body (-Z). Sits in the plateau zone, no arch above it at y = 0."""
    bore = (
        cq.Workplane(xy_plane_z_up)
...
def build_valve_body():
    ...
    # Above the deck the body's lateral envelope is the body_r cylinder:
    # the rectangular column corners, arch rail ends, and cove filler
    # beyond body_r are outside it. Clip range z >= 0 clears the shank.
    clip_cyl = (
        cq.Workplane("XY")
        .circle(body_r)
        .extrude(arc_peak_z)
    ...
    # Seat into the world frame: +90 deg about Z sends +localX (port) ->
    # +worldY (back) and -localX (lever) -> -worldY (front). See docstring.
