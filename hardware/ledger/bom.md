...
| Drip-pan moisture / leak sensor — **source TBD** (generic 2-probe conductivity water-detect module w/ comparator + digital output) | Backflow safety telltale: the Multiplex 19-0897 vent weeps to the internal drip pan and this sensor watches it — a wet pan flags a cross-contamination fault. Digital output → ESP32 GPIO 13 (SIG-9) per [`wiring/esp32-pinout.mmd`](/hardware/wiring/esp32-pinout.mmd); "drip pan dry" is a required check in [`assembly/acceptance-and-burn-in.md`](/hardware/assembly/acceptance-and-burn-in.md). ~$3 est.; specific Prime SKU not yet selected — this sensor was wired + on the carrier (J4 / GPIO 13) but absent from this BOM entirely. | 1 | $3.00 | $3.00 |
