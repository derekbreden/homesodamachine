...
| Press the [65](TOTAL_INSERTS) heat-set inserts — shell faces, cap columns, reservoir caps, touch-flo pods, wall bosses, condenser fingers, the display plate's pair, the faucet display cover's one, the nameplate's pair, the enclosure's [4](SEAM_INSERTS) Y-seam stations — the Z seams slide and take no insert — floor posts | CC-05, ES-01, EN-01 | FX-888D + T18 tip kit, [12](FOAM_CLAMP_INSERTS) of them in the shell faces alone; [61](TOTAL_M3_INSERTS) M3 and the floor's four M5, so the tip changes once | 10 |
