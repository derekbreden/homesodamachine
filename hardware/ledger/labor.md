...
| Press the [60](TOTAL_INSERTS) heat-set inserts — shell faces, cap columns, reservoir caps, touch-flo pods, wall bosses, condenser fingers, the display plate's pair, the collet plate's pair, the nameplate's pair, floor posts | CC-05, ES-01, EN-01 | FX-888D + T18 tip kit, [12](FOAM_CLAMP_INSERTS) of them in the shell faces alone; [56](TOTAL_M3_INSERTS) M3 and the floor's four M5, so the tip changes once | 10 |
| Drive the [60](TOTAL_SCREWS) machine screws that close the build | — | [12](FOAM_SCREWS) foam-cap, [4](PUMP_MOUNT_SCREWS) water-pump, [12](RES_SCREWS) reservoir-cap, [3](TOUCHFLO_SCREWS) touch-flo, [17](SHELF_SCREWS) shelf, [2](COND_SCREWS) condenser, [2](DISPLAY_COVER_SCREWS) display plate, [2](PLATE_RETAINER_SCREWS) collet plate, [2](NAMEPLATE_SCREWS) nameplate, [4](FLOOR_SCREWS) floor | 5 |
