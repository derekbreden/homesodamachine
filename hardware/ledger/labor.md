...
| Press the [71](TOTAL_INSERTS) heat-set inserts — shell faces, cap columns, reservoir caps, touch-flo pods, wall bosses, condenser fingers, the display plate's pair, the faucet display cover's one, the ceiling panel's pair, the nameplate's pair, the enclosure's [10](SEAM_INSERTS) seam cross-pin stations, floor posts | CC-05, ES-01, EN-01 | FX-888D + T18 tip kit, [12](FOAM_CLAMP_INSERTS) of them in the shell faces alone; [67](TOTAL_M3_INSERTS) M3 and the floor's four M5, so the tip changes once | 10 |
| Drive the [71](TOTAL_SCREWS) machine screws that close the build | — | [12](FOAM_SCREWS) foam-cap, [4](PUMP_MOUNT_SCREWS) water-pump, [12](RES_SCREWS) reservoir-cap, [3](TOUCHFLO_SCREWS) touch-flo, [17](SHELF_SCREWS) shelf, [2](COND_SCREWS) condenser, [2](DISPLAY_COVER_SCREWS) display plate, [2](CEILING_SCREWS) ceiling panel, [2](NAMEPLATE_SCREWS) nameplate, [1](FAUCET_DISPLAY_SCREWS) faucet display cover, [10](SEAM_SCREWS) enclosure seam cross-pins — [8](SEAM_M3X10) M3 × 10 plus the [2](CORNER_SCREWS) four-corner M3 × 12, every one driven from a ±X exterior face — [4](FLOOR_SCREWS) floor | 5 |
