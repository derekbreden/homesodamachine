...
| **Bambu TPU Feed Assist Module** | Bambu order us776951289157341185, [purchases.md §15](/hardware/ledger/purchases.md) | Active push on the TPU feed path — short, direct, and free of the drag a long path adds. Ships as extruder components, main control board, PTFE tubes and cabling: **its housing and frame are printed on the machine itself** and are not in the box. Mounts on either hotend — on the right the left hotend still prints (PLA supports under TPU); on the left, dual-hotend printing is off, because the left hotend's vertical travel destabilises the feed. Takes no AMS auto load/unload, and needs a feed that turns its own spool: the AMS HT through its TPU outlet, or the PolyDryer Box TPU already prints out of (see [What dries where](#what-dries-where)). Ordered 2026-09-16, not yet shipped. | [$49.99](T_TPU_FEED) |
