...
|---|---|---|---:|---:|
| Cold-core PET-GF, 0.8 TC | Cold-core shell, four foam-cap pieces — the stack the box's own fluted skin is carried onto, printed on the big nozzle rather than the exterior's fine one ([foam-shell/print-log.md](/hardware/printed-parts/cold-core/foam-shell/print-log.md)) | [13.3](MT_RATE_BULK) h/kg — est., the [12.8](MT_RATE_BULK_PETG) measured plate carried across the stock | [2.480](MT_KG_BULK) kg | [33.0](MT_H_BULK) |
| Enclosure exterior PET-GF, 0.4 TC | The four quadrants, the lower pump cradle and its top clamp, the ceiling panel, display cover plate, tee carrier, two service tabs and two tab locks — the enclosure mechanisms and show surfaces printed at the finish the box is judged on ([enclosure/print-log.md](/hardware/printed-parts/enclosure/enclosure/print-log.md)) | [31.2](MT_RATE_EXT) h/kg — **measured** | [3.215](MT_KG_EXT) kg | [100.3](MT_H_EXT) |
...
| **Printer time per unit** | | | **[6.917](MT_KG)** kg | **[175.8](MT_H_PRINT)** |

Spread across [2](MT_PRINTERS) machines that is **[87.9](MT_H_PRINT_WALL) hours** of wall clock, and it is the longest pole in the build by an order of magnitude.
...
|---|---:|---:|---|
| [2](MT_PRINTERS) × H2C | [87.9](MT_H_PRINT_WALL) h wall | [100](MT_CEIL_PRINT) | **the bottleneck** |
...
At [65 %](MT_DUTY) machine duty — failed prints, plate changes, filament swaps, maintenance, the hours nobody is in the shop to restart a plate — the printers give **[~65](MT_UNITS_YEAR) units a year**. A third H2C moves that to [~97](MT_UNITS_YEAR_3); nothing else bought moves it at all.
...
| Stage | Hours | |
|---|---:|---|
| Print every part | [87.9](MT_H_PRINT_WALL) | 2 printers, both on this unit |
...
| Burn-in | 8.0 | |
| Finish and pack | 1.0 | |
| **Turnaround** | **[115.9](MT_H_TURN)** | **[4.8](MT_DAYS_TURN) days** |
...
A second unit behind the first does not cost another [4.8](MT_DAYS_TURN) days — it costs the bottleneck's [87.9](MT_H_PRINT_WALL) hours, since its prints start the moment the first unit's come off the plates.
