...
**The print rates are measured, not assumed.** Two slices carry the estimate, one per bulk configuration.

- **Cold-core, 0.8 nozzle.** The inner shell sliced at **1142.47 g / [14 h 22 m](MT_MEASURED)** on an H2C — 0.4 layer, PETG, 21 mm³/s volumetric cap, 15 % infill ([foam-shell/print-log.md](/hardware/printed-parts/cold-core/foam-shell/print-log.md)). Against that part's [1.325](MT_MEASURED_KG) kg geometry mass in [bom.md](/hardware/ledger/bom.md) §7 that is **[10.8](MT_RATE_BULK) hours per geometry-kg**.
- **Enclosure exterior, 0.4 High Flow.** The front-top sliced at **[16 h](MT_MEASURED_EXT)** on an H2C — 0.24 layer, 0.42 mm outer wall, PETG, the same 21 mm³/s cap, 15 % infill ([enclosure/print-log.md](/hardware/printed-parts/enclosure/enclosure/print-log.md)). Against that piece's [1.751](MT_MEASURED_EXT_KG) kg of geometry that is **[9.1](MT_RATE_EXT) hours per geometry-kg**. The finer nozzle costs nothing here because both profiles run into the same volumetric ceiling: the melt, not the orifice, is what sets the wall speed.
...
|---|---|---|---:|---:|
| Bulk PETG, 0.8 nozzle | Cold-core shell, four foam-cap pieces | [10.8](MT_RATE_BULK) h/kg — **measured** | [1.963](MT_KG_BULK) kg | [21.2](MT_H_BULK) |
| Enclosure exterior PETG, 0.4 High Flow | The four quadrants, the pump cartridge and its cap — the show surfaces, printed at the finish the box is judged on ([enclosure/print-log.md](/hardware/printed-parts/enclosure/enclosure/print-log.md)) | [9.1](MT_RATE_EXT) h/kg — **measured** | [6.897](MT_KG_EXT) kg | [62.8](MT_H_EXT) |
...
| Small PETG parts | ASSE drip pan, plug stack, PRV shroud, reed bridge, fuse clamp | [30](MT_RATE_SMALL) h/kg — est., travel and layer-change overhead dominate a small part | [0.167](MT_KG_SMALL) kg | [5.0](MT_H_SMALL) |
| PET-CF, 0.4 nozzle | Faucet shell + above-counter plate — fine layers, 50 °C chamber, hardened nozzle | [60](MT_RATE_PETCF) h/kg — est. | [0.163](MT_KG_PETCF) kg | [9.8](MT_H_PETCF) |
| **Printer time per unit** | | | **[10.070](MT_KG)** kg | **[118.1](MT_H_PRINT)** |

Spread across [2](MT_PRINTERS) machines that is **[59.1](MT_H_PRINT_WALL) hours** of wall clock, and it is the longest pole in the build by an order of magnitude.
...
|---|---:|---:|---|
| [2](MT_PRINTERS) × H2C | [59.1](MT_H_PRINT_WALL) h wall | [148](MT_CEIL_PRINT) | **the bottleneck** |
...
At [65 %](MT_DUTY) machine duty — failed prints, plate changes, filament swaps, maintenance, the hours nobody is in the shop to restart a plate — the printers give **[~96](MT_UNITS_YEAR) units a year**. A third H2C moves that to [~145](MT_UNITS_YEAR_3); nothing else bought moves it at all.
...
| Stage | Hours | |
|---|---:|---|
| Print every part | [59.1](MT_H_PRINT_WALL) | 2 printers, both on this unit |
...
| Burn-in | 8.0 | |
| Finish and pack | 1.0 | |
| **Turnaround** | **[87.1](MT_H_TURN)** | **[3.6](MT_DAYS_TURN) days** |
...
A second unit behind the first does not cost another [3.6](MT_DAYS_TURN) days — it costs the bottleneck's [59.1](MT_H_PRINT_WALL) hours, since its prints start the moment the first unit's come off the plates.
