...
- **Enclosure exterior, 0.4 nozzle.** The front-top sliced at **[16 h](MT_MEASURED_EXT)** on an H2C — 0.24 layer, 0.42 mm outer wall, PETG, the same 21 mm³/s cap, 15 % infill ([enclosure/print-log.md](/hardware/printed-parts/enclosure/enclosure/print-log.md)). Against that piece's [1.751](MT_MEASURED_EXT_KG) kg of geometry that is 9.1 hours per PETG kg. The exterior runs PET-GF15 at the 18 mm³/s its own slice carries and 1.43 g/cm³; a nozzle lays grams at (cap × density), so the same measurement reads **[9.5](MT_RATE_EXT) hours per geometry-kg** there. The finer nozzle costs nothing either way — the melt, not the orifice, sets the wall speed.
...
| Small PETG parts | ASSE drip pan, plug stack, PRV shroud, reed bridge, fuse clamp | [30](MT_RATE_SMALL) h/kg — est., travel and layer-change overhead dominate a small part | [0.139](MT_KG_SMALL) kg | [4.2](MT_H_SMALL) |
| Faucet PET-GF, 0.4 TC | Faucet shell + above-counter plate — fine layers, 50 °C chamber, supported the whole height ([faucet-shell/print-log.md](/hardware/printed-parts/faucet/faucet-shell/print-log.md)) | [60](MT_RATE_PETGF) h/kg — est. | [0.181](MT_KG_PETGF) kg | [10.9](MT_H_PETGF) |
| **Printer time per unit** | | | **[10.959](MT_KG)** kg | **[129.7](MT_H_PRINT)** |

Spread across [2](MT_PRINTERS) machines that is **[64.8](MT_H_PRINT_WALL) hours** of wall clock, and it is the longest pole in the build by an order of magnitude.
...
|---|---:|---:|---|
| [2](MT_PRINTERS) × H2C | [64.8](MT_H_PRINT_WALL) h wall | [135](MT_CEIL_PRINT) | **the bottleneck** |
...
At [65 %](MT_DUTY) machine duty — failed prints, plate changes, filament swaps, maintenance, the hours nobody is in the shop to restart a plate — the printers give **[~88](MT_UNITS_YEAR) units a year**. A third H2C moves that to [~132](MT_UNITS_YEAR_3); nothing else bought moves it at all.
...
| Stage | Hours | |
|---|---:|---|
| Print every part | [64.8](MT_H_PRINT_WALL) | 2 printers, both on this unit |
...
| Burn-in | 8.0 | |
| Finish and pack | 1.0 | |
| **Turnaround** | **[92.8](MT_H_TURN)** | **[3.9](MT_DAYS_TURN) days** |
...
A second unit behind the first does not cost another [3.9](MT_DAYS_TURN) days — it costs the bottleneck's [64.8](MT_H_PRINT_WALL) hours, since its prints start the moment the first unit's come off the plates.
