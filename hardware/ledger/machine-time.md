...
| Enclosure exterior PETG, 0.4 High Flow | The four quadrants, the pump cartridge and its cap — the show surfaces, printed at the finish the box is judged on ([enclosure/print-log.md](/hardware/printed-parts/enclosure/enclosure/print-log.md)) | [9.1](MT_RATE_EXT) h/kg — **measured** | [6.243](MT_KG_EXT) kg | [56.8](MT_H_EXT) |
...
| **Printer time per unit** | | | **[9.399](MT_KG)** kg | **[111.9](MT_H_PRINT)** |

Spread across [2](MT_PRINTERS) machines that is **[55.9](MT_H_PRINT_WALL) hours** of wall clock, and it is the longest pole in the build by an order of magnitude.
...
|---|---:|---:|---|
| [2](MT_PRINTERS) × H2C | [55.9](MT_H_PRINT_WALL) h wall | [157](MT_CEIL_PRINT) | **the bottleneck** |
...
At [65 %](MT_DUTY) machine duty — failed prints, plate changes, filament swaps, maintenance, the hours nobody is in the shop to restart a plate — the printers give **[~102](MT_UNITS_YEAR) units a year**. A third H2C moves that to [~153](MT_UNITS_YEAR_3); nothing else bought moves it at all.
...
| Stage | Hours | |
|---|---:|---|
| Print every part | [55.9](MT_H_PRINT_WALL) | 2 printers, both on this unit |
...
| Burn-in | 8.0 | |
| Finish and pack | 1.0 | |
| **Turnaround** | **[83.9](MT_H_TURN)** | **[3.5](MT_DAYS_TURN) days** |
...
A second unit behind the first does not cost another [3.5](MT_DAYS_TURN) days — it costs the bottleneck's [55.9](MT_H_PRINT_WALL) hours, since its prints start the moment the first unit's come off the plates.
