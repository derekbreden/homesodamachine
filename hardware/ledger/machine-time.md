...
|---|---|---|---:|---:|
| Bulk PETG, 0.8 nozzle | Cold-core shell, both enclosure sets, four foam-cap pieces | [10.8](MT_RATE_BULK) h/kg — **measured** | [6.052](MT_KG_BULK) kg | [65.4](MT_H_BULK) |
...
| **Printer time per unit** | | | **[7.169](MT_KG)** kg | **[96.7](MT_H_PRINT)** |

Spread across [2](MT_PRINTERS) machines that is **[48.4](MT_H_PRINT_WALL) hours** of wall clock, and it is the longest pole in the build by an order of magnitude.
...
|---|---:|---:|---|
| [2](MT_PRINTERS) × H2C | [48.4](MT_H_PRINT_WALL) h wall | [181](MT_CEIL_PRINT) | **the bottleneck** |
...
At [65 %](MT_DUTY) machine duty — failed prints, plate changes, filament swaps, maintenance, the hours nobody is in the shop to restart a plate — the printers give **[~118](MT_UNITS_YEAR) units a year**. A third H2C moves that to [~177](MT_UNITS_YEAR_3); nothing else bought moves it at all.
...
| Stage | Hours | |
|---|---:|---|
| Print every part | [48.4](MT_H_PRINT_WALL) | 2 printers, both on this unit |
...
| Burn-in | 8.0 | |
| Finish and pack | 1.0 | |
| **Turnaround** | **[76.4](MT_H_TURN)** | **[3.2](MT_DAYS_TURN) days** |
...
A second unit behind the first does not cost another [3.2](MT_DAYS_TURN) days — it costs the bottleneck's [48.4](MT_H_PRINT_WALL) hours, since its prints start the moment the first unit's come off the plates.
