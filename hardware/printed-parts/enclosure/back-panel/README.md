...
The 3× PP1208E umbilical-port bulkheads stand in **one row** across the top of the field, all three on a single Z, evenly pitched (`enclosure_assembly.PANEL_X`). The order across it is the inboard runs': the two flavor gates take the west of the row, each on the side its run arrives from, and the carb union takes the [east](CARB_END) end with its DIGITEN meter lying inline ahead of it. The **blue-ringed (carbonated-water) bulkhead is the end one** — the user reads it by position in the row, not by a vertex.
