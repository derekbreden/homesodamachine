...
| 5 | Backflow-vent observation | — | Moisture sensor in the internal drip pan under the Multiplex 19-0897's atmospheric vent | n/a for the vent — no panel hole; the basin's own withdrawal slot is an open item | The vent does not exit through the panel. It terminates inside the cabinet over a printed drip pan; the ESP32-monitored moisture sensor in the pan is the telltale. Detail: `../../../future.md` §"Backflow vent monitoring". The basin itself draws aft on rails to be emptied and wants a slot through this panel — see Open items. |
...
The 3-tube umbilical from the faucet down to the rear panel is bundled into a single sleeved run, wound in 1" nominal spiral wrap after both ends are terminated, per [`/hardware/assembly/faucet-and-umbilical.md`](/hardware/assembly/faucet-and-umbilical.md) §4.
...
- `../../../bom.md §3` — water-inlet path (Waterdrop filter, Multiplex 19-0897 backflow, the PI4512F6S + PP061208W outlet stack, water-split PP0208E tee, PP1208E panel bulkhead, HAOCHEN install-kit tee).
...
The 3× PP1208E umbilical-port bulkheads are arranged in a **triangular cluster** on the rear panel — three circles tangent in the densest packing. Three 1/4" OD tubes naturally pack the same way inside the umbilical bundle (densest-three-circle triangle), so the panel-side hole pattern mirrors the bundle-side tube pattern: the user presents the bundle to the panel and each tube already sits in front of its matching bulkhead. The blue-ringed (carbonated-water) bulkhead sits at the top vertex of the triangle. The tap-water bulkhead and the C14 inlet sit left of the cluster — the C14 in the panel's upper corner, directly above the power assembly its cordage drops to, its flange top level with the tap-water nut's. That shared top edge is the field's, and the wall takes its ceiling from it ([`enclosure.py`](/hardware/printed-parts/enclosure/enclosure/enclosure.py) `port_top`).
...
- **Drip-pan withdrawal slot**: the basin rides rails and draws aft to be emptied ([`../drip-pan/README.md`](/hardware/printed-parts/enclosure/drip-pan/README.md)), so it wants a rectangular opening through this face on the basin's own section plus a running clearance, with a bezel on the basin closing it when home. Not cut. The umbilical triangle already stands east of the lane it needs (`UMBILICAL_X`); what is undecided is the bezel — whether it is part of the basin, a separate cap, or a hinged door — and whether the opening wants a lip the bezel seats against.
