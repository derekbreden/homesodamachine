...
- **Flavor A / Flavor B — black.** Standard 1/4" OD black LLDPE from the existing FWS bulk spool (`bom.md §3` and elsewhere). Both flavor bulkheads wear a port ring in the wall's own black, which identifies nothing — flavor A vs flavor B routing is handled by the manifold and is not user-visible at the face. What the ring carries there is the rim, so no station on the face is a different kind of thing from its neighbour.

User rule at install: **blue tube into the blue-ringed bulkhead**. The four unions stand as a rectangle on two columns and two storeys, so the blue one is read by its ring rather than by its place in a line — it takes the [east](CARB_END) column of the upper storey, with the white-ringed tap-water union across from it and the two unmarked flavor unions below. Black-into-either-black is unambiguous because both flavor tubes route through the same wall-side bundle and the user does not need to distinguish them at the face.

The ring is a **separately printed part in a pocket of the wall** — [`../port-ring/`](../port-ring/README.md). The back wall raises a field across the port stations, each marked station carries a pocket one ring's thickness deep in its crown, and the union's own nut draws flange, ring and wall together. Nothing else fastens it, and the pocket is its own depth so it cannot sit proud. Colour is which spool the ring prints in, so the piece running `enclosure-back-top` needs no second filament.

Net identification scheme on the rear face: **blue = carbonated water** [#1f6feb](CARB_COLOR), **black / plain = flavor lines**, **white = tap water** [#ffffff](WATER_COLOR) (the water bulkhead body itself is the white-marked station), **red = CO2** [#d63a3a](CO2_COLOR) (the CO2 bulkhead's own ring, east of the blue one on the same storey — station 4 above).
...
- **Labels** have no CAD. The port rings' pads are cut ([`../enclosure/enclosure.py`](../enclosure/enclosure.py) `_port_field`, struck from [`enclosure_assembly.back_wall_field`](/hardware/manifold-layout/enclosure_assembly.py)); nothing lettered is drawn on the face yet.
