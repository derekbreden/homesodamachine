...
The ring is a **separately printed part in a pad on the wall** — [`../port-ring/`](../port-ring/README.md). The back wall raises one pad per station, a plain cylinder with the ring's pocket cut through it that leaves a rim of printed wall around each ring, and the fitting's own nut draws flange, ring and wall together. Nothing else fastens it, and the pocket is the ring's own depth so the two faces come out one plane. Colour is which spool the ring prints in, so the piece running `enclosure-back-top` needs no second filament.
