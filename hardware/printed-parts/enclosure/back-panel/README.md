# Rear + right-side panels

The external-connection faces of the enclosure. The rear face carries the umbilical port that accepts the three tubes coming down from the under-cabinet faucet through the countertop; the right (+X) side face, in the window behind the source-select row, carries the AC inlet and the tap-water inlet, stacked on one station where the cordage and feed drop beside the electronics stack and water deck they serve. The CO2 inlet lives on the front panel — see [`/hardware/future.md`](/hardware/future.md) §"Enclosure layout". Both faces are walls of the printed enclosure pieces (the holes are cut by [`../enclosure/enclosure.py`](/hardware/printed-parts/enclosure/enclosure/enclosure.py) from `_contents.py`'s port layout), printed in **PETG** — see §"Panel material" below.

The panels have no fluid-pressure duty. They are connection-management faces: hole patterns sized for off-the-shelf bulkheads and panel-mount receptacles, with printed bezels, recesses, and labels where the user-facing fit-and-feel benefits.

## Connections inventory

| # | Connection | Face | Hardware | Panel hole | Notes |
|---|---|---|---|---|---|
| 1 | AC inlet | Right side | IEC 60320 C14 panel-mount receptacle ([MXR B07DCXKNXQ](https://www.amazon.com/dp/B07DCXKNXQ)) | rectangular cutout per C14 spec | Above the tap-water inlet on the shared side-wall station. Recessed [3–5 mm](AC_RECESS_DEPTH) into the panel face with a printed shroud around the perimeter so the C13 cord housing nests flush. Cord housing is the strain relief — no separate grommet on this panel. (The cable gland in `bom.md §11` is the cord strain relief on the compressor shroud, a different sheet-metal part.) Recess detail in §"AC inlet recess" below. |
| 2 | Water inlet | Right side | John Guest PP1208E 1/4" OD black PP push-to-connect bulkhead union ([B00JYFU8MM](https://www.amazon.com/dp/B00JYFU8MM)) — customer pushes the install-kit 1/4" LLDPE into this bulkhead, no tools | ø[17.0 mm](PANEL_HOLE_D) panel hole (identical SKU and hole to the 3 umbilical PP1208E ports on the rear face) | **Customer-facing 1/4" JG QC**, just above the side wall's seam-lip band, below the AC inlet. The install-kit Waterdrop 15UC-UF filter mounts customer-side, inline upstream of this bulkhead (`bom.md §3`). Inboard, the line runs through the PP010822E 1/4" PTC × 1/4" NPT M → GAGIRA 316L SS 3/8" NPT F × 1/4" NPT F reducing coupling → ASSE 1022's 3/8" MPT inlet, then the ASSE 1022's 3/8" MFL outlet → FFL38BARB38 → JoyTube silicone → SeaFlo. The brewhardware FFL38BARB38 sits *inside* the cabinet (downstream of the ASSE 1022's MFL outlet), not on this panel. Install kit ships **two** under-sink tee options + extra 1/4" LLDPE: a JG PP0208E 1/4" PTC × 1/4" PTC × 1/4" PTC union tee (black NSF-cert PP) for homes with 1/4" LLDPE already under the sink, and an HAOCHEN 3/8"×3/8"×1/4" angle-stop add-a-tee for homes with a 3/8" angle stop + braided compression supply. |
| 5 | Backflow-vent observation | — | Moisture sensor in the internal drip pan under the Multiplex 19-0897's atmospheric vent | n/a — drip pan + sensor mount inboard of the panels, no panel hole | The vent does not exit through either panel. It terminates inside the cabinet over a printed drip pan; the ESP32-monitored moisture sensor in the pan is the telltale. Detail: `../../../future.md` §"Backflow vent monitoring". |
| 6 | Umbilical port | Rear | 3× John Guest PP1208E 1/4" OD black PP push-to-connect bulkhead unions ([B00JYFU8MM](https://www.amazon.com/dp/B00JYFU8MM)) | 3× ø[17.0 mm](PANEL_HOLE_D) panel holes (same hole the §8 reservoir-cap PP1208E uses; see `printed-parts/cold-core/reservoir/reservoir.py` lines 251–310 for the pocket / panel-hole geometry that ports here) | In the window right of the bag-circuit row — the one band of the rear face the cold core and the Z-seam lip leave open. Accepts the 3-tube umbilical bundle that runs from the under-cabinet Westbrass faucet down through the countertop to the rear of the appliance: 1× carbonated water + 2× flavor. User pushes each tube into its matching bulkhead — no tools. Same JG black-PP / NSF 51 + NSF 61 / 150 psi @ 70 °F bulkhead family already used inside the cold core, so the SKU is shared and the bulk 10-pack already in stock covers both uses. `bom.md §8`. |
...
The C14 receptacle is recessed [3–5 mm](AC_RECESS_DEPTH) into the right-side panel face with a printed shroud around the inlet perimeter. On insertion, the C13 cord housing nests into the recess and ends flush with the panel surface, masking the gap between cord and inlet bezel. IEC 60320 specifies the male-blade insertion region only, not face-to-face mating distance.
...
- `../../../bom.md §3` — water-inlet path (Waterdrop filter, Multiplex 19-0897 backflow, FFL38BARB38, JoyTube, PP1208E panel bulkhead, HAOCHEN install-kit tee).
...
Design-in-progress. No CAD generator yet. This README is the source-of-truth for the panel's connection inventory until the geometry reaches a CAD generator; see `../nameplate/README.md` for the equivalent state on the sister rear-face artifact.
