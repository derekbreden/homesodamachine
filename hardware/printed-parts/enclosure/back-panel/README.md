...
- **Labels** have no CAD. The port field and its ring pockets are cut ([`../enclosure/enclosure.py`](../enclosure/enclosure.py) `_port_field`, struck from [`enclosure_assembly.back_wall_field`](/hardware/manifold-layout/enclosure_assembly.py)); nothing lettered is drawn on the face yet.
