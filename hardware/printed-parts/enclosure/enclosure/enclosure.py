...
# What the joint reaches inboard of a flank's interior face: groove, arm and head all
# stand inside this. The pack already stands `side_band_inset` off the walls where it
# meets the seam's furniture, and this stands inside that band.
...
def seam_bosses(inner, y_joint, splits):
    ...
    runs over the seam's own storey, mouth to rim, reaching `rail_reach_in`
    inboard — inside `side_band_inset`, which is why the band the pack
...
def _lip_denied(placed, inner, y_span, plate, y_joint):
    ...
    The seam's furniture on a flank is the RAIL: groove, arm and head together reach
    ...
    for x_in, sx, ry0, ry1, _lane in _z_rail_runs(inner, y_joint, col, plate):
        ...
        if yb <= ya + 1e-6:
            continue
        lane = _ybox(min(x_in, x_in + sx * rail_reach_in),
                     max(x_in, x_in + sx * rail_reach_in),
                     ya, yb, iz0, iz1)
...
def _z_rail_runs(inner, y_joint, col, plate, chase=()):
    ...
    both columns alike, because both tops slide the same way. `lane_aft` is how far
    aft of the stop the channel carries its FULL section (`_z_rail_channels`).
    ...
    a rail may stand only where the top's own solid can open for it. The two columns
    answer that differently because their aft ends are different things.

    THE FRONT COLUMN OPENS ALL THE WAY THROUGH. Aft of its run the top piece carries
    nothing but its own seam band and the Y-seam TONGUE, and both of those are
    inboard surfaces the channel may cut — so the lane runs clear off the piece's aft
    end and NOTHING aft of the run has to be swept around. The run then reaches its
    own structural limit, `wall + z_lip_y_margin` short of the joint, where the Y
    telescope's overlap begins and the Z lip must stop. Fore it opens on the tee
    wall: fore of `plate["wall_aft_y"]` the top stands that wall across the flank at
    home, and nothing of front-bottom stands proud of the mouth fore of it.

    THE BACK COLUMN CANNOT. Its aft end is the rear end wall's outer skin and corner
    rounds — SHOW surface, which no channel opens — so a rail there ends where that
    skin's sweep begins: run and travel meet at the HORIZON (`_rail_horizon`), and
    the lane needs no deep zone because the horizon has already emptied it. The flank
    that carries the PRV chase stops sooner still — `slide_slip` fore of the rib —
    and takes its stop block there, mid-flank."""
...
def _rail_x(x_in, sx):
    ...
    outboard face (`slide_slip` of lap clearance off the top's own wall), the foot's
    inboard face one `hook_lap` past it — the catch is the overlap of those two — the
    arm's sliding face `slide_slip` past that, and the arm's back."""
    x_hk = x_in + sx * slide_slip
    x_f = x_hk + sx * hook_lap
...
def _z_rail_heads(inner, y_joint, zj, col, plate, chase=()):
    ...
    is the joint's one down-looking flat — `hook_lap + slide_slip` proud, an
...
def _z_rail_feet(inner, y_joint, zj, col, plate, chase=()):
    ...
    Each is one box: the wall's inner face out to the foot's face, mouth to the
    ...
    rooted to the wall down its whole height; where a grown flank already carries
    that section the slab fuses into it, and the channel cut finds one figure
    either way."""
    z_foot = zj + hook_foot
    out = None
    for x_in, sx, y0, y1, _lane in _z_rail_runs(inner, y_joint, col, plate, chase):
...
def _plate_fore_guides(inner, outer, bay, plate, pump_trays):
    ...
    Nothing stands in that lane — the pump cartridge's sweep ends one `plate_guide_x_air`
    inboard of the cheek at every height — so the extra section is free.
...
def plate_head_spans(bay, inner):
    ...
    One per outer tail, and outboard of the pump cartridge's whole sweep at both ends."""
    cart_x0, cart_x1 = _cap_x_span(bay)
    return [(inner[0], cart_x0 - plate_guide_x_air),
            (cart_x1 + plate_guide_x_air, inner[1])]
...
def _back_top_flanks(inner, outer, box, y_joint, zj):
    ...
    # The PRV chase cuts its own share of this band later (`_vent_chase`): each piece carries
    # the height of the passage it owns.
...
def _vent_chase(solid, inner, outer, stations, y0, y1, z0, z1):
    ...
    EACH PIECE CARRIES ITS OWN HEIGHT OF IT. The passage crosses the Z seam, so the rib is
    clipped to the band the piece owns and both pieces build their share: back-bottom from
    the ramp's run-out up to the seam plane, back-top from that plane up to the mouth.
    Below the seam the piece's own skin is the duct's west face, as it is for the groove;
    above it the liner is.

    THE TWO HALVES BUTT AT THE SEAM PLANE, closed to the seam's own tolerance and no closer
    — this is a relief path that opens to the room through the wall two storeys down, not a
    sealed duct.

    WHAT THIS COSTS IS THE SLIDE, AND `z-slide-back-clear` READS IT. The rib is 16 mm along
    Y and the passage through it is 12, so 2 mm of it stands at each end at the duct's full
    depth — out to `interior_x`, which is the band back-bottom's rail head occupies. Given
    to the piece that MOVES, those two slabs sweep the length of that rail.
    ...
    for sx, sy, sz in stations:
        ...
        # The station is the flank's; the HEIGHT is this piece's, and a piece the passage
        # does not reach builds nothing.
        ...
        # CLIPPED TO THE PIECE'S OWN HEIGHT. The seam plane is where the two halves meet.
        rib = rib.intersect(_ybox(outer[0] - 1.0, outer[1] + 1.0,
                                  sy - half - 1.0, sy + half + 1.0, z0, z1))
...
def build_piece(box, y_side, z_side, halves_cache=None):
    ...
    if y_side == "front" and z_side == "top" and box.pump_bay:
        ...
        piece = piece.fuse(_plate_cap(inner, box.collet_plate, box.pump_bay))
