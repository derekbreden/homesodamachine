...
- +Y is the appliance's height axis. y=0 BOTTOM, y=H TOP.
- +Z is the appliance's depth axis. z=0 FRONT, z=APPLIANCE_D BACK.
- The box origin is at the front-bottom-left corner.

Cold-core placement:
The foam shell is built in its own world frame centered on the X and Z
axes (X = [-141.5, +141.5], Z = [-90.5, +90.5]) with its floor at Y=0.
...
condenser/Zone D. The cold core sits on the floor (Y=0)."""
...
#
# Native foam-shell frame centers X and Z at 0, with floor at Y=0:
#   X: -outer_shell_x_length/2 .. +outer_shell_x_length/2
#   Z: -outer_shell_z_length/2 .. +outer_shell_z_length/2
#   Y: 0 .. foam_shell_outer_height
#
...
# - Push it to the BACK so its +Z face hugs the back wall (Z = APPLIANCE_D),
#   leaving the front depth for Zone D / condenser.
# - Sit it on the floor (Y stays at 0).
COLD_CORE_X_OFFSET = APPLIANCE_W / 2
COLD_CORE_Y_OFFSET = 0.0
COLD_CORE_Z_OFFSET = APPLIANCE_D - outer_shell_z_length / 2
...
def build_enclosure_box() -> cq.Workplane:
    ...
    return cq.Workplane("XY").box(APPLIANCE_W, H, APPLIANCE_D, centered=False)
