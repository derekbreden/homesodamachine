...
COMPONENTS = [
    ...
    # Electronics shelf
    _c("pcba",              "real",        True,  "cap",  "Controller board (pcb/pcba), bolted straight to four boss columns of the cold core's top foam cap, which stand on through the cap's lid to hold the board's through-hole tails clear of it — no tray floor under it. A quarter turn from its own frame so the long axis runs down the west column: the USB-C edge (J14) looks south into the open band ahead of the cap, where a hand reaches it with the back panel on, and the J10 12 V throats look north up the column, both edges the board must keep reachable facing a lane rather than a wall, and the twelve top-entry wafers between them plugging from the bay's own opening above. The board is 85.05 x 72.85 as fabbed and its four mounting holes are fixed; the stations they land on are the cap's (`_cold_core_interface.deck_mounts`), so the column, the board and its connector map move as one. M3 SHCS into a ruthex short in each column"),
    _c("psu",               "real",        True,  "cap",  "Mean Well IRM-90-12ST 12 V open-frame supply (reference/meanwell-irm90), on four boss columns of the same cap in the aft strip, west of the drip basin and behind the pump. Its potted base is flat, so those columns stop at the cap's mouth rim and the PSU lies on the lid's own face beside the pump, the screws crossing the lid to reach an insert beneath it. Laid ACROSS the strip: its mounting holes span most of its length, and that span is longer than what the west column has left in Y once the board is in it, while the strip has the run in X once the basin turns narrow. Its AC end faces the rear-panel C14 inlet's own column above it. M3 SHCS into a ruthex short in each column; the AC distribution, both relays, the ground bus and the DC block are not yet stationed"),
]
