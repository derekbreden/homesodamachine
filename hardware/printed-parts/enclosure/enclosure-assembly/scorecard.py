...
COMPONENTS = [
    ...
    _c("suction-chain",     "real",        True,  "none", "MAACFLOW 3/8\" barb + PP450822E 1/4\" PTC, made up as one piece (reference/seaflo-suction-chain) — the discharge chain's two end fittings with nothing between them, since nothing holds pressure off the pump on the inlet side. STANDS UP in the pocket between V-K's outlet and the suction barb, collet DOWN at the tube and barb UP at the stub: that pocket is 60.5 × 45.2 in plan and a Ø17 body's axis has a 49.1 mm diagonal to lie in against the 54.4 this stands made up, so Z is the only dimension that holds it. Seated by its COLLET, the mouth water-4 ends on. Bracket TBD"),
    ...
    _c("nozzle-tray-assembly", "real",     True,  "cap", "V-G, the NOZZLE-A GATE, alone on the family's one-seat plate — the LAST of the aft stand's three rows, one `contents.AFT_TRAY_BAY` behind V-K's. It carries no junction of its OWN: V-G-I is fed by Y-D a storey and a half down in the front column, and V-G-O runs alone to its bulkhead — so the plate is placed by those two runs and by nothing else, its X on the inlet's column (`contents.nozzle_gate_in_x`), its Y on the band the west lane leaves it (`contents.nozzle_tray_y`). TURNED a quarter turn about Z, which is what puts its two collets on ±X: the inlet opens WEST down the lane that feeds it and the outlet EAST toward its bulkhead's column, so the plate's two faces answer the two runs and neither owes a corner to reach the one it wants. Its outlet is one of the only two lines the manifold sends out of the machine, and that is what puts this gate up here instead of in the front column. Bolted flat on the cap's lid through its two mount ears to cap columns under the lid, M3 × 16 into a ruthex short in each; its stations stand where the placed ears land (`contents.TRAY_MOUNTS`), held by `deck-mounts-land`"),
...
def _declare_placement_rules():
    ...
    return {
        ...
        # The suction chain stands free in the pocket east of the pump, on the stub that is the
        # only thing that leaves the suction — nothing under it and nothing beside it it can
        # bear on, so what this rule holds is the pocket's own aft wall: the stand's aft row.
        "suction-chain":     [("clear", "nozzle-b-tray-assembly", 1.0)],
...
def _declare_ports():
    ...
    declared = [
        ...
        _p("tube-port", "suction-chain", "fluid", *contents.suct_terminal("tube-port"), 6.35, "vk-tray-assembly V-K-O — segment water-4", "PP450822E 1/4\" PTC collet, facing DOWN off the foot of the standing chain at the head of water-4's one corner — the valve's own column, one stock arc and a collet's lead off its outlet in each of Y and Z"),
        _p("barb-tip",  "suction-chain", "fluid", *contents.suct_terminal("barb-tip"),  15.1, "seaflo-pump suction — segment water-7", "MAACFLOW 3/8\" hose barb, facing UP off the chain's crown at the stub that climbs the pump's east flank; worm-gear clamp"),
