...
HERE=ROOT/'.cache/prints/2026-09-21-carrier-support-access'
HERE.mkdir(parents=True,exist_ok=True)
DURABLE=ROOT/'hardware/printed-parts/enclosure/tee-readiness/full-enclosure-print/carrier-receiver-access.json'
...
base=ROOT/'.cache/prints/2026-09-21-enclosure-front-mechanism-h2c-v1'
audit_path=base/'enclosure-front-mechanism-black-z018-h2c-v1-input.support-audit.json'
...
sources=[ROOT/'hardware/printed-parts/faucet/prepare_display_print.py',ROOT/'hardware/scripts/enclosure_support_audit.py',native_path,audit_path,gcode,ROOT/'hardware/printed-parts/enclosure/tee-carrier/_simple_carrier.py',ROOT/'hardware/printed-parts/enclosure/tee-carrier/entry-backing-check.json']
...
# The inverse STL/plate transform places emitted nominal contact at Y112.640000916
# against native Y112.640. Classify only this sub-micron rear-plane sliver as
...
result={'status':'pass' if all(c['pass'] for c in checks) else 'fail','scope':'One loose right carrier and one detached receiver-roof support branch; no mechanism geometry change or physical cleanup measurement.',
 ...
 'native_support_island':'interface-11 in the retained four-part native slice','native_roads':count,'native_layers_z_mm':sorted(set(round(z,6) for z in zs)),'maximum_native_width_mm':max_width,'additional_xy_buffer_mm':0.0,
 ...
 'limits':['The check covers a detached short branch, not the entire connected bed-rooted support tree.','Rear-wall contact is present in the emitted support envelope; no positive starting wall gap or low detachment force is inferred.','Detachment force, cleanup time and contact finish remain physical observations.','The final carrier plate must still be sliced in the accepted upright orientation and its actual branch topology/removal route checked.'],
