...
rows = [(enc / 'enclosure-back-top.stl', enc / 'enclosure-back-top.step')]
summary = []
for mesh_path, native_path in rows:
    ...
    pass_failed = 'lint pass failed:' in errors.getvalue()
    report = {
        'status': 'read_only_lint_complete' if not pass_failed and not drift else 'lint_input_or_pass_error',
...
save(HERE / 'summary.json', {'status': 'all_requested_parts_linted_read_only', 'parts': summary,
                           'back_top_is_current_corrected_publication': True})
