...
STEM = 'tee-carrier-plate-black-z004-mark2-v15-six-wall-band-original-order'
...
with zipfile.ZipFile(source) as a, zipfile.ZipFile(target) as b:
    ...
    assert changed == [ranges_member] and b.testzip() is None
differences = {key: {'reference': previous[key], 'value': value}
               for key, value in overrides.items()}
config.update(archive_stem=STEM,
              title='Tee carrier only; six walls at Z 0–6.1 mm; original wall-first order; 15% overlap; no supports; Mark2')
...
preparation = {
    ...
    'wall_sequence': 'Original sequence: inner walls, outer wall, then infill.',
    ...
    'hypothesis': 'Six walls in the lower rounded band may retain the expanding edge with the original wall-first order. This isolates the wall-count change against the steady-cooling v13 reference.',
    'independent_variable': 'Six walls at Z 0–6.1 mm; original wall-first order, speeds, acceleration and 15% overlap.',
    'user_authority': 'Derek requests six walls through the lower 0.08 mm curves and restoring the original order. This archive is prepared for a future print; the active v14 job is unchanged.',
    'physical_reference': 'hardware/printed-parts/enclosure/tee-readiness/full-enclosure-print/native-slice-reviews/2026-09-24-tee-carrier-plate-mark2-v14/physical-result.json',
