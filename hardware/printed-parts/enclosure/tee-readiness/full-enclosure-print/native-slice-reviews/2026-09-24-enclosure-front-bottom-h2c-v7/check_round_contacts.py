"""Check all support roads near the handhold band against actual round facets."""
...
with zipfile.ZipFile(next(JOB.glob('*-input.3mf'))) as z:
...
v=np.array([[float(e.get(a)) for a in ['x','y','z']] for e in tree.iter(tag('vertex'))])+[162.5,160.,90.400001526]
all_faces=np.array([[int(t.get(a)) for a in ['v1','v2','v3']] for t in tree.iter(tag('triangle'))])
f=all_faces[prep['blocked_round_triangle_indices']]
assert len(f)==15984
tri=v[f];low=tri[:,:,2].min(axis=1);high=tri[:,:,2].max(axis=1)
segments=json.loads((JOB/'round-band-support-segments.json').read_text())
assert segments,'No support roads were parsed'
...
report={'scope':'All Support, Support transition and Support interface roads from print Z28.5 through 44.3 mm; full bead width plus 0.05 mm XY allowance; exact clipped downward-round triangle projections through 0.60 mm above support top.',
