...
for side in (-1,1):
    ...
    before=old.intersect(region).translate((-side*cover.skirt_inset,0,0))
    after=live.intersect(region)
    difference=diff(before,after);assert difference<1e-5,difference
    skirt_checks.append({'side':side,'translation_x_mm':-side*cover.skirt_inset,'symmetric_difference_mm3':difference})
...
report={'unchanged_housing_inputs_sha256':paths,'source_export_equivalent':True,'bezel_window_thickness_and_outer_perimeter_unchanged':True,
        'bezel_symmetric_difference_mm3':bezel_difference,'skirt_translations':skirt_checks,'original_reference_step_sha256':sha(JOB/'reference/display-cover.step'),
        ...
        'perimeter_clearance_mm':cover.cover_slip,'skirt_inset_each_mm':cover.skirt_inset,'nominal_catch_overlap_mm':cover.catch_overlap,
