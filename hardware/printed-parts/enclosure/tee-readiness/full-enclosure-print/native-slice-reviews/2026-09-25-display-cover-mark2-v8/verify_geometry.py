...
for side in (-1,1):
    ...
    before=old.intersect(region).translate((-side*(cover.skirt_inset-0.6),0,0))
    after=live.intersect(region)
    difference=diff(before,after);assert difference<1e-5,difference
    skirt_checks.append({'side':side,'translation_x_mm':-side*(cover.skirt_inset-0.6),'symmetric_difference_mm3':difference})
...
report={'unchanged_housing_inputs_sha256':paths,'source_export_equivalent':True,'bezel_window_thickness_and_outer_perimeter_unchanged':True,
        ...
        'perimeter_clearance_mm':cover.cover_slip,'previous_trial_skirt_inset_each_mm':0.6,'additional_skirt_inset_each_mm':cover.skirt_inset-0.6,'skirt_inset_each_mm':cover.skirt_inset,'nominal_catch_overlap_mm':cover.catch_overlap,
