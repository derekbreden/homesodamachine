...
for side in (-1,1):
    ...
    before=old.intersect(region).translate((-side*(cover.skirt_inset-0.9),0,0))
    after=live.intersect(region)
    difference=diff(before,after);assert difference<1e-5,difference
    skirt_checks.append({'side':side,'translation_x_mm':-side*(cover.skirt_inset-0.9),'symmetric_difference_mm3':difference})
...
for lateral in (-cover.cover_slip,0.,cover.cover_slip):
    ...
    each=[]
    for side in (-1,1):
        skirt=cover.build_cover_skirt(side).translate((lateral,0,cover.retention.BEARING_SLIP+.01))
        volume=abs(near.intersect(skirt).Volume())
        overlap=round(cover.catch_overlap+side*lateral,6)
        assert (volume>1e-5) if overlap>1e-5 else (volume<1e-5)
        each.append({'side':side,'catch_overlap_mm':overlap,'retaining_intersection_mm3':volume})
    rows.append({'lateral_shift_mm':lateral,'seated_intersection_mm3':seated_volume,'pull_out_probe_mm':cover.retention.BEARING_SLIP+.01,'retaining_intersection_mm3':caught,'each_skirt':each})
...
report={'unchanged_housing_inputs_sha256':paths,'source_export_equivalent':True,'bezel_window_thickness_and_outer_perimeter_unchanged':True,
        ...
        'perimeter_clearance_mm':cover.cover_slip,'previous_trial_skirt_inset_each_mm':0.9,'additional_skirt_inset_each_mm':cover.skirt_inset-0.9,'skirt_inset_each_mm':cover.skirt_inset,'nominal_catch_overlap_mm':cover.catch_overlap,
        'minimum_catch_overlap_at_full_lateral_float_mm':round(cover.minimum_catch_overlap,6),'glass_intersection_mm3':cover.glass_shadow(),
        ...
        'physical_fit_or_retention_force_measured':False,'retention_limit':'Both hooks overlap 0.3 mm when centered. At full 0.3 mm lateral float one hook has zero overlap and the opposite hook has 0.6 mm; physical retention must be evaluated.','pass':True}
