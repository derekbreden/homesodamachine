...
The mounting datum is [x 66.9, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 46.9 and 86.9](C14_SCREWS). The working opening is [31.95 × 23.17 mm R3](C14_OPENING). The tunnel is one [56.77 × 35.17 mm](C14_BLOCK) block from the pocket's mouth to the wall. Cut into its fore face, the pocket follows the measured [49.77 × 22.17 mm](C14_FLANGE) rounded/tapered flange profile with [0.5 mm](C14_POCKET_SLIP) insertion slip; the block keeps at least [3 mm](C14_POCKET_WALL) of material around that profile in XZ, and the pocket's lip stands [3 mm](C14_POCKET_LIP) past the seated flange's inboard face. The block's 45° print corbel is one wedge its full width over the [10.25 mm](C14_RUN) run: one plane from the mouth's bottom edge to the wall, with material under every point of the block. The block does not move the cutout, screws, or seating plane, and the exterior wall stays on its established planes. The sliding ceiling panel's running pocket follows the block's rectangle.
