...
The mounting datum is [x 66.9, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 46.9 and 86.9](C14_SCREWS). The working opening is [31.95 × 23.15 mm R3](C14_OPENING). Behind the seating plane, a collar follows the measured [49.77 × 22.17 mm](C14_FLANGE) rounded/tapered flange profile with [0.5 mm](C14_POCKET_SLIP) insertion slip, keeps [3 mm](C14_COLLAR_WALL) of printed material everywhere around that profile in XZ, and continues [3 mm](C14_COLLAR_EXTENSION) past the flange's inboard edge. A sheared copy of the exact outer profile runs from the collar mouth to the wall, rooting both wider ends in the wall outside the narrower tunnel relief without widening that tunnel. Its 45° print corbel therefore spans the full [10.25 mm](C14_CORBEL_RUN); the tunnel's companion corbel follows its own R3 lower corners without leaving square ledges or air channels under them. The collar does not move the cutout, screws, or seating plane, and the exterior wall stays on its established planes. Its upper arc travels with the sliding ceiling panel; `c14-collar-complete` measures the installed union and the empty pocket across both printed owners.
