...
The mounting datum is [x 66.9, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 46.9 and 86.9](C14_SCREWS). The nominal working opening is [24.3 × 18.3 mm R1.5](C14_OPENING), with a further [0.25 mm](SUPPORTED_CLEARANCE) toward machine −Z at its supported floor. The tunnel is one [56.07 × 30.55 mm](C14_BLOCK) block from the pocket's mouth to the wall. Cut into its fore face, the pocket is a [49.77 × 22.17 mm](C14_POCKET) rounded/tapered outline — the inlet's [49.97 × 21.88 mm](C14_FLANGE) flange's width and its rim's height — with [0.15 mm](C14_POCKET_SLIP) insertion slip, [5 mm](C14_POCKET_DEPTH) deep from mouth to floor, so the seated part's flange back lies at the mouth; the block keeps at least [3 mm](C14_POCKET_WALL) of material around that outline in XZ. The block runs [10.25 mm](C14_RUN) from the mouth to the wall and its crown fuses into back-top's ceiling slab, so on that ceiling-down print it stands on the bed through the slab and the wall; its underside looks print-up. The block does not move the cutout, screws, or seating plane, and the exterior wall stays on its established planes.
