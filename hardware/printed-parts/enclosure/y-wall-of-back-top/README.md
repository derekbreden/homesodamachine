...
|---|---|---|---|---|---|
| 1 | AC inlet | Rear | IEC 60320 C14 panel-mount receptacle ([MXR B07DCXKNXQ](https://www.amazon.com/dp/B07DCXKNXQ)) | rounded rectangular cutout per C14 spec, bored through a printed tunnel standing round it on the inner face | Easternmost station, on the umbilical row's own storey, at the end of the wall the electronics bay stands against — its cordage drops the wall and runs forward to the AC distribution on that column. It lands from **inside**: flange on that tunnel's fore face with an M3 heat-set either side of the bore, shroud back up the tunnel and out through the cutout, its rim standing [0.75 mm](C14_SHROUD_PROUD) proud of the wall. The C13 cord housing nests [3–5 mm](AC_RECESS_DEPTH) down the same bore on insertion. Cord housing is the strain relief — no separate grommet here, and no cable gland anywhere on this wall. Recess detail in §"AC inlet recess" below. |
...
The recess is the cutout itself, carried inboard. A printed tunnel stands round the hole on the wall's inner face (`enclosure._c14_tunnel`) and the receptacle is screwed to the seating face at the floor of that tunnel's flange pocket, so the bore the C13 cord housing enters runs the whole depth of wall and tunnel together, and the shroud rising back up it stands [0.75 mm](C14_SHROUD_PROUD) proud of the wall's outer plane in the cutout. On insertion the housing nests [3–5 mm](AC_RECESS_DEPTH) down that bore, masking the gap between cord and inlet bezel. IEC 60320 specifies the male-blade insertion region only, not face-to-face mating distance.

The mounting datum is [x 66.9, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 46.9 and 86.9](C14_SCREWS). The nominal working opening is [24.3 × 18.3 mm R1.5](C14_OPENING), with a further [0.25 mm](SUPPORTED_CLEARANCE) toward machine −Z at its supported floor. The tunnel is one [56.07 × 30.55 mm](C14_BLOCK) block from the pocket's mouth to the wall. Cut into its fore face, the pocket follows the measured [49.77 × 22.17 mm](C14_FLANGE) rounded/tapered flange profile with [0.15 mm](C14_POCKET_SLIP) insertion slip; the block keeps at least [3 mm](C14_POCKET_WALL) of material around that profile in XZ, and the pocket's lip stands [3 mm](C14_POCKET_LIP) past the seated flange's inboard face. The block runs [10.25 mm](C14_RUN) from the mouth to the wall and its crown fuses into back-top's ceiling slab, so on that ceiling-down print it stands on the bed through the slab and the wall; its underside looks print-up. The block does not move the cutout, screws, or seating plane, and the exterior wall stays on its established planes.
