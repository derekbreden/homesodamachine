...
The recess is the cutout itself, carried inboard. A printed tunnel stands round the hole on the wall's inner face (`enclosure._c14_tunnel`) and the receptacle is screwed to the seating face at the floor of that tunnel's flange pocket, so the bore the C13 cord housing enters runs the whole depth of wall and tunnel together and nothing on this station stands proud of the wall. On insertion the housing nests [3–5 mm](AC_RECESS_DEPTH) down that bore, masking the gap between cord and inlet bezel. IEC 60320 specifies the male-blade insertion region only, not face-to-face mating distance.

The mounting datum is [x 66.9, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 46.9 and 86.9](C14_SCREWS). The working opening is [31.95 × 23.17 mm R3](C14_OPENING). The tunnel is one [56.77 × 35.17 mm](C14_BLOCK) block from the pocket's mouth to the wall. Cut into its fore face, the pocket follows the measured [49.77 × 22.17 mm](C14_FLANGE) rounded/tapered flange profile with [0.5 mm](C14_POCKET_SLIP) insertion slip; the block keeps at least [3 mm](C14_POCKET_WALL) of material around that profile in XZ, and the pocket's lip stands [3 mm](C14_POCKET_LIP) past the seated flange's inboard face. The block runs [10.25 mm](C14_RUN) from the mouth to the wall and its crown fuses into back-top's ceiling slab, so on that ceiling-down print it stands on the bed through the slab and the wall; its underside looks print-up. The block does not move the cutout, screws, or seating plane, and the exterior wall stays on its established planes.
