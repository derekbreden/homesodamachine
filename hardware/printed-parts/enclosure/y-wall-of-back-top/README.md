...
The mounting datum is [x 69, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 49 and 89](C14_SCREWS). The working opening is [31.95 × 23.15 mm R3](C14_OPENING). Behind the seating plane, a separate collar follows the measured [49.77 × 22.17 mm](C14_FLANGE) rounded/tapered flange profile with [0.5 mm](C14_POCKET_SLIP) insertion slip, keeps [3 mm](C14_COLLAR_WALL) of printed material everywhere around that profile in XZ, and continues [3 mm](C14_COLLAR_EXTENSION) past the flange's inboard edge. The tunnel's 45° print corbel is a sheared copy of its rounded-rectangle profile and follows its R3 lower corners without leaving square ledges under them. The collar does not move the cutout, screws, or seating plane and does not add thickness to the wall the flange bears against. Its upper arc travels with the sliding ceiling panel; `c14-collar-complete` measures the installed union and the empty pocket across both printed owners.
