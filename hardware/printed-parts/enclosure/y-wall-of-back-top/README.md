...
The mounting datum is [x 66.9, z 336.21, seat y 458.75](C14_SEAT), with screw axes at [x 46.9 and 86.9](C14_SCREWS). The working opening is [31.95 × 23.17 mm R3](C14_OPENING). The tunnel is one [56.77 × 35.17 mm](C14_BLOCK) block from the pocket's mouth to the wall. Cut into its fore face, the pocket follows the measured [49.77 × 22.17 mm](C14_FLANGE) rounded/tapered flange profile with [0.5 mm](C14_POCKET_SLIP) insertion slip; the block keeps at least [3 mm](C14_POCKET_WALL) of material around that profile in XZ, and the pocket's lip stands [3 mm](C14_POCKET_LIP) past the seated flange's inboard face. The block runs [10.25 mm](C14_RUN) from the mouth to the wall and its crown fuses into back-top's ceiling slab, so on that ceiling-down print it stands on the bed through the slab and the wall; its underside looks print-up, and its two floors, which look print-down, fall away at 45°: the aperture's inward from the show face's edge until it runs out of the block's underside, the pocket's toward its mouth from a crest on the seating face that the flange's bottom edge lands on. The block does not move the cutout, screws, or seating plane, and the exterior wall stays on its established planes.
