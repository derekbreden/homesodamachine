"""Narrow-Edition enclosure — a PETG box sized to the placed contents, split
into two printable halves (front + back) that telescope and cross-pin together.

Same machinery as the wide Kitchen-Edition enclosure (telescoping seam, four
corner cross-pins, the 45° display facet, the hopper opening), but wrapped
around the NARROW contents: the cold core, compressor shroud, and hopper funnel
are each rotated 90° about Z, trading X width for Y depth. The box comes out much
narrower (it follows the rotated cold core's 181 mm footprint and the
refrigeration row) and correspondingly longer front-to-back.

That longer Y forces the one substantive change from the wide build: the seam.
The wide enclosure splits just behind the display housing, leaving everything —
cold core included — in a shallow front cap's back half. Here the back half
(cold core + trays) would be far over the H2C bed if the seam stayed that far
forward, so the seam moves to the middle: just ahead of the cold core, so the
front half holds the front block (refrigeration, pumps, display, hopper) and the
back half houses the cold core. Both halves then fit the bed.

main() exports the two printable halves (narrow-enclosure-front.step,
narrow-enclosure-back.step) plus narrow-enclosure.step — the two halves as
separate solids in assembled position, seams intact.
"""
...
sys.path.insert(0, str(_repo / "hardware" / "printed-parts" / "enclosure" / "narrow-enclosure-assembly"))
...
# inside. The funnel is rotated 90° about Z here, so the opening is narrow in X
# and deep in Y (the transpose of the wide build's 150 × 80).
hopper_hole_x = 80.0    # opening width (X), nominal before the corner-pod clamp
hopper_hole_y = 150.0   # opening depth (Y), from the inner front wall back
...
# corner bosses cross-pin the seam with M3 screws from the ±X exterior. Each boss
# is a D-section pin: round where it registers in the front socket bore, with a
# flat tab running +Y to the lip rim where the back-half corner brace backs it.
boss_to_coldcore = 6.0       # clear gap from the lip's +Y tip back to the cold core
...
# The telescoping overlap is derived, not free: with y_boss = y_joint + plug_dia/2
# (plug −Y on the mouth), the pod's +Y face lands on the rim iff lip_len =
# plug_dia/2 + socket_r.
...
def _dims():
    ...
    # Narrow split: the deep Y forces a mid-box seam. It sits just ahead of the
    # cold core — front half = front block, back half = cold core — so both
    # printed halves fit the H2C bed. The lip rim lands boss_to_coldcore ahead of
    # the cold core's front face.
    y_joint = cold_front_y - lip_len - boss_to_coldcore
...
def _report_split(front, back):
    ...
    for tag, h in (("front", front), ("back", back)):
        ...
        print(f"  {tag} fits H2C bed: {fits}  ({b.xlen:.0f}×{b.ylen:.0f}×{b.zlen:.0f})")
...
def main():
    ...
    assy = cq.Assembly(name="narrow-enclosure")
    assy.add(front, name="narrow_enclosure_front", color=cq.Color(0.80, 0.84, 0.90))
    assy.add(back, name="narrow_enclosure_back", color=cq.Color(0.70, 0.74, 0.82))

    export_step(front, str(_here.parent / "narrow-enclosure-front.step"))
    export_step(back, str(_here.parent / "narrow-enclosure-back.step"))
    export_assembly(assy, str(_here.parent / "narrow-enclosure.step"))
    print("-> narrow-enclosure-front.step")
    print("-> narrow-enclosure-back.step")
    print("-> narrow-enclosure.step (assembled halves)")
    _report_facet(front)
