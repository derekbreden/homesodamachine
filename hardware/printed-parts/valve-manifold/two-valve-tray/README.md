...
- **All five are placed**, in two stands. THREE make the front column's head column,
  ...
  pair's two junction ports SIDE BY SIDE, which is the shape a trident's parallel
  outlets take — and whether the pair gets one is the room ahead of it: Y-A, Y-B and
  Y-H hang ahead of their pairs on their own port planes, and Y-E stands a Tee ACROSS
  the strip its pair leaves between the head column and the pump row.
