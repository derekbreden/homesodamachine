...
- **The cable leaves by the top of the channel it is already in.** The channel runs the shell's full height and opens at its top face, so the cable rises the way it was laced and meets the ⌀[6.8 mm](CONDUIT_BORE_D) conduit the cap carries over that mouth — [reed-cable-a (134.5, 31), reed-cable-b (-134.5, 31)](REED_CONDUIT_XY) in the shell's frame, out through the lid beside the seven fluid conduits. It crosses no pocket wall, runs no lane, and puts no bore in the −X front face, which is mated shut against the refrigeration base. Each pocket's −Y wall is opened once and for that side's draw, at x = [±97](FLAVOR_HOLE_X).
- The column is **held mechanically** by the channel — bottom shelf catches it from below, ±y side walls constrain it laterally, the cap on top traps it from above when installed. **No foam encapsulation** — the reed channel is its own cavity outboard of the pocket wall, and the pre-soldered column drops into it after the body pour has cured ([cold-core.md](/hardware/assembly/cold-core.md) "Final assembly"), so foam never reaches the reeds or their cable. The bag pocket the cable crosses on its way out is an air cavity: the reservoir fills it to [0.25 mm](RESERVOIR_CLEARANCE) on every side and at the top, leaving the pour no way in.
...
The dominant term in the magnet-to-reed path is **how far the donut floats off the cavity far wall**, not the wall thickness. The fixed part of the path is the reservoir wall (3 mm) + reservoir/pocket clearance (0.5 mm) + ~half a reed body — together ~5 mm. The variable part is the donut-to-wall gap, set by `ROD_POSITION_X` against the measured 27.75 mm donut and its off-center seating on the 1/8" rod (the rod's bore clearance lets the donut settle toward the wall). `ROD_POSITION_X` is not a station of its own: it is struck off the cavity far wall at x = [126.2](CAVITY_FAR_WALL_X), parked [2.5 mm](MAGNET_WALL_BIAS) PAST where a concentric donut would touch it, so the wall pushes back and holds the magnet against itself for the whole travel. The donut's own bore is loose on the rod — it was made for the donor's stem — and that slop is what the bias spends. At `ROD_POSITION_X = [114.8](ROD_POSITION_X)` the donut rides the wall with under a millimetre of retreat anywhere in the travel, which is the comfortable-margin end of the table below. `cold-core-layout`'s `floats-couple` measures that retreat against the ~2 mm reach below and reads red if it opens up.
...
- **Reed-column wire** — one **BNTECHGO 22 AWG black silicone 5P ribbon** per reservoir, which is the column's conductor count exactly: [4](REEDS_PER_RES) reed signals and one common return ([bom.md](/hardware/ledger/bom.md) §9). One ribbon runs from the board to the top of the reed channel and gives up a conductor at each reed height, so the column is soldered along one cable rather than laced from five. Reservoir A's is the whole J6 loom; reservoir B's is the 5P half of the seven-conductor J7 loom, whose 3P half takes the two carbonator reeds ([`/hardware/assembly/cable-assemblies.md`](/hardware/assembly/cable-assemblies.md) § Ribbon pairs). Silicone, not a jacket — it stays flexible and survives thermal cycling in the cold core, where a PVC-jacketed multiconductor would stiffen. The jacketed cable in the build is only what leaves the cabinet or the shroud.
