...
- The cable exits through a **dedicated ⌀6.5 cable hole** driven in −Y through both the pocket's −Y wall and the outer shell's −Y wall — the same front wall the shared copper/water slot pierces — at `(x = [±109](CABLE_HOLE_X), y = [-62.5](WALL_HOLE_Y), z = [6](WALL_HOLE_Z))`. It shares the y and z of the flavor-line pass-through (the bulkhead's 1/4" tube hole, which sits at the same y and z). The two holes flank the bulkhead axis in ±X — flavor line inboard at x = [±97](FLAVOR_HOLE_X), cable hole outboard at x = [±109](CABLE_HOLE_X) — so the two ⌀6.5 holes keep clear PETG between them. The bag-pocket bottom is open, so the cable runs free from the bottom of the reed channel through the pocket interior to this hole — no dedicated cable channel.
