...
- The **foam shell carries the channel that holds the column** — no separate printed reed-holder part. The channel is a **3-walled box that extrudes OUTWARD** from the bag_pocket_shell's far ±X wall into the outer foam zone. The original wall is the channel's back / inner face; two new ±z side walls extend outward; a new outer face closes the box on the foam-zone side. The channel has two segments:
  ...
  - A **horizontal segment at low y** (straddling `reservoir_bulkhead_port_y` = 18, matching the bulkhead's y so the cable level lines up with the existing bulkhead pass-through). Runs from the vertical channel in +Z direction along the bag-pocket far ±X wall to a **dedicated ⌀6.5 coaxial cable hole** through both the bag-pocket far ±X wall and the outer ±X shell wall, at (y = port_position_y, z = bag_pocket_width/2 − 10) — same y, z as the existing bulkhead tube pass-through, just on the ±X side and a few mm offset from the bulkhead's +Z hole.
...
- **Foam-shell channel CAD.** Implemented in `_foam_shell_geometry.py` `build_reed_channels()` and `cut_reed_cable_holes()`. Vertical reed channel at z=−45, 6 mm × 8 mm cavity, full height, top open. Horizontal cable channel at low y (cavity y range 14..22, straddling `reservoir_bulkhead_port_y` = 18), running in +Z to z = 63, 5 mm × 8 mm cavity. Coaxial ⌀6.5 cable hole through both the bag-pocket far ±X wall and the outer ±X shell wall at (y=18, z=62.5). Cable's path: reed column → bottom of vertical channel → horizontal channel → coaxial cable hole → outside cold core. Refine the channel cross-section (currently 6mm vertical × 8mm horizontal cavity widths) once the cable's actual OD is characterized.
