...
- **The cable leaves by the top of the channel it is already in.** The channel runs the shell's full height and opens at its top face, so the cable rises the way it was laced and meets the ⌀6.5 conduit the cap carries over that mouth — [reed-cable-a (134.5, 31), reed-cable-b (-134.5, 31)](REED_CONDUIT_XY) in the shell's frame, out through the lid beside the seven fluid conduits. It crosses no pocket wall, runs no lane, and puts no bore in the −X front face, which is mated shut against the refrigeration base. Each pocket's −Y wall is opened once and for that side's draw, at x = [±97](FLAVOR_HOLE_X).
