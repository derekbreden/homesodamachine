...
that line's own CORRIDOR: the run swept at the project's ⌀[6.8](PORT_HOLE_DIAMETER)
...
# takes a slot. Two lines on one crossing would be two tubes in one ⌀[6.8](PORT_HOLE_DIAMETER) hole.
...
# go — so this crossing costs no bore, notches no bearing segment, and the ⌀[6.8](PORT_HOLE_DIAMETER)
...
# CO2 inlet — the ⌀[6.8](PORT_HOLE_DIAMETER) reach in to the bottom plate's own lane-side
...
def cut_line_corridors(foam_shell, gives_way):
    ...
    crosses on the same tight fit either way, the shell's ⌀[6.8](PORT_HOLE_DIAMETER) round a
...
def cut_prv_vent_port(foam_shell):
    ...
    ⌀[6.8](PORT_HOLE_DIAMETER) corridor, the same tight fit round the tube that keeps the pour
