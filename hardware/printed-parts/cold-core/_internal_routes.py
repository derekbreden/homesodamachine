...
# by the SHELL's top and not by the conduit's mouth, because what is over the shell is a ⌀[6.8](PORT_HOLE_DIAMETER)
...
def route_corridor(name, radius=port_hole_radius):
    """One line's OPENING — the same run swept at the ⌀[6.8](PORT_HOLE_DIAMETER) shell
    ...
    ⌀[6.8](PORT_HOLE_DIAMETER)-around-⌀[6.35](LLDPE_TUBE_OD) tight fit that keeps the body pour out of a
