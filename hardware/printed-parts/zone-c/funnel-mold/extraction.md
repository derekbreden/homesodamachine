...
[Extraction section checks](extraction-loads.json), generated by
`check_extraction_structure.py`, verify continuous solid stock in all four
bearing columns, lifting-arm roots and guide blades. The calculations screen
the load path in both axial extraction and incidental sideways handling.

A 100 N sideways force at the full 54 mm free blade tip gives nominal root
bending stresses of **7.16 MPa radially** and **1.99 MPa tangentially**. This
uses a 10.8 × 38.8 mm inscribed rectangle, excluding the chamfers and travel
mark, and gives no stiffness credit to the flared root or the guide sleeve.
With an assumed 1,000 MPa elastic modulus, the corresponding calculated tip
deflections are 1.29 mm and 0.10 mm. These are sensitivity calculations, not
measured print properties or allowable loads.

For scale, a hypothetical perfect vacuum acting over the entire 147 × 147 mm
core mouth would resist extraction by about **2.19 kN** at one atmosphere.
Divided equally among four screws, the section model gives 8.12 MPa nominal
bending at an arm root and 2.04 MPa maximum normal stress in a bearing column.
The JSON also records the nut-bearing and roof checks, and a 1,000 N-per-jack
case. Actual adhesion, leakage and unequal jack forces are not established by
that calculation. Backing-air channels vent structural spaces; they do not
guarantee pressure equalization at an adhered silicone interface.
...
checks; no rated lifting load is assigned to the printed parts.
