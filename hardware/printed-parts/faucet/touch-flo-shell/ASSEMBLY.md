...
**Under-counter plate — SendCutSend 0.060" 304 SS, D-shaped half (order
qty 2).** File `touch_flo_under_counter_plate.dxf` (generated by
`generate_dxf.py` in the same directory) contains one D-shaped half;
order quantity 2 per appliance. The two identical pieces mate along
their straight edges to form a complete plate. Each half is a
semicircle of the Ø 54.35 mm disc, with a half-shank-hole indent and
a half-pill-slot indent on its mating edge — the indents form the
full shank hole and pill slot when the two halves are mated.
