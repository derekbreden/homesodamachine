...
| DC-5 | PUMPS J13 (`AM2` / `AM1` → pump A, `BM2` / `BM1` → pump B) | Fixed black Molex `43020-0400` pump connector in the centred ridge-wall station; mating `43025-0400` receptacle on the Kamoer pump cartridge | 2 motor pairs, 4 circuits | [22](AWG_SIG) | [~350 mm](LEN_PUMP_FIXED) fixed J13 half + [~400 mm](LEN_CARTRIDGE) cartridge half | J13 XH → male `43031-0001` contacts in the fixed housing; female `43030-0001` contacts in the removable housing → four female Fastons kept on the pump tabs | Each DRV8870 H-bridge drives one motor differentially, and each Micro-Fit circuit carries one motor conductor at the pump's published ~[0.8 A](PUMP_PEAK_A), never both pumps. Molex's conservative fully-loaded six-circuit wire-to-wire figure is 4 A/contact at 22 AWG, covering the driver's 3.6 A peak capability. On the bench, viewed into the fixed mating face with the key up: circuit `1 AM2`, `2 AM1`, `3 BM2`, `4 BM1`; installed, the key and mating thumb latch face down into the empty pump bay. The fixed half returns toward the main board through the +X ridge-wall cable clip. On the removable half, the two peeled motor pairs are retained in the cartridge's fully embedded pump-well channels; the unpeeled connector-to-channel span remains the loop which follows the cartridge out. With power removed, draw the cartridge, press the connector's thumb latch, pull the housing straight forward until clear, then lower it through the bay; the four Fastons stay on the pump tabs through routine cartridge service. Both cable halves are direct conductors with no splice. |
