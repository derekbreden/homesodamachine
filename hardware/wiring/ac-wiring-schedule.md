...
The Teyleten relay #1 sits with the rest of the electronics, on the shelf and **away from the refrigeration compartment**. Rationale: keep an arcing contact out of the volume a hydrocarbon leak pools in, and keep the mains reaching the compressor down to [3](SHROUD_WIRES) conductors — switched H + N + G — rather than the [5](SHROUD_WIRES_ALT) a relay at the compressor would need. See [`/hardware/future.md`](/hardware/future.md) "Safety".
...
| AC-4 | Teyleten relay #1 contact output ("normally open") | Compressor terminal block | H_switched | [18](AWG_SHROUD) | [~400 mm](LEN_COMPRESSOR) | Crimp fork to relay; female disconnect to compressor terminal | Switched hot. One of the three conductors of the [18 AWG](AWG_SHROUD_U) SJOOW jacketed lead that runs unbroken from the shelf to the compressor — no penetration clamps it, so the jacket is the strain relief. Length includes service slack. |
| AC-5 | AC distribution block (N) | Compressor terminal block | N | [18](AWG_SHROUD) | [~400 mm](LEN_COMPRESSOR) | Crimp ferrule at distribution block; female disconnect to compressor terminal | The SJOOW's second conductor, in the same jacket as AC-4. |
| AC-6 | Earth bus on chassis ground point | Compressor body, under a floor screw | G | [18](AWG_SHROUD) | [~400 mm](LEN_COMPRESSOR) | Ring terminal both ends | The SJOOW's third conductor, in the same jacket as AC-4 / AC-5. Bonds the compressor body to chassis ground — the machine's only bond to the refrigeration metalwork. The ring goes under the screw head on the grommet's steel bushing, never into the rubber. |
