...
| DC-8 | L298N Board A onboard [5 V](V_LOGIC) reg (7805/78M05) output | ESP32-DevKitC 5 V/VIN pin (and on to the S3 via its cord) | + + GND | [22](AWG_SIG) | [~50 mm](LEN_SHORT) | Pin header / screw terminal | [5 V](V_LOGIC) for the MCUs is made on the L298N from its [12 V](V_DC) VS feed (DC-4). Same rail tees to relay VCC (LV-3). Confirm thermal margin at bring-up ([`firmware-and-commissioning.md`](/hardware/assembly/firmware-and-commissioning.md) step 2). |
...
| SIG-6 | ESP32-S3-Touch-LCD-1.47 faucet flavor display on the gooseneck head | ESP32 GPIO 32 (TX) + GPIO 35 (RX) + display power + GND, up the umbilical | UART + power | [24](AWG_LV) | [~1 m](LEN_UMBILICAL) (up the umbilical) | Direct TTL UART to the 1.47" display's ESP32-S3, which breaks out TTL UART (no transceiver — RS485 is reserved for the 4.3B, which has no free TTL UART). A conservative baud over a twisted pair shares the umbilical with SIG-4; the display takes [5 V](V_LOGIC) up the umbilical. |
