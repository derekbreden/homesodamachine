...
The appliance has two functional electrical zones, both on the AC inlet side. The cold core occupies the rear of the enclosure (insulated, no electronics). The compressor + condenser sit middle-bottom. Everything else lives on **one electronics shelf lying on the cold core's foam-cap top, on the rear-wall C14 inlet's column**.

| Zone | Location | Contents |
|---|---|---|
| Electronics shelf | On the foam-cap top in the band above the cold core, ahead of the rear-wall port bodies | C14 inlet, AC distribution block, Mean Well IRM-90-12ST PSU, Teyleten relay #1 (compressor switch), Teyleten relay #2 (diaphragm pump switch), [12 V](V_DC) distribution block, controller PCBA, ground bus |
| Compressor zone | Front of the enclosure floor | Hermetic compressor bolted down on four floor posts, its clip-on PTC start relay/overload module on its own terminal block, condenser fan beside it. **Nothing encloses the terminal block** — see [`/business/regulatory.md`](/business/regulatory.md) for the 60335-2-24 fire-enclosure gap |
...
The Teyleten relay #1 sits with the rest of the electronics, on the shelf and **away from the refrigeration compartment**. Rationale: keep an arcing contact out of the volume a hydrocarbon leak pools in, and keep the mains reaching the compressor down to [3](COMP_WIRES) conductors — switched H + N + G — rather than the [5](COMP_WIRES_ALT) a relay at the compressor would need. See [`/hardware/future.md`](/hardware/future.md) "Safety".
...
| J8 | I2C | `GND`, `3V3`, `SDA`, `SCL` | I²C expansion header; the bus's star point on the board, no loom |
...
|---|---|---|---|---|---|---|---|
| AC-1 | C14 inlet (rear wall) | AC distribution block on electronics shelf | H + N + G | [16](AWG_MAINS) | [~150 mm](LEN_MID) (the C14 inlet's cordage drops the rear wall and runs forward over the foam-cap top to the shelf) | Solder to the C14 inlet's tabs; crimp ferrules into Wago 221 lever block | Inlet ships with solder-tab pins; [16 AWG](AWG_MAINS_U) appliance wire drops from the inlet to the distribution block on the shelf below. The C14 inlet lands directly on the AC distribution block with no device in series; all downstream loads (PSU, compressor, etc.) tap the block. Ground-fault protection is deferred — see [`/pie-in-the-sky/gfci.md`](/pie-in-the-sky/gfci.md). |
...
| AC-4 | Teyleten relay #1 contact output ("normally open") | Compressor terminal block | H_switched | [18](AWG_COMP_LEAD) | [~400 mm](LEN_COMPRESSOR) | Crimp fork to relay; female disconnect to compressor terminal | Switched hot. One of the three conductors of the [18 AWG](AWG_COMP_LEAD_U) SJOOW jacketed lead that runs unbroken from the shelf to the compressor — no penetration clamps it, so the jacket is the strain relief. Length includes service slack. |
| AC-5 | AC distribution block (N) | Compressor terminal block | N | [18](AWG_COMP_LEAD) | [~400 mm](LEN_COMPRESSOR) | Crimp ferrule at distribution block; female disconnect to compressor terminal | The SJOOW's second conductor, in the same jacket as AC-4. |
| AC-6 | Earth bus on chassis ground point | Compressor body, at its terminal-box earth screw | G | [18](AWG_COMP_LEAD) | [~400 mm](LEN_COMPRESSOR) | Ring terminal both ends | The SJOOW's third conductor, in the same jacket as AC-4 / AC-5. Bonds the compressor body to chassis ground — the machine's only bond to the refrigeration metalwork. The ring goes under the screw head on the grommet's steel bushing, never into the rubber. |
...
| DC-4 | [12 V](V_DC) distribution block | Board inlet J10 `V12` + `GND` | + + GND | [16](AWG_MAINS) | [~150 mm](LEN_MID) | Ferrules under the J10 screw clamps | Everything the board feeds — [10](SOLENOID_COUNT) manifold valves + the V-K supply valve, both peristaltic pumps, the condenser fan, display [12 V](V_DC), and both logic rails — draws through this run: ~[3.33 A](BOARD_PEAK_A) peak (both pumps priming + [3](MAX_VALVES) valves + the fan), against a 17 A-rated block. The SeaFlo's [5 A](DIAPHRAGM_A) on DC-3 parallels it off the same supply — [8.32 A](COINCIDENT_A) against [6.7 A](PSU_MAX_A) if a refill overlaps a dispense, which is why relay #2 is gated on the dispense window ([`power.mmd`](/hardware/wiring/power.mmd)). `V12` seats on the east pad, `GND` west; reversing them cooks the polarized bulk cap, the buck, and the drivers. |
| DC-5 | PUMPS J13 (`AM1` / `AM2` → pump A, `BM1` / `BM2` → pump B) | Kamoer pump A / pump B, in their trays on `enclosure-pump-cartridge` | 2 motor pairs | [22](AWG_SIG) | [~400 mm](LEN_CARTRIDGE) to the pumps + [~100 mm](LEN_SHORT_2) of pump-lead pigtail | XH housing at J13; quick-disconnect spades on the pump leads (BOM §11) | DRV8870 H-bridge outputs, ~[0.8 A](PUMP_PEAK_A) peak per pump. The cartridge carries both pumps out through the front-wall bay on service, so DC-5 is the only run whose device leaves the box. Its two spade pairs are the last connection off and the first on — pulled off the motor tabs with the cartridge already drawn clear, and made off again before the deck goes back in, because the tabs are unreachable once it is home ([`/hardware/service/pump-replacement.md`](/hardware/service/pump-replacement.md)). Each lead reaches its motor with the cartridge drawn fully out of the bay, since the pairs stay made off across the whole extraction stroke and the harness is never what stops it; nothing between J13 and the tabs clamps it. |
...
| DC-7 | MANIFOLD B J2 (`COM` + `OUT1`–`OUT2`) | 2 Beduan solenoid coils on manifold B | 4 of the 6-way J2 trunk populated | [22](AWG_SIG) | [~300 mm](LEN_MANIFOLD) to the manifold, then [~150 mm](LEN_MID) fan-out per valve | XH housing at J2; `COM` into a Wago 221-415 at the manifold; female disconnects per valve | Same pattern as DC-6. The trunk populates `COM` + `OUT1` + `OUT2` + `FAN` (the fan sink is DC-8) + `OUT3` (V-K's − on run DC-9); `OUT4` is wired on the board but carries no valve — a spare channel, left unpopulated in the harness. |
...
| SIG-4 | SENSORS J4 `IO25` + `V5` + `GND` | DIGITEN flow meter (inline on the carbonated-water riser, lying in the strip ahead of the cold core's front face, boss up) | pulse + [5 V](V_LOGIC) + GND | [22](AWG_LV) | [~150 mm](LEN_MID) (internal, within the electronics-shelf zone) | Pulse interrupt; flow detection is internal — does not leave the enclosure. |
...
The peristaltic pumps have no such nut: DC-5 is two DRV8870 H-bridge pairs, each motor driven differentially on its own two conductors, so there is no rail to share and nothing to fan out. The four run unbroken from the J13 housing to the four motor tabs, and those four spade joints are the whole of what parts electrically for the pump cartridge to come out of its bay.
