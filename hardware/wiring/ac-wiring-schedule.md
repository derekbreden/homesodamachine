...
| SIG-2 | Reed switch — low (carbonator) | MCP23017 0x21 PB0 + GND | switch + GND | [24](AWG_LV) | [~600 mm](LEN_COLD_CORE) | INPUT_PULLUP (MCP internal). |
| SIG-3 | Reed switch — high (carbonator) | MCP23017 0x21 PB1 + GND | switch + GND | [24](AWG_LV) | [~600 mm](LEN_COLD_CORE) | INPUT_PULLUP (MCP internal). |
| SIG-4 | DIGITEN flow meter (Zone B, on the carbonated-water line where it exits the cold core near the shelf) | ESP32 GPIO 15 + [5 V](V_LOGIC) + GND | pulse + V + GND | [24](AWG_LV) | [~150 mm](LEN_MID) (internal, within the electronics-shelf zone) | Pulse interrupt; flow detection is internal — does not leave the enclosure. |
