...
| SIG-9 | SENSORS J4 `IO23` + `IO27` + `GND` | Backflow vent moisture sensor | switched VCC + DO + GND | [22](AWG_LV) | [~350 mm](LEN_MOISTURE) to the drip pan sleeve's dock, under the −X wall | Per [`/hardware/README.md`](/hardware/README.md) "Safety". Lands on the two leads of the dock's male half in the sleeve's backstop; the plate's own leads stay in the pan and the pan's travel mates the pair. VCC is GPIO-sourced (`IO23`), driven only while sampling — the wet electrodes sit unpowered between samples. |
