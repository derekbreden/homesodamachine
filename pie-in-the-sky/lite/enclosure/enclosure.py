"""Lite Edition enclosure — a transparent PETG box sized to the placed contents,
split into two printable halves (front + back) that telescope and cross-pin
together.

The same architecture as the Kitchen edition, adapted to the Lite contents (no
cold core, compressor, or condenser — the reservoir-pockets box is the heavy
back-bottom anchor instead). Dimensions follow the contents at build time: the
bounding box of the parts placed by `../enclosure-assembly/_contents.py` is
computed live, padded by an interior clearance, then walled out. Features:

  * A flat 45° display-mounting facet (a solid surface) chamfered into the
    top-front-left corner, flush to the −X edge — the same Waveshare
    ESP32-S3-Touch-LCD-4.3B config display as the Kitchen edition.
  * A front↔back split: the front half's rear wall telescopes (a full-wall
    ...
    part of the top/bottom (±Z) wall. The BACK half carries the PLUG (a
    cylinder reaching inward from the corner with a screw clearance through it);
    the FRONT half's lip carries the SOCKET (a pod bored to receive the plug,
    open on its +Y face so the plug drops in as the halves close, with a ruthex
    M3 heat-set at the deep end). An M3 SHCS from the ±X exterior passes through
    the plug into the heat-set, cross-pinning the two halves. The back half is
    sized so the reservoir seats behind the bosses, clear.
  * A hopper opening punched through the top wall, right of the display housing
    and flush to the front, where the removable funnel drops in.
...
# display (../../../../hardware/reference/waveshare-43b-display/), facing
# up-and-forward (−Y front / +Z up) toward the standing user. The glass is the
# datum — centered on the facet, which is the glass + a 3 mm buffer all around:
...
# The screw spans the head seat to the front heat-set, so the pin body is
# screw_len − heatset_depth long.
boss_to_reservoir = 14.0     # clear gap from the lip's +Y tip back to the reservoir
...
def _report_split(front, back):
    ...
    yb = _y_boss(y_joint)
    res = _contents.build()["reservoir-pockets"][0]
    clash = sum(
        res.intersect(
    ...
    print(f"  reservoir vs bosses: {clash:.1f} mm³ overlap  ({'CLEAR' if clash < 1 else 'CLASH'})")
...
def main():
    ...
    variables = {
        ...
        "LITE_OUTER_X": f"{ox1 - ox0:.4g} mm",
        "LITE_OUTER_Y": f"{oy1 - oy0:.4g} mm",
        "LITE_OUTER_Z": f"{oz1 - oz0:.4g} mm",
    }
