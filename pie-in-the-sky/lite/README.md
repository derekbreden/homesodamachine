...
- **A valve manifold** with the same source-selection / output-routing pattern as the main appliance. Hopper input, bag output to faucet, clean-cycle paths. Lite divergences — clean water comes from the Lillium, and the runs the user sees through the enclosure go clear — in [`fluid-topology-manifold.mmd`](/pie-in-the-sky/lite/fluid-topology-manifold.mmd).
...
- **Power tray — designed.** [`electronics/power-tray/`](/pie-in-the-sky/lite/printed-parts/electronics/power-tray/) — the Kitchen power tray minus the Teyleten relay (the Lite has no compressor/SeaFlo): Mean Well PSU + Wago AC distribution + ground stack on heat-set bosses. It stands vertical as a side-wall panel in the front-left corner. The logic/driver boards (ESP32, MCP23017, ULN2803A, L298N) still want their own tray — not yet drawn.
