...
The current Kitchen build does not integrate the module: the C14 inlet lands onto the three mains splices in the +X wall's own Wago wells with no device in series (`enclosure._side_wells`). This doc holds the integrated GFCI as a live want for a later pass.
