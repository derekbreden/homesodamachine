...
def main() -> int:
    ...
    if args.write:
        ...
        if q.returncode != 0:
            ...
            # THE BUILD STAMP IS NEVER UP TO DATE AND CARRIES NOTHING. Bazel re-runs
            # `BazelWorkspaceStatusAction` on every invocation — that is how a build gets stamped —
            # so it answers "not up-to-date" to a question about outputs it is not one of.
            # `stable-status.txt` is named nowhere in BUILD.bazel, in graph.json, or in the carry
            # table below. Every other line here is an output whose inputs moved.
            owed = [ln for ln in owed if "BazelWorkspaceStatusAction" not in ln]
        if q.returncode != 0 and owed:
