...
Outputs: tools/anim_frames/frame_XX.svg, tools/anim_frames/preview.gif, and the
RGB565 headers the enclosure display compiles in, firmware/src_front/images/anim_NN.h
"""
...
HEADER_DIR = os.path.join(
    os.path.dirname(__file__), "..", "firmware", "src_front", "images"
)
SVG_SIZE = 1024  # original SVG viewBox
RENDER_SIZE = 360  # the enclosure display's frame edge — the SVG renders straight to it
...
def render_frames():
    """Render the NUM_FRAMES SVGs to RENDER_SIZE square RGB images composited onto
...
def write_headers(frames):
    """Write anim_NN.h RGB565 PROGMEM headers into the enclosure display's tree."""
