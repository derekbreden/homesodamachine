...
para('A <b>1-3/8 in counter hole</b>, cold water and grounded <b>120 V.</b> A filled <b>5 lb CO2 cylinder</b> with the supplied regulator and CO2 tether fitted and leak-checked. Two <b>14.8 fl oz</b> bottles of SodaStream-compatible concentrate.',800,33,532,12,15,limit=60)
...
text('1 Lower',x+2,218,9.5,'Semibold')
arrow(*p(382,866),*p(585,928),head=6)
text('2 Back',x+75,252,9.5,'Semibold')
...
p=pic('mount-under-tighten-clean.png',x+324,197,90,83,crop=(670,260,900,503))
turn([p(739,375),p(718,348),p(843,340),p(829,374)],head=5)
text('Turn nut',x+339,181,9.5,'Semibold')
...
text('Closed',x+49,282,8.5,'Semibold',MUTED)
para('<b>Close the cold-water shutoff.</b> Run the cold tap until it stops. Put a cup and towel under the fitting.',x+136,230,280,12.1,15,limit=60)
...
para('<b>Push the collet press against the release ring and hold.</b> With your other hand, <b>pull the white tube out.</b>',x+136,300,280,12.1,15,limit=60)
...
pic('cylinder-actions/co2-connected.png',x+2,522,167,108,crop=(80,0,1600,1500))
...
para('<b>Have the cylinder shop fit and leak-check</b> the regulator and gray tether connector. Keep the two gray fittings assembled.',x,644,416,12.6,16,limit=48)
...
text('Open',x+113,628,8.5,'Semibold',MUTED)
para('Open the water shutoff <b>slowly.</b> Watch every water joint for a <b>full minute.</b>',x,644,268,12.4,16,limit=64)
x=starts[1]
gas_pose=((.08,1,.12),(-34.7549267293,3.0634158833,-30.1918478747),129.375,(1800,1500))
p=pic('cylinder-actions/startup-gas.png',x+4,544,139,86,crop=(173,53,1752,1360))
pic('cylinder-actions/startup-gas.png',x+216,547,45,45,crop=(533,48,873,388))
text('Upper',x+218,598,9.5,'Semibold')
text('gauge',x+218,610,9.5,'Semibold')
leader('Cylinder valve',x+143,550,projected(p,(-115,0,33),*gas_pose),side='left')
leader('Big knob',x+143,590,projected(p,(0,45,0),*gas_pose),side='left')
leader('Small knob',x+143,615,projected(p,(0,27,-47),*gas_pose),side='left')
para('Open the <b>cylinder valve</b> and <b>small knob.</b> Use the <b>big knob</b> to set the upper needle in <b>green.</b> Check the gray connector and red CO2 port for leaks <b>before power.</b>',x,644,268,12.4,16,limit=64)
x=starts[2]
p=pic('insertion-actions/power-ready.png',x+5,545,258,84,crop=(0,160,1800,1140))
...
p=pic('insertion-actions/fill-ready.png',355,810,277,148,crop=(290,190,1550,1500))
