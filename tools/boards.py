...
on the controller PCBA drives the Q2/Q3 auto-reset lattice and reboots the board;
a tool you run to find out what is plugged in should not restart what is plugged in.
...
CH340    = (0x1A86, 0x7523)   # the PCBA's on-board CH340C at J14
...
S3_ENVS = [
    ("esp32s3_front",  "4.3B front-face display"),
    ("esp32s3_faucet", '1.47" faucet display'),
    ("esp32s3_config", '1.28" round config display'),
]
...
def main():
    ...
    if pcba:
        for p in pcba:
            print(f"  controller PCBA   {p.device}   (CH340C at J14)")
    else:
        print("  controller PCBA   NOT FOUND")
    ...
    if not pcba and not s3:
        print("  Nothing here. The PCBA needs 12 V at J10 before its CH340C enumerates —")
        print("  USB VBUS powers nothing on that board.")
        print()

    if pcba:
        print("  Flash the controller:")
