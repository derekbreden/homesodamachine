...
<script>
// Beacon-style logger: POSTs {event, data} to /api/log so we can trace
// notification flows on iOS PWA without DevTools. data must be JSON-able.
window.cliLog=function(ev,data){try{fetch("/api/log",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:ev,data:data||{}}),keepalive:true}).catch(function(){});}catch(e){}};
...
// Cold-launch redirect bridge. iOS PWA notification taps cold-launch the
// PWA at start_url ("/"), bypassing both clients[i].navigate() and any SW
// fetch-handler interception (start_url cold-launch fetches don't always
// hit the SW). The SW push and notificationclick handlers persist the
// target URL to Cache Storage; this script reads from cache on every page
// load and redirects if a recent entry exists. Polls briefly because the
// push handler may write to cache after the page begins loading.
(function(){if(!("caches" in window))return;var done=false;function check(retries){if(done)return;caches.open("hsm-pending-nav").then(function(c){return c.match("/__pending").then(function(r){if(!r)return false;return r.text().then(function(t){var target=null;try{var p=JSON.parse(t);if(p&&p.ts&&Date.now()-p.ts<60000)target=p.url;}catch(e){target=t;}return c.delete("/__pending").then(function(){if(target){var u;try{u=new URL(target,window.location.origin);}catch(e){return false;}if(u.href!==window.location.href){done=true;window.location.replace(u.href);return true;}}return false;});});});}).then(function(found){if(!found&&retries>0)setTimeout(function(){check(retries-1);},150);}).catch(function(){if(retries>0)setTimeout(function(){check(retries-1);},150);});}check(8);})();
// Server-backed pending-nav redirect. iOS PWA appears to skip SW push and
// notificationclick events entirely (page-side cache poll above never
// fires), so the browser-side bridges are useless when the PWA is closed.
// This path is independent: server records (token → url) when the FCM send
// succeeds, PWA fetches it on home-page load using the FCM token stored in
// localStorage at subscribe-time, and redirects if a recent row exists.
// Only runs on "/" so navigating around the app afterwards doesn't fire
// stale redirects.
(function(){if(window.location.pathname!=="/"){cliLog("pending-nav-skip",{reason:"not-root"});return;}var t;try{t=localStorage.getItem("hsmFcmToken");}catch(e){cliLog("pending-nav-skip",{reason:"localStorage-throw"});return;}if(!t){cliLog("pending-nav-skip",{reason:"no-token"});return;}cliLog("pending-nav-fetch",{tokenPrefix:t.slice(0,16)});fetch("/api/pending-nav?token="+encodeURIComponent(t)).then(function(r){return r.ok?r.json():null;}).then(function(d){cliLog("pending-nav-resp",{hasUrl:!!(d&&d.url),url:d&&d.url});if(!d||!d.url)return;try{var u=new URL(d.url,window.location.origin);if(u.href!==window.location.href){cliLog("pending-nav-redirect",{from:window.location.href,to:u.href});window.location.replace(u.href);}else{cliLog("pending-nav-noop",{reason:"same-href"});}}catch(e){cliLog("pending-nav-error",{err:String(e)});}}).catch(function(e){cliLog("pending-nav-fetch-err",{err:String(e)});});})();
</script>
<meta charset="utf-8">
