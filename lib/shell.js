...
// hook.
//
// The server keeps the most recent push per token (no single-use, 7-day
// TTL) so a banner tapped hours later still gets the user to the right
// place. iOS doesn't tell us *which* notification was tapped, so "most
// recent" is the heuristic — almost always what the user means by "the
// notification I just tapped".
//
// Idempotency: each push gets a new ts server-side. The client tracks
// lastTs in localStorage and skips rows whose ts is <= lastTs (already
// acted on). Without this, every focus event after a redirect would
// keep re-redirecting on every subsequent focus.
//
// On the cold-launch path, iOS opens the PWA at fcmOptions.link
// directly, so the page already lands on the target — we still update
// lastTs to mark the row consumed.
(function(){
  function check(){
    ...
    fetch("/api/pending-nav?token="+encodeURIComponent(t))
      ...
      .then(function(d){
        if(!d||!d.url||!d.ts)return;
        var lastTs=0;
        try{lastTs=parseInt(localStorage.getItem("hsmLastPendingNavTs")||"0",10)||0;}catch(e){}
        if(d.ts<=lastTs)return;
        try{localStorage.setItem("hsmLastPendingNavTs",String(d.ts));}catch(e){}
        try{
...
(function(){
  ...
  // Posts toast wording mirrors notifyPostsChanged in lib/push.js — same
  // strings here as the FCM banner so the user sees one consistent
  // identity regardless of which surface the notification reached them
  // on. Single post: title is the post's frontmatter title (already
  // resolved server-side and shipped in the SSE payload), body is a
  // generic "New entry on the blog". Batch: title is the count, body
  // joins the first few titles.
  function describePosts(posts){
    if(posts.length===1){
      return{title:posts[0].title||"New blog post",body:"New entry on the blog"};
    }
    var head=posts.slice(0,3).map(function(p){return p.title;}).join(", ");
    var body=posts.length>3?head+", …":head;
    return{title:posts.length+" new updates",body:body};
  }
  ...
  function dispatchPostsChanged(posts){
    if(!posts||!posts.length)return;
    window.dispatchEvent(new CustomEvent("hsm:posts-changed",{detail:{posts:posts}}));
    // Always toast for posts — there's no /blog in-place refresh path
    // (no per-post page, /blog renders server-side as a list). Tap takes
    // the user to the post anchor.
    var d=describePosts(posts);
    showToast(d.title,d.body,posts[0].link);
  }
  ...
  es.addEventListener("message",function(ev){
    ...
    if(msg.type==="hello"){
      ...
      if(msg.recent&&msg.recent.commit&&msg.recent.commit!==seenRecentCommit){
        ...
        if(msg.recent.files)dispatchFilesChanged(msg.recent.files);
        if(msg.recent.posts)dispatchPostsChanged(msg.recent.posts);
      }
      return;
    }
    ...
    if(msg.type==="posts-changed"){
      dispatchPostsChanged(msg.posts||[]);
    }
  });
})();
</script>
