...
// cliLog uses navigator.sendBeacon (more reliable than fetch+keepalive on
// iOS Safari, which silently drops keepalive beacons mid-page-unload).
// Falls back to fetch+keepalive only if sendBeacon is unavailable.
window.cliLog=function(ev,data){try{var body=JSON.stringify({event:ev,data:data||{},build:window.__BUILD__});if(navigator.sendBeacon){var blob=new Blob([body],{type:"application/json"});navigator.sendBeacon("/api/log",blob);}else{fetch("/api/log",{method:"POST",headers:{"Content-Type":"application/json"},body:body,keepalive:true}).catch(function(){});}}catch(e){}};
cliLog("page-load",{path:window.location.pathname,search:window.location.search,standalone:!!(window.matchMedia&&window.matchMedia("(display-mode: standalone)").matches),visibility:document.visibilityState,hasFocus:document.hasFocus(),referrer:document.referrer||null});
// Comprehensive lifecycle/visibility/focus instrumentation. Goal: see every
// event iOS PWA fires (or doesn't) when a notification is tapped, so we
// know which hook to attach the redirect to. Each beacon includes the
// document.visibilityState at fire-time for context.
(function(){
  function snap(){return{vis:document.visibilityState,focus:document.hasFocus()};}
  document.addEventListener("DOMContentLoaded",function(){cliLog("dom-content-loaded",snap());});
  window.addEventListener("load",function(){cliLog("window-load",snap());});
  window.addEventListener("beforeunload",function(){cliLog("beforeunload",snap());});
  window.addEventListener("pagehide",function(e){cliLog("pagehide",{persisted:e.persisted,vis:document.visibilityState});});
  window.addEventListener("pageshow",function(e){cliLog("pageshow",{persisted:e.persisted,vis:document.visibilityState});});
  document.addEventListener("visibilitychange",function(){cliLog("visibilitychange",{state:document.visibilityState,hidden:document.hidden});});
  window.addEventListener("focus",function(){cliLog("window-focus",snap());});
  window.addEventListener("blur",function(){cliLog("window-blur",snap());});
  // Page Lifecycle API (Chrome supports; Safari may or may not).
  document.addEventListener("freeze",function(){cliLog("doc-freeze",{});});
  document.addEventListener("resume",function(){cliLog("doc-resume",snap());});
  window.addEventListener("online",function(){cliLog("online",{});});
  window.addEventListener("offline",function(){cliLog("offline",{});});
  if("serviceWorker" in navigator){
    navigator.serviceWorker.addEventListener("controllerchange",function(){cliLog("sw-controllerchange",{hasController:!!navigator.serviceWorker.controller});});
    cliLog("sw-state-init",{hasController:!!navigator.serviceWorker.controller,readyState:document.readyState});
  }
})();
...
(function(){
  ...
  function checkServer(){
    ...
    var trig=checkServer.trigger||"load";
    cliLog("pending-nav-fetch",{tokenPrefix:t.slice(0,16),trigger:trig});
    // Pass trigger in the URL so the server-side endpoint logs it; gives
    // us a reliable record of which event fired the check even if the
    // page-side cliLog beacon gets dropped during page unload.
    return fetch("/api/pending-nav?token="+encodeURIComponent(t)+"&trigger="+encodeURIComponent(trig)).then(function(r){return r.ok?r.json():null;}).then(function(d){
