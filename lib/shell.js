...
(function(){
  ...
  function describeFiles(files){
    ...
    var title;
    if(files.length===1){
      if(files[0].endsWith(".step"))title="STEP updated";
      else if(files[0].endsWith(".mmd"))title="Diagram updated";
      else title="File updated";
    }else if(stepN===files.length){title=files.length+" STEPs updated";}
    else if(mmdN===files.length){title=files.length+" diagrams updated";}
    else{title=files.length+" files updated";}
    var head=files.slice(0,3).join(", ");
    var body=files.length===1?files[0]:(files.length>3?head+", …":head);
