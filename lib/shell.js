...
// stale redirects.
(function(){if(window.location.pathname!=="/")return;var t;try{t=localStorage.getItem("hsmFcmToken");}catch(e){return;}if(!t)return;fetch("/api/pending-nav?token="+encodeURIComponent(t)).then(function(r){return r.ok?r.json():null;}).then(function(d){if(!d||!d.url)return;try{var u=new URL(d.url,window.location.origin);if(u.href!==window.location.href)window.location.replace(u.href);}catch(e){}}).catch(function(){});})();
</script>
<meta charset="utf-8">
