...
<script>
// SW reload bridge. When the SW navigates an existing window via
// notificationclick to a URL whose pathname matches the current page
// (e.g. /blog#post-NEW while we're already on /blog#post-OLD),
// browsers treat that as a same-document hash change and don't
// refetch — user sees the stale content. The SW posts this message
// after navigate(); we reload to force a fresh fetch. For real
// path-changing navigations the page has already reloaded by the
// time this would fire and the message is harmlessly missed.
(function(){if(!("serviceWorker" in navigator))return;navigator.serviceWorker.addEventListener("message",function(e){if(e.data&&e.data.type==="reload-after-navigate")window.location.reload();});})();
</script>
<meta charset="utf-8">
