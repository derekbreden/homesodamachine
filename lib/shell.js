...
//
// On the cold-launch path, iOS already opens the PWA at fcmOptions.link
// directly, so the page lands on the target — the fetch still runs and
...
// In-app toast for foreground push. We can't suppress the iOS PWA system
// banner from this code path (would require switching to data-only FCM
// payloads + a heartbeat-based foreground detector in the SW — deferred),
// so the system banner still shows. A foreground-tap of that banner is a
// no-op on iOS PWA, which leaves the user with no way to act on the push
// — the toast IS that way: tap to navigate, ✕ to dismiss.
//
// On same-URL pending-nav events (e.g. push for the STEP file you're
// already viewing) we reload instead of toasting. Camera state is
// preserved across reload via the step-camera:* localStorage entries the
// dev viewer already writes. No toast for that case — the reload itself
// is the response.
(function(){
  if(!("EventSource" in window))return;
  var token;try{token=localStorage.getItem("hsmFcmToken");}catch(e){return;}
  if(!token)return;
  ...
  function showToast(title,body,url){
    ...
    var place=function(){
      ...
      link.href=url;
      var t=document.createElement("div");t.className="hsm-toast-title";t.textContent=title||"Update";
  ...
  es.addEventListener("message",function(ev){
    ...
    if(msg.type!=="pending-nav"||!msg.url)return;
    try{
      var u=new URL(msg.url,window.location.origin);
      if(u.href===window.location.href){window.location.reload();return;}
      showToast(msg.title,msg.body,u.href);
    }catch(e){}
  });
})();
</script>
