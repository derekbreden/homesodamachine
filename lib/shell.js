...
html.notifs-enabled .site-nav a.nav-bell { display: inline-flex; }
.site-nav a.nav-bell + a.nav-gear { margin-left: 0.875rem; }
.site-nav a.nav-bell svg,
...
/* When the bell is showing, the gear no longer needs the auto-margin
   (handled by the bell+gear sibling rule above). When the bell is
   hidden, .nav-gear keeps its auto-margin from this rule. */
.site-nav a.nav-bell.has-unread::after {
  content: "";
  position: absolute;
  top: -1px;
  right: -3px;
...
const BELL_SVG = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"></path><path d="M13.73 21a2 2 0 0 1-3.46 0"></path></svg>`;
...
export function renderNav({ surface = "public", active = null, devPrefix = "/dev" }) {
  ...
  // Bell is hidden by default via CSS (.nav-bell { display: none }) and
  // unhidden when html.notifs-enabled is set by HEAD_TAGS — i.e. only
  // for users who have an FCM token in localStorage. The .has-unread
  // class on the bell, set by HEAD_TAGS from the notifications fetch,
  // toggles the accent-colored dot.
  return `<nav class="site-nav ${surfaceCls}" id="site-nav" aria-label="Primary">
${items}
  <a href="/notifications" class="nav-bell${bellActive}" data-nav="notifications" aria-label="Notifications">${BELL_SVG}</a>
