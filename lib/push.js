...
export function mountPushRoutes(app) {
  ...
  // One log line per call so we can see in the deploy log whether warm
  // taps are hitting this endpoint and what they're getting back.
  app.get("/api/pending-nav", async (req, res) => {
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        console.log(`[pending-nav] HIT token=${tokenPrefix} url=${rows[0].url}`);
