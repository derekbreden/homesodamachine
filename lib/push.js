...
export async function notifyFilesChanged({ files }) {
  ...
  // STEP files live on /3d, mermaid files on /charts. Pick the deep link
  ...
  // (mixed → "files"); inferred from the file list rather than passed
  // separately so callers don't have to think about it.
  const stepCount = files.filter((f) => f.endsWith(".step")).length;
  const mermaidCount = files.filter((f) => f.endsWith(".mmd")).length;
