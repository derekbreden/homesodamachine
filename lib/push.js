...
// SSE broadcast hook — set by initPush. Lets fanOutToTokens deliver a
// real-time pending-nav signal to a foregrounded PWA whose FCM token
// just got a push. Optional; if unset (or no SSE subscriber for the
// token), the page-side /api/pending-nav fetch on focus is the fallback.
let broadcastToTokenFn = null;
...
export function initPush({ databasePool, serviceAccountJson, broadcastToToken } = {}) {
  pool = databasePool || null;
  if (typeof broadcastToToken === "function") broadcastToTokenFn = broadcastToToken;
...
export function mountPushRoutes(app) {
  ...
  // window.focus. Single-use: row is deleted on read. 5-minute TTL keeps
...
async function fanOutToTokens(tokens, message, errorContext, link) {
  ...
  // Pull title/body off the FCM message so we can replay them in the
  // SSE-driven in-app toast (see shell.js HEAD_TAGS). Every notify*
  // function sets these on `notification` already; we just forward.
  const ssePayload = link
    ? {
        type: "pending-nav",
        url: link,
  ...
  for (const row of tokens) {
    try {
      ...
      if (link) {
        ...
        if (broadcastToTokenFn && ssePayload) {
          try { broadcastToTokenFn(row.token, ssePayload); } catch {}
        }
      }
    } catch (e) {
      const code = e.code || "";
