...
export function mountEvents(app, { commit = "unknown" } = {}) {
  ...
  // Per-token broadcast: only writes to subscribers whose EventSource was
  // opened as /api/events?token=<this token>. Used by the FCM send path
  // to deliver a real-time pending-nav signal to the foregrounded PWA
  // whose token just got a push, so it can show an in-app toast (system
  // banner tap is a no-op on iOS PWA in foreground).
  function broadcastToToken(token, msg) {
    if (!token) return;
    const payload = `data: ${JSON.stringify(msg)}\n\n`;
    for (const res of subscribers) {
      if (res.__hsmToken !== token) continue;
      try {
        res.write(payload);
      } catch {
        // ignore; close handler will remove
  ...
  app.get("/api/events", (req, res) => {
    ...
    res.__hsmToken = String(req.query.token || "");
    subscribers.add(res);
  ...
  return { broadcast, broadcastToToken, subscribers };
}
